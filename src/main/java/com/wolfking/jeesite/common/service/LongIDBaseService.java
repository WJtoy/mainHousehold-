...
public abstract class LongIDBaseService {
	 ...
	 * 数据范围过滤  2016-6-14
	 * @param user 当前用户对象，通过“entity.getCurrentUser()”获取
	 * @param companyAlias 公司表别名
	 * @param officeAlias 机构表别名
	 * @param userAlias 用户表别名，多个用“,”逗号隔开，传递空，忽略此参数
	 * @return 标准连接条件对象
	 */
	public static String dataScopeFilter(User user, String companyAlias,String officeAlias, String userAlias) {
		...
		if (!user.isAdmin()){
			boolean isDataScopeAll = false;
			for (Role r : user.getRoleList()){
				//for (String oa : StringUtils.split(officeAlias, ",")){  // mark on 2016-6-14 
					if (!dataScope.contains(r.getDataScope())){   //&& StringUtils.isNotBlank(companyAlias)){
						if (Role.DATA_SCOPE_ALL.equals(r.getDataScope())){
							isDataScopeAll = true;
						}
//						else if (Role.DATA_SCOPE_COMPANY_AND_CHILD.equals(r.getDataScope())){
//							sqlString.append(" OR " + companyAlias + ".id = '" + user.getCompany().getId() + "'");
//							sqlString.append(" OR " + companyAlias + ".parent_ids LIKE '" + user.getCompany().getParentIds() + user.getCompany().getId() + ",%'");
//						}
//						else if (Role.DATA_SCOPE_COMPANY.equals(r.getDataScope())){
//							sqlString.append(" OR " + companyAlias + ".id = '" + user.getCompany().getId() + "'");
//							// 包括本公司下的部门 （type=1:公司；type=2：部门）
//							sqlString.append(" OR (" + companyAlias + ".parent_id = '" + user.getCompany().getId() + "' AND " + companyAlias + ".type = '2')");
//						}
						else if (Role.DATA_SCOPE_OFFICE_AND_CHILD.equals(r.getDataScope())){
...
//							}
	...
	public static void dataScopeFilter(BaseEntity<?> entity, String sqlMapKey, String officeWheres, String userWheres) {
		...
		// 获取到最大的数据权限范围
		long roleId = 0; //zhoucy: String roleId = "";
