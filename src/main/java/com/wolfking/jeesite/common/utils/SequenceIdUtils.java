...
public class SequenceIdUtils {
     ...
     * 构造函数，从配置文件中读取参数
     */
    public SequenceIdUtils() {
        long workerId = Long.parseLong(Global.getConfig("sequence.workerid"));
        long dataCenterId = Long.parseLong(Global.getConfig("sequence.datacenterid"));
...
//        }
     ...
     */
    public SequenceIdUtils(long workerId,long dataCenterId) {
        if (workerId > maxWorkerId || workerId < 0) {
            throw new IllegalArgumentException(String.format("worker Id can't be greater than %d or less than 0", maxWorkerId));
        }
