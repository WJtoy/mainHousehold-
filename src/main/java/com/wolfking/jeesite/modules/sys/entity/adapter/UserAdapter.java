...
public class UserAdapter extends TypeAdapter<User> {
        ...
        while (in.hasNext()) {
            switch (in.nextName()) {
                ...
                case "attributesMap":
                    if (in.peek() == JsonToken.NULL) {
                        in.nextNull();
                        model.setAttributesMap(new HashMap<>());
                    } else {
                        Type type = new TypeToken<Map<Integer, String>>(){}.getType();
                        Map<Integer, String> maps = GsonUtils.getInstance().getGson().fromJson(in.nextString(), type);
                        model.setAttributesMap(maps);
                    }
                    break;
                case "loginDate":
                    if(in.peek() == JsonToken.NULL){
    ...
    public void write(final JsonWriter out, final User model) throws IOException {
        ...
        if (true == model.isCustomer() && model.getAttributesMap() != null && !model.getAttributesMap().isEmpty()) {
            out.name("attributesMap").value(GsonUtils.toGsonString(model.getAttributesMap()));
        }
