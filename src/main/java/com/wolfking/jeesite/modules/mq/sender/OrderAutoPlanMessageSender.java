...
import com.wolfking.jeesite.modules.mq.conf.OrderAutoPlanRetryMessageConfig;
...
@Slf4j
@Component
public class OrderAutoPlanMessageSender implements RabbitTemplate.ConfirmCallback {

    private RabbitTemplate rabbitTemplate;
    private RetryTemplate retryTemplate;

    @Autowired
    public OrderAutoPlanMessageSender(RabbitTemplate manualRabbitTemplate, RetryTemplate kklRabbitRetryTemplate) {
     ...
     * @param times   第几次发送
     */
    public void sendRetry(MQOrderAutoPlanMessage.OrderAutoPlan message, int delay, int times) {
        try {
            retryTemplate.execute((RetryCallback<Object, Exception>) context -> {
                ...
                rabbitTemplate.convertAndSend(
                        OrderAutoPlanRetryMessageConfig.MQ_ORDER_AUTO_PLAN,
                        OrderAutoPlanRetryMessageConfig.MQ_ORDER_AUTO_PLAN,
                ...
                LogUtils.saveLog("MQOrderAutoPlanMessage","retry",msgJson,throwable,null);
                return null;
            });
        } catch (Exception e) {
            if(message == null) {
            ...
            try {
                LogUtils.saveLog("MQOrderAutoPlanMessage","retry",msgJson,e,null);
            }catch (Exception e1){
