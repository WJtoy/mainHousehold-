...
import com.wolfking.jeesite.modules.customer.rpt.service.CtCustomerNewOrderDailyRptService;
...
@Controller
@RequestMapping(value = "${adminPath}/customer/rpt/customerNewOrderDaily/")
public class CtCustomerNewOrderDailyRptController extends BaseRptController {

    @Autowired
    private CtCustomerNewOrderDailyRptService ctCustomerNewOrderDailyRptService;
    ...
    public String customerNewOrderDailyRpt(RptSearchCondition rptSearchCondition, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if (user.isCustomer()) {
            ...
            return "modules/customer/rpt/ctCustomerNewOrderDailyReport";
        }
        Page<RPTCustomerNewOrderDailyRptEntity> page = new Page<>(request, response);
        if (rptSearchCondition.isSearching()) {
            page = ctCustomerNewOrderDailyRptService.getCustomerNewOrderDailyRptList(page, customerId, saleId, query);
        }
    ...
    public AjaxJsonEntity checkExportTask(RptSearchCondition rptSearchCondition) {
        ...
        try {
            ...
            if (customerId != 0L) {
                ctCustomerNewOrderDailyRptService.checkRptExportTask(customerId, saleId, endDate, user);
            } else {
                result.setSuccess(false);
                result.setMessage("创建报表导出任务失败：权限不足");
            }
    ...
    public AjaxJsonEntity export(RptSearchCondition rptSearchCondition) {
        ...
        try {
            ...
            if (customerId != 0L) {
                ctCustomerNewOrderDailyRptService.createRptExportTask(customerId, saleId, endDate, user);
