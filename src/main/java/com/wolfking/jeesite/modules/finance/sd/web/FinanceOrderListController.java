...
import com.wolfking.jeesite.modules.finance.sd.service.FinanceOrderListService;
 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/finance/sd/orderList/")
@Slf4j
public class FinanceOrderListController extends BaseController {
    ...
    @Autowired
    private FinanceOrderListService financeOrderListService;

    private static final String MODEL_ATTR_PAGE = "page";
    private static final String MODEL_ATTR_ORDER = "order";

    private static final String VIEW_NAME_PLANNING_LIST = "modules/finance/sd/orderList/planingList";
    private static final String VIEW_NAME_NOAPPOINTMENT_LIST = "modules/finance/sd/orderList/noAppointmentList";
    private static final String VIEW_NAME_ARRIVEAPPOINTMENT_LIST = "modules/finance/sd/orderList/arriveAppointmentList";
    private static final String VIEW_NAME_PASSAPPOINTMENT_LIST = "modules/finance/sd/orderList/passAppointmentList";
    private static final String VIEW_NAME_PENDING_LIST = "modules/finance/sd/orderList/pendingList";
    private static final String VIEW_NAME_SERVICED_LIST = "modules/finance/sd/orderList/servicedList";
    private static final String VIEW_NAME_FOLLOWUP_FAIL_LIST = "modules/finance/sd/orderList/followUpFailList";
    private static final String VIEW_NAME_UMCOMPLETED_LIST = "modules/finance/sd/orderList/uncompletedList";
    private static final String VIEW_NAME_ALL_LIST = "modules/finance/sd/orderList/allList";
    private static final String VIEW_NAME_COMPLETED_LIST = "modules/finance/sd/orderList/completedList";
    private static final String VIEW_NAME_REMINDER_LIST = "modules/finance/sd/orderList/reminderList";

    private static final String VIEW_NAME_TMALLANOMALY_LIST = "modules/finance/sd/orderList/tmallAnomalyList";
    private static final String VIEW_NAME_TMALLSERVICEMONITOR_LIST = "modules/finance/sd/orderList/tmallServiceMonitorList";
    private static final String VIEW_NAME_RUSHING_LIST = "modules/finance/sd/orderList/rushingList";
    private static final String VIEW_NAME_COMPLAIN_LIST = "modules/finance/sd/orderList/complainList";
    ...
    private OrderSearchModel setSearchModel(User user,OrderSearchModel searchModel,Model model ,
        ...
        String checkRegion = financeOrderListService.loadAndCheckUserRegions(searchModel,user);
    ...
    public String planingList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            page = financeOrderListService.findKefuPlaningOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
    ...
    public String noAppointmentList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            ...
            page = financeOrderListService.findKefuNoAppointmentOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String arriveAppointmentList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = financeOrderListService.findKefuArriveAppointmentOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String passAppointmentList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = financeOrderListService.findKefuPassAppointmentOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String pendinglist(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            loadUrgentLevels(order);
            page = financeOrderListService.findKefuPendingOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String servicedList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = financeOrderListService.findKefuServicedOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String followUpFailList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = financeOrderListService.findKefuFollowUpFailOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String uncompletedList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = financeOrderListService.findKefuUncompletedOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String allList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if (request.getMethod().equalsIgnoreCase("post") || findNoticeMessage) {
            ...
            try {
                ...
                page = financeOrderListService.findKefuAllOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
            } catch (Exception e) {
                addMessage(model, "错误：" + e.getMessage());
            }
        } else {
    ...
    public String completedList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if (request.getMethod().equalsIgnoreCase("post")) {
            ...
            try {
                page = financeOrderListService.findKefuCompletedOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
            } catch (Exception e) {
                addMessage(model, "错误：" + e.getMessage());
            }
        } else {
    ...
    public String tmallAnomalyList(TmallAnomalyRecourseSearchVM searchEntity, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        String checkRegion = financeOrderListService.loadAndCheckUserRegions(searchEntity,user);
        if(StringUtils.isNotEmpty(checkRegion)){
            addMessage(model, checkRegion);
            model.addAttribute("page", page);
            model.addAttribute("searchEntity", searchEntity);
        ...
        try {
            //查询
            page = financeOrderListService.findKefuTmallAnomalyList(new Page<>(request, response, DEFAULT_PAGE_SIZE), searchEntity);
        } catch (Exception e) {
    ...
    public String tmallServiceMonitorList(TmallServiceMonitorSearchVM tmallServiceMonitor, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        String checkRegion = financeOrderListService.loadAndCheckUserRegions(tmallServiceMonitor,user);
        if(StringUtils.isNotEmpty(checkRegion)){
            addMessage(model, checkRegion);
            model.addAttribute("page", page);
            model.addAttribute("entity", tmallServiceMonitor);
        ...
        page = financeOrderListService.findKefuTmallServiceMonitorList(new Page<>(request, response, DEFAULT_PAGE_SIZE), tmallServiceMonitor);
        model.addAttribute("entity", tmallServiceMonitor);
        model.addAttribute(MODEL_ATTR_PAGE, page);
        return VIEW_NAME_TMALLSERVICEMONITOR_LIST;
    }
    ...
    public String rushingList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = financeOrderListService.findKefuRushingOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String complainList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = financeOrderListService.findKefuComplainOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String reminderList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            ...
            page = financeOrderListService.findReminderOrderLit(new Page<OrderSearchModel>(request, response, DEFAULT_PAGE_SIZE), order,true);
            //加急
            loadUrgentLevels(order);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
    ...
    public String historyPlanList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        User user = UserUtils.getUser();
        page.setPageSize(20);
        String checkRegion = financeOrderListService.loadAndCheckUserRegions(order,user);
        ...
        page = financeOrderListService.getHistoryPlanListForKefu(page, order);
        model.addAttribute("page", page);
