...
import com.wolfking.jeesite.modules.finance.md.dao.FiCustomerCurrencyDao;
import com.wolfking.jeesite.modules.finance.md.dao.FiCustomerFinanceDao;
...
public class FiCustomerService {
    ...
    @Resource
    private FiCustomerFinanceDao fiCustomerFinanceDao;

    @Resource
    private FiCustomerCurrencyDao fiCustomerCurrencyDao;
    ...
    public void save(Customer customer, Double oldCredit) throws Exception {
        ...
        if (isNew) {
            ...
            finance.preInsert();
            fiCustomerFinanceDao.insert(finance);
            ...
            customer.getFinance().setId(customerId);
            finance.preUpdate();
            fiCustomerFinanceDao.update(finance);
            //修改客户主帐号
        ...
        if (!isNew) {
            try {
                ...
                if (!oldCredit.equals(customer.getFinance().getCredit())) {
                    ...
                    fiCustomerCurrencyDao.insert(customerCurrency);
                }
            } catch (Exception e) {
    ...
    public void saveCustomerFinance(CustomerFinance customerFinance, Integer type) {
        if (customerFinance.getId() != null) {
            ...
            if (type == 1) {
                fiCustomerFinanceDao.updatePublic(customerFinance);
            } else if (type == 2) {
                fiCustomerFinanceDao.updatePrivate(customerFinance);
            }
        }
    }
    ...
    public void deleteById(long id) {
        ...
        fiCustomerFinanceDao.deleteById(id);
    }
    ...
    public CustomerFinance getFinance(long id) {
        //切换为微服务
        CustomerFinance finance = fiCustomerFinanceDao.get(id);
