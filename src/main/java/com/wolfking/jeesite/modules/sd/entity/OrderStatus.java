...
public class OrderStatus implements Serializable {
    ...
    public static String STATUS_TITLE_IN_CANCEL_APPLY_RESPONSIBLE = "申请退单前的状态:[";
...
//    private User engineer;// 安维主账号派单给安维子账号 (engineer_id,原来的sub engineer)
    ...
    public String getRealCancelApplyComment() {
        String result = "";
        if (StrUtil.isNotEmpty(cancelApplyComment)) {
            String[] arr = StrUtil.split(cancelApplyComment, OrderStatus.STATUS_TITLE_IN_CANCEL_APPLY_RESPONSIBLE);
            if (arr.length == 2) {
                result = arr[0];
                if (StrUtil.endWith(result, ",")) {
     ...
     * @return
     */
    public Dict getStatusInCancelApplyComment() {
        Dict result = new Dict("20", "");
        if (StrUtil.isNotEmpty(cancelApplyComment)) {
            String temp;
            String[] arr = StrUtil.split(cancelApplyComment, OrderStatus.STATUS_TITLE_IN_CANCEL_APPLY_RESPONSIBLE);
            if (arr.length == 2) {
                temp = arr[1];
                if (StrUtil.endWith(temp, "]")) {
                    temp = StrUtil.removeSuffix(temp, "]");
                }
                arr = StrUtil.split(temp, "-");
                if (arr.length == 2) {
                    result.setValue(arr[1]);
                    result.setLabel(arr[0]);
                } else {
                    result.setLabel(temp);
                }
            }
        }
        return result;
