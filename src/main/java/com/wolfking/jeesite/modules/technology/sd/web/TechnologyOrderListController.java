package com.wolfking.jeesite.modules.technology.sd.web;
...
import com.wolfking.jeesite.modules.technology.sd.service.TechnologyOrderListService;
 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/technology/sd/orderList/")
@Slf4j
public class TechnologyOrderListController extends BaseController {
    ...
    @Autowired
    private TechnologyOrderListService technologyOrderListService;

    private static final String MODEL_ATTR_PAGE = "page";
    private static final String MODEL_ATTR_ORDER = "order";

    private static final String VIEW_NAME_PLANNING_LIST = "modules/technology/sd/orderList/planingList";
    private static final String VIEW_NAME_NOAPPOINTMENT_LIST = "modules/technology/sd/orderList/noAppointmentList";
    private static final String VIEW_NAME_ARRIVEAPPOINTMENT_LIST = "modules/technology/sd/orderList/arriveAppointmentList";
    private static final String VIEW_NAME_PASSAPPOINTMENT_LIST = "modules/technology/sd/orderList/passAppointmentList";
    private static final String VIEW_NAME_PENDING_LIST = "modules/technology/sd/orderList/pendingList";
    private static final String VIEW_NAME_SERVICED_LIST = "modules/technology/sd/orderList/servicedList";
    private static final String VIEW_NAME_FOLLOWUP_FAIL_LIST = "modules/technology/sd/orderList/followUpFailList";
    private static final String VIEW_NAME_UMCOMPLETED_LIST = "modules/technology/sd/orderList/uncompletedList";
    private static final String VIEW_NAME_ALL_LIST = "modules/technology/sd/orderList/allList";
    private static final String VIEW_NAME_COMPLETED_LIST = "modules/technology/sd/orderList/completedList";
    private static final String VIEW_NAME_REMINDER_LIST = "modules/technology/sd/orderList/reminderList";

    private static final String VIEW_NAME_TMALLANOMALY_LIST = "modules/technology/sd/orderList/tmallAnomalyList";
    private static final String VIEW_NAME_TMALLSERVICEMONITOR_LIST = "modules/technology/sd/orderList/tmallServiceMonitorList";
    private static final String VIEW_NAME_RUSHING_LIST = "modules/technology/sd/orderList/rushingList";
    private static final String VIEW_NAME_COMPLAIN_LIST = "modules/technology/sd/orderList/complainList";
    ...
    private OrderSearchModel setSearchModel(User user,OrderSearchModel searchModel,Model model ,
        ...
        String checkRegion = technologyOrderListService.loadAndCheckUserRegions(searchModel,user);
    ...
    public String planingList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            page = technologyOrderListService.findKefuPlaningOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
    ...
    public String noAppointmentList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            ...
            page = technologyOrderListService.findKefuNoAppointmentOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String arriveAppointmentList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = technologyOrderListService.findKefuArriveAppointmentOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String passAppointmentList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = technologyOrderListService.findKefuPassAppointmentOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String pendinglist(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            loadUrgentLevels(order);
            page = technologyOrderListService.findKefuPendingOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String servicedList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = technologyOrderListService.findKefuServicedOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String followUpFailList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = technologyOrderListService.findKefuFollowUpFailOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String uncompletedList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = technologyOrderListService.findKefuUncompletedOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String allList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if (request.getMethod().equalsIgnoreCase("post") || findNoticeMessage) {
            ...
            try {
                ...
                page = technologyOrderListService.findKefuAllOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
            } catch (Exception e) {
                addMessage(model, "错误：" + e.getMessage());
            }
        } else {
    ...
    public String completedList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if (request.getMethod().equalsIgnoreCase("post")) {
            ...
            try {
                page = technologyOrderListService.findKefuCompletedOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
            } catch (Exception e) {
                addMessage(model, "错误：" + e.getMessage());
            }
        } else {
    ...
    public String tmallAnomalyList(TmallAnomalyRecourseSearchVM searchEntity, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        String checkRegion = technologyOrderListService.loadAndCheckUserRegions(searchEntity,user);
        if(StringUtils.isNotEmpty(checkRegion)){
            addMessage(model, checkRegion);
            model.addAttribute("page", page);
            model.addAttribute("searchEntity", searchEntity);
        ...
        try {
            //查询
            page = technologyOrderListService.findKefuTmallAnomalyList(new Page<>(request, response, DEFAULT_PAGE_SIZE), searchEntity);
        } catch (Exception e) {
    ...
    public String tmallServiceMonitorList(TmallServiceMonitorSearchVM tmallServiceMonitor, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        String checkRegion = technologyOrderListService.loadAndCheckUserRegions(tmallServiceMonitor,user);
        if(StringUtils.isNotEmpty(checkRegion)){
            addMessage(model, checkRegion);
            model.addAttribute("page", page);
            model.addAttribute("entity", tmallServiceMonitor);
        ...
        page = technologyOrderListService.findKefuTmallServiceMonitorList(new Page<>(request, response, DEFAULT_PAGE_SIZE), tmallServiceMonitor);
        model.addAttribute("entity", tmallServiceMonitor);
        model.addAttribute(MODEL_ATTR_PAGE, page);
        return VIEW_NAME_TMALLSERVICEMONITOR_LIST;
    }
    ...
    public String rushingList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = technologyOrderListService.findKefuRushingOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String complainList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            //加急
            loadUrgentLevels(order);
            page = technologyOrderListService.findKefuComplainOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order, true);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
        model.addAttribute(MODEL_ATTR_PAGE, page);
    ...
    public String reminderList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            ...
            page = technologyOrderListService.findReminderOrderLit(new Page<OrderSearchModel>(request, response, DEFAULT_PAGE_SIZE), order,true);
            //加急
            loadUrgentLevels(order);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
    ...
    public String historyPlanList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        User user = UserUtils.getUser();
        page.setPageSize(20);
        String checkRegion = technologyOrderListService.loadAndCheckUserRegions(order,user);
        ...
        page = technologyOrderListService.getHistoryPlanListForKefu(page, order);
        model.addAttribute("page", page);
