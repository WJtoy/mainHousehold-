...
@Service
public class MSCrushCoverageRptService {

    @Autowired
    private MSCrushCoverageRptFeign msCrushCoverageRptFeign;

    @Autowired
    private ReportExportTaskService reportExportTaskService;

    public List<RPTCrushCoverageEntity> getCrushCoverAreasRptData(List<Long> productCategoryIds) {
        ...
        MSResponse<List<RPTCrushCoverageEntity>> msResponse = msCrushCoverageRptFeign.getCrushCoverageList(rptGradedOrderSearch);
    ...
    public void createRptExportTask(List<Long> productCategoryIds,User user) {
        String reportTitle = "突击区域报表";
        RPTGradedOrderSearch rptGradedOrderSearch = new RPTGradedOrderSearch();
        rptGradedOrderSearch.setProductCategoryIds(productCategoryIds);
        String searchConditionJson = RedisGsonUtils.toJson(rptGradedOrderSearch);
        reportExportTaskService.createRptExportTask(RPTReportEnum.CRUSH_COVERAGE_RPT, RPTReportTypeEnum.OTHER_REPORT, user, reportTitle, searchConditionJson);
    }
