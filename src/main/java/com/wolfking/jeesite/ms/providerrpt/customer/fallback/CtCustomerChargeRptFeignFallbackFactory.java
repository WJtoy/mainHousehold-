...
@Component
@Slf4j
public class CtCustomerChargeRptFeignFallbackFactory implements FallbackFactory<CtCustomerChargeRptFeign> {

    @Override
    public CtCustomerChargeRptFeign create(Throwable throwable) {

        if(throwable != null){
            log.error("CtCustomerChargeRptFeignFallbackFactory:{}",throwable.getMessage());
        }

       return new CtCustomerChargeRptFeign() {
           @Override
