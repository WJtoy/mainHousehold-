...
@Service
public class MSKAReminderResponseTimeRptService {
    @Autowired
    private MSKAReminderRptFeign mskaReminderRptFeign;
    ...
    public Page<RPTReminderResponseTimeEntity> getReminderResponseTimeList(Page<RPTReminderResponseTimeEntity> page, String orderNo, Long areaId, String reminderNo,
                                                                           Integer reminderTimes, Date beginDate, Date endDate,Long customerId) {
        ...
        if (beginDate != null && endDate != null) {
            ...
            if (microServicesProperties.getReport().getEnabled()) {
                MSResponse<MSPage<RPTReminderResponseTimeEntity>> msResponse = mskaReminderRptFeign.getReminderResponseTimeList(search);
    ...
    public void checkRptExportTask(String orderNo,Long areaId,String reminderNo,
                                   Integer reminderTimes,Date beginDate,Date endDate,Long customerId,User user) {

        RPTReminderResponseTimeSearch searchCondition = setSearchCondition(orderNo,areaId,reminderNo,reminderTimes,beginDate,endDate,customerId);
        ...
        reportExportTaskService.checkRptExportTask(RPTReportEnum.KA_REMINDER_RESPONSETIME_RPT, user, searchConditionJson);
    }
    ...
    public void createRptExportTask(String orderNo,Long areaId,String reminderNo,
        ...
        searchCondition.setPageNo(1);
        searchCondition.setPageSize(200000);
        String reportTitle = DateUtils.formatDate(beginDate, "yyyy年MM月dd日") + "~" + DateUtils.formatDate(endDate, "yyyy年MM月dd日")+ "KA回复时效报表";
        String searchConditionJson = RedisGsonUtils.toJson(searchCondition);
        reportExportTaskService.createRptExportTask(RPTReportEnum.KA_REMINDER_RESPONSETIME_RPT, RPTReportTypeEnum.ORDER_REPORT, user, reportTitle, searchConditionJson);
    }
