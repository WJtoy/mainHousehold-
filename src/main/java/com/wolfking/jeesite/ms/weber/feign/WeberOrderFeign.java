package com.wolfking.jeesite.ms.weber.feign;
 ...
 * 威博B2B微服务接口调用
 */
@FeignClient(name = "kklplus-b2b-weber", fallbackFactory = WeberOrderFeignFallbackFactory.class)
public interface WeberOrderFeign {
    ...
    MSResponse orderPlanned(@RequestBody WeberOrderPlanned orderPlanned);

    /**
     * 预约服务时间
     */
    @PostMapping("/orderAppointed/appointed")
    MSResponse orderAppointed(@RequestBody WeberOrderAppointed orderAppointed);

    /**
     * 完成服务
     */
    @PostMapping("/orderCompleted/completed")
    MSResponse orderCompleted(@RequestBody WeberOrderCompleted orderCompleted);

    /**
     * 取消工单
     */
    @PostMapping("/orderCancelled/cancelled")
    MSResponse orderCancelled(@RequestBody WeberOrderCancelled orderCancelled);
    ...
    MSResponse saveLog(@RequestBody WeberOrderLog orderLog);
}
