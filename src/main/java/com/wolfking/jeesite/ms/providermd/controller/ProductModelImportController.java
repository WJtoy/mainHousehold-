...
@Slf4j
public class ProductModelImportController extends BaseController {
    ...
    public String importForm(CustomerProductModel customerProductModel, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        List<CustomerBrand> customerBrandList = Lists.newArrayList();
        if(customerProductModel.getCustomerId() != null && customerProductModel.getCustomerId() > 0){
            customerBrandList = customerBrandService.findAllList(searchCustomerBrand);
        }

        if (erroFlag){
            model.addAttribute("customerProductModel", customerProductModel);
            model.addAttribute("customerBrandList",customerBrandList);
//            return "modules/providermd/productModelImportForm";
            return "modules/providermd/productModelImportFormNew";
        }
...
//        return "modules/providermd/productModelImportForm";
    ...
    public String readExcel(MultipartFile file, @RequestParam(required = false) String customerId,@RequestParam(required = false) String brandId, RedirectAttributes redirectAttributes, HttpServletRequest request)
    {
        ...
        if(file == null){
            ...
            return "redirect:" + Global.getAdminPath() + "/provider/md/customerProductModel/importForm?repage";
        }
        ...
        if (StringUtils.isBlank(brandId)){
            ...
            customerBrand = customerBrandService.getById(Long.valueOf(brandId));
        }

        String key =String.format(RedisConstant.MD_TMP_PRODUCT_MODEL,user.getId());
        //移除上次读取数据
        redisUtils.remove(RedisConstant.RedisDBType.REDIS_MD_DB,key);
        List<Product> customerProductList = productService.getCustomerProductList(Long.valueOf(customerId));
