...
public class B2BPushOrderProcessLogToMSMQSender implements RabbitTemplate.ConfirmCallback {
     ...
     * @param times   第几次发送
     */
    public void sendRetry(MQB2BOrderProcessLogMessage.B2BOrderProcessLogMessage message, int times) {
        try {
            retryTemplate.execute((RetryCallback<Object, Exception>) context -> {
                context.setAttribute(B2BMQConstant.RETRY_CONTEXT_ATTRIBUTE_KEY_MESSAGE, message);
                rabbitTemplate.convertAndSend(
                        B2BMQConstant.MQ_B2BCENTER_PUSH_ORDERPROCESSLOG_TO_MS_RETRY,
                        B2BMQConstant.MQ_B2BCENTER_PUSH_ORDERPROCESSLOG_TO_MS_RETRY,
                ...
                Throwable throwable = context.getLastThrowable();
                String msgJson = new JsonFormat().printToString(msg);
                B2BFailureLogUtils.saveFailureLog(B2BMQQueueType.B2BCENTER_PUSH_ORDERPROCESSLOG_TO_MS_RETRY, msgJson, msg.getCreateById(),
                        B2BProcessFlag.PROCESS_FLAG_FAILURE, 0,
            ...
            String msgJson = new JsonFormat().printToString(message);
            B2BFailureLogUtils.saveFailureLog(B2BMQQueueType.B2BCENTER_PUSH_ORDERPROCESSLOG_TO_MS_RETRY, msgJson, message.getCreateById(),
