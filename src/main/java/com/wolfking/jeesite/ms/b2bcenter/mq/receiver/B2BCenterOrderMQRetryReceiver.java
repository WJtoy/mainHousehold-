...
public class B2BCenterOrderMQRetryReceiver implements ChannelAwareMessageListener {
    ...
    public void onMessage(Message message, Channel channel) throws IOException {
        ...
        try {
            //b2BOrderMsg = MQB2BOrderMessage.B2BOrderMessage.parseFrom(message.getBody());//去除重复代码
            ...
            if (b2BOrderMsg != null && StringUtils.isNotBlank(b2BOrderMsg.getOrderNo()) && B2BDataSourceEnum.isB2BDataSource(b2BOrderMsg.getDataSource())) {
                ...
                LogUtils.saveLog("B2B自动转单失败:消息体错误", "B2BCenterOrderMQRetryReceiver.onMessage", "", null, user);
            }
        } catch (Exception e) {
            if (b2BOrderMsg != null) {
                String msgJson = new JsonFormat().printToString(b2BOrderMsg);
                LogUtils.saveLog("B2B自动转单失败", "B2BCenterOrderMQRetryReceiver.onMessage", msgJson, e, user);
            } else {
                LogUtils.saveLog("B2B自动转单失败", "B2BCenterOrderMQRetryReceiver.onMessage", "", e, user);
            }
        }
    }
}
