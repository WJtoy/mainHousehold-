...
<head>
	...
	<script type="text/javascript">
		...
		function showHistoryPlanList(areaLevel){
			...
			if(areaLevel === 2){
				parameters = parameters +  "&area.id="+ areaId + "&area.name=" + ${fns:urlEncode(areaName)} + "&area.parent.id=0";
			}else if(areaLevel === 3){
				parameters = parameters +  "&area.id="+ subAreaId + "&area.name=" + ${fns:urlEncode(subAreaName)} + "&area.parent.id=" + areaId;// + "&area.parent.name=" + areaName;
			}
        ...
        function showServicePointAdd(areaId,productCategoryId,address,subAreaId) {
            ...
            window.location.href="${ctx}/md/servicepoint/addServicePointForPlanForm?dialogType=layer&area.id=" +areaId + '&productCategoryId='+ productCategoryId +"&address=" + encodeURI(address) +"&layerIndex=" + layerIndex + "&parentLayerIndex=" +parentLayerIndex +"&subArea.id=" + subAreaId;
        }
        function unableServicePoint(areaId,productCategoryId,address,subAreaId) {
            var layerIndex = $("#layerIndex").val();
            var parentLayerIndex = $("#parentLayerIndex").val();
            window.location.href="${ctx}/md/servicepoint/unableSelectForPlan?dialogType=layer&area.id=" +areaId + '&productCategoryId='+ productCategoryId +"&address=" + encodeURI(address) +"&layerIndex=" + layerIndex + "&parentLayerIndex=" +parentLayerIndex +"&subArea.id=" + subAreaId;
        }
...
<body>
   <c:set var="currentuser" value="${fns:getUser()}" />
	<ul id="navtabs" class="nav nav-tabs">
		...
		<c:if test="${currentuser.isRushKefu()==true}">
	...
	<div style="margin-left:3px;margin-right:3px;">
		<form:form id="searchForm" modelAttribute="servicePoint"  action="${ctx}/md/servicepoint/selectForPlan" method="post" class="breadcrumb form-inline">
