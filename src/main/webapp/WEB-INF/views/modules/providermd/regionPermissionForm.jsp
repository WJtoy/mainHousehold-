...
<head>
    ...
    <script type="text/javascript">
        ...
        function child(obj) {
            var data = eval(obj);
            $("#subCityId").val(data.cityId);
            $("#type").val(data.type);
            // $("#provinceId").val(data.areaId);
        }
        ...
        function searchCategory() {
            ...
            var productCategoryId = options.val();// 品类id
            if (productCategoryId == '' || productCategoryId <= 0) {
                layerError("请选择品类", "错误提示");
                return false;
            }
            var loadingIndex;
            var options = {
                url: "${ctx}/provider/md/regionPermission/findRegionPermission",
                type: 'post',
                dataType: 'json',
        ...
        $(document).ready(function () {
            ...
            $("#btnSubmit").click(function () {
                ...
                $("input[type='checkbox'][name='subArea']").each(function (index, element) {
                    ...
                    entity['regionPermissions[' + index + '].type'] = parseInt(type);
                    if (checked) {
                ...
                var loadingIndex;
                var options = {
                    url: "${ctx}/provider/md/regionPermission/save",  //默认是form的action， 如果申明，则会覆盖
...
<form:form id="search_form" modelAttribute="regionPermission" action="" method="post" class="bread_crumb form-search">
    ...
    <label><span class=" red">*</span>品&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;类：</label>
    <select id="productCategory" name="productCategoryId" class="input-small" style="width:150px;">
        <option value=""/>
        请选择</option>
    ...
    &nbsp;
    <input id="btnSeach" type="button" class="layui-btn layui-btn-sm" onclick="searchCategory();" value="查询"
           style="width: 64px;height: 32px;background: #0096DA;"/>
    <input type="hidden" id="subCityId" name="cityId">
</form:form>

<sys:message content="${message}"/>

<input type="hidden" id="subCityId">
<%--省id--%>
                                ...
                                <c:otherwise>
                                    <td style="width: 75%;line-height: 35px;">
                                        <%--<div style="width: 20%;float: left;">--%>
                                            <%--<input type="checkbox" name="subArea" lay-filter="subArea" data-countyid="${areaCounty.id}"--%>
                                                   <%--value="${areaCounty.id}" lay-skin='primary'>--%>
                                        <%--</div>--%>
                                    </td>
                                </c:otherwise>
                            </c:choose>
...
<div id="editBtn">
    ...
    <input id="btnCancel" class="btn layui-btn layui-btn-primary" type="button" value="关 闭"
