...
<head>
    ...
    <script type="text/javascript" language="JavaScript">
        function validate(f) {
            var b = $('#areaIdLevel').val();
            console.log(b);
            if (b == "0") {
                top.$.jBox.error("区域不能只选择省份","错误提示");
                top.$.jBox.closeTip();
                return false;
            } else {
                return true;
    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function() {
            ...
            $("#btnSubmit").click(function(){
                ...
                $("#searchForm").attr("action","${ctx}/rpt/provider/specialChargeArea/countyList");
                $("#searchForm").submit();
            });
                        ...
                        if(data && data.success == true){
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/rpt/provider/specialChargeArea/countyExport?"+ (new Date()).getTime(),
                                        data:$(searchForm).serialize(),
                                        success: function (data) {
                                            if(ajaxLogout(data)){
                                                return false;
...
<ul class="nav nav-tabs">
    ...
    <li>
        <a href="${ctx}/rpt/provider/specialChargeArea/cityList">省市特殊费用分布表</a>
    </li>

    <li class="active"><a href="javascript:void(0);">区/县特殊费用分布表</a></li>
</ul>
<form:form id="searchForm"  modelAttribute="rptSearchCondition"  action="${ctx}/rpt/provider/specialChargeArea/countyList" method="post" class="breadcrumb form-search" onsubmit="return validate(this)">
    <div style="width: 90%">
