    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function () {
            ...
            $("#btnSubmit").click(function () {
                ...
                $("#searchForm").attr("action", "${ctx}/rpt/provider/complainCompleteRatio/provinceComplainCompleteOrderRpt");
                $("#searchForm").submit();
            });
                        ...
                        if(data && data.success == true){
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/rpt/provider/complainCompleteRatio/provinceComplainCompletedExport?"+ (new Date()).getTime(),
                                        data:$(searchForm).serialize(),
                                        success: function (data) {
                                            if(ajaxLogout(data)){
                                                return false;
...
<ul class="nav nav-tabs">
    <li class="active"><a href="javascript:void(0);">省投诉率</a></li>
    <li>
...
<form:form id="searchForm" modelAttribute="rptSearchCondition"
           action="${ctx}/rpt/provider/complainCompleteRatio/provinceComplainCompleteOrderRpt" method="post" class="breadcrumb form-search">
    <div style="width: 90%;">
            ...
            <c:forEach items="${list}" var="pItem">
                ...
                <c:choose>
                    <c:when test="${rowIndex < list.size()}">
                        <tr>
                            <td>
                                <a href="${ctx}/rpt/provider/complainCompleteRatio/cityComplainCompleteOrderRpt?provinceId=${pItem.provinceId}&selectedYear=${rptSearchCondition.selectedYear}&selectedMonth=${rptSearchCondition.selectedMonth}&areaName=${pItem.provinceName}">
                            ...
                            <c:forEach var="i" begin="1" end="${rptSearchCondition.days}" step="1">
                                ...
                                <td>
                                    <fmt:formatNumber maxFractionDigits="0">${pItem[dlname]}</fmt:formatNumber>
                                </td>
                                <td>
                                    <fmt:formatNumber maxFractionDigits="0">${pItem[alname]}</fmt:formatNumber>
                                </td>
                                <td style="color: red;">
                                    <fmt:formatNumber maxFractionDigits="2"> ${pItem[dlname] eq 0 ? pItem[alname]*100 : pItem[alname]/pItem[dlname]*100}</fmt:formatNumber>%
                                </td>
                                <td>
                                    <fmt:formatNumber maxFractionDigits="0">${pItem[clname]}</fmt:formatNumber>
                                </td>
                                <td style="color: red;">
                                    <fmt:formatNumber maxFractionDigits="2"> ${pItem[dlname] eq 0 ? pItem[clname]*100 :pItem[clname]/pItem[dlname]*100}</fmt:formatNumber>%
                                </td>
                            </c:forEach>
                            <td>
                            ...
                            <td>
                                <fmt:formatNumber maxFractionDigits="0"> ${pItem.totalAmount}</fmt:formatNumber>
                            </td>
                            <td style="color: red;">
                                <fmt:formatNumber maxFractionDigits="2"> ${pItem.total eq 0 ? pItem.totalAmount*100 : pItem.totalAmount/pItem.total*100}</fmt:formatNumber>%
                            </td>
                            <td>
                                <fmt:formatNumber maxFractionDigits="0"> ${pItem.evaluateAmount}</fmt:formatNumber>
                            </td>
                            <td style="color: red;">
                                <fmt:formatNumber maxFractionDigits="2"> ${pItem.total eq 0 ? pItem.evaluateAmount*100 : pItem.evaluateAmount/pItem.total*100}</fmt:formatNumber>%
                            </td>
                        </tr>
                    </c:when>
                    <c:otherwise>
