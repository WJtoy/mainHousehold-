    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function () {
            ...
            $("#btnSubmit").click(function () {
                ...
                $("#searchForm").attr("action", "${ctx}/rpt/provider/uncompletedOrder/uncompletedOrderReport");
                $("#searchForm").submit();
            });
            ...
            $("#btnExport").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "${ctx}/rpt/provider/uncompletedOrder/checkExportTask?"+ (new Date()).getTime(),
                        ...
                        if(data && data.success == true){
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/rpt/provider/uncompletedOrder/export?"+ (new Date()).getTime(),
                                        data:$(searchForm).serialize(),
                                        success: function (data) {
                                            if(ajaxLogout(data)){
                                                return false;
...
<div id="divGrid" style="overflow: auto;">
    <table id="contentTable" class="table table-striped table-bordered table-condensed table-hover" style="table-layout:fixed; margin-top: 0px;border-top-width: 0px;">
        ...
        <c:forEach items="${page.list}" var="orderMaster">
            ...
            <td rowspan="${orderMaster.maxRow}">${orderMaster.customer.name}</td>
            ...
            <c:forEach begin="0" end="${orderMaster.maxRow-1}" var="i">
                ...
                <c:if test="${i eq 0}">
                    ...
                    <td rowspan="${orderMaster.maxRow}"><fmt:formatDate value="${orderMaster.closeDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
