    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function () {
            ...
            $("#btnSubmit").click(function () {
                $("#pageNo").val(1);
                $("#searchForm").attr("action", "${ctx}/rpt/provider/detailedReconciliation/getNetworkReconciliationNew");
                $("#searchForm").submit();
            });
            ...
            $("#btnExport").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "${ctx}/rpt/provider/detailedReconciliation/checkExportTaskNew?" + (new Date()).getTime(),
                        ...
                        if (data && data.success == true) {
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/rpt/provider/detailedReconciliation/exportNew?" + (new Date()).getTime(),
                                        data: $(searchForm).serialize(),
                                        success: function (data) {
                                            if (ajaxLogout(data)) {
                                                return false;
...
<form:form id="searchForm" modelAttribute="rptSearchCondition" action="${ctx}/rpt/provider/detailedReconciliation/getNetworkReconciliationNew" method="post" class="breadcrumb form-search" onsubmit="return validate(this)">
...
<table id="treetable" class="fancyTable datatable table table-bordered table-condensed" style="table-layout:fixed"
    ...
    <table id="contentTable" class="table  table-bordered table-condensed" style="table-layout:fixed;">
        ...
        <c:forEach items="${page.list[0].list}" var="item">
            ...
            <c:forEach begin="0" end="${item.maxRow-1}" var="i">
                ...
                <c:if test="${i eq 0}">
                    ...
                    <td rowspan="${item.maxRow}">${item.engineerTotalCharge+item.engineerInsuranceCharge+item.engineerUrgentCharge+item.engineerTimelinessCharge+item.engineerCustomerTimelinessCharge+item.engineerTaxFee+item.engineerInfoFee+item.engineerPraiseFee}</td>
                    ...
                    <c:set var="totalCharge"
                           value="${totalCharge+item.engineerTotalCharge +item.engineerInsuranceCharge+item.engineerUrgentCharge+item.engineerTimelinessCharge+item.engineerCustomerTimelinessCharge+item.engineerTaxFee+item.engineerInfoFee+item.engineerPraiseFee}"/>
                </c:if>
        ...
        <c:if test="${page.list[0].list.size()>0}">
            <tr>
                ...
                <td style="color: red;">${totalTravelCharge}</td>
                ...
                <td style="color: red;">${engineerTaxFee}</td>
                <td style="color: red;">${engineerInfoFee}</td>
