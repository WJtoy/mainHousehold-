    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function () {
            ...
            $("#btnSubmit").click(function () {
                ...
                $("#searchForm").attr("action", "${ctx}/rpt/provider/keFuCompleteTime/keFuCompleteTimeReport");
                $("#searchForm").submit();
            });
            ...
            $("#btnExport").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "${ctx}/rpt/provider/keFuCompleteTime/checkExportTask?" + (new Date()).getTime(),
                        ...
                        if (data && data.success == true) {
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/rpt/provider/keFuCompleteTime/export?" + (new Date()).getTime(),
                                        data: $(searchForm).serialize(),
                                        success: function (data) {
                                            if (ajaxLogout(data)) {
                                                return false;
...
<ul class="nav nav-tabs">
    ...
    <li>
        <a href="${ctx}/rpt/provider/keFuCompleteTime/keFuCompleteTimeChart">完成时效图表</a>
    </li>
</ul>
<c:set var="currentuser" value="${fns:getUser() }" />
<form:form id="searchForm" modelAttribute="rptSearchCondition" action="${ctx}/rpt/provider/keFuCompleteTime/keFuCompleteTimeReport" method="post" class="breadcrumb form-search">
    <div style="width: 90%">
...
<div id="divGrid" style="overflow-x:hidden;">
    <table id="contentTable" class="table  table-bordered table-condensed table-hover" style="table-layout:fixed;">
        ...
        <tr>
            ...
            <th colspan="4" width="400">24小时</th>
            <th colspan="4" width="400">48小时</th>
            <th colspan="4" width="400">72小时</th>
            <th colspan="4" width="400">72小时外</th>
        ...
        <c:forEach items="${rptSearchCondition.list}" var="item">
            ...
            <tr>
                ...
                <td>${item.cancel24hour}</td>
                <td>${item.cancel24hourRate}%</td>
                <td>${item.complete48hour + item.complete24hour}</td>
                <td>${item.complete48hourRate}%</td>
                <td>${item.cancel48hour + item.cancel24hour}</td>
                <td>${item.cancel48hourRate}%</td>
                <td>${item.complete72hour + item.complete48hour + item.complete24hour}</td>
                <td>${item.complete72hourRate}%</td>
                <td>${item.cancel72hour + item.cancel48hour + item.cancel24hour}</td>
                <td>${item.cancel72hourRate}%</td>
                <td>${item.overComplete72hour}</td>
                <td>${item.overComplete72hourRate}%</td>
                <td>${item.overCancel72hour}</td>
                <td>${item.overCancel72hourRate}%</td>
