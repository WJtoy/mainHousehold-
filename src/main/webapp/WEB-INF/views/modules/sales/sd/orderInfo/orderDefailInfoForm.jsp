...
<head>
	...
	<%@ include file="/WEB-INF/views/modules/sales/sd/orderInfo/tpl/orderDetail.html" %>
	<%@ include file="/WEB-INF/views/modules/sales/sd/orderInfo/tpl/orderCrushList.html" %>
	<%@ include file="/WEB-INF/views/modules/sales/sd/orderInfo/tpl/orderAuxiliaryMaterialList.html" %>
	<%@ include file="/WEB-INF/views/modules/sales/sd/orderInfo/tpl/reminderConfirmItem.html" %>
	<style type="text/css">
...
<script type="text/javascript">
    ...
    function updateFeedback(feedbackId, title) {
        ...
        if (btnFeedback) {
            ...
            btnFeeback.attr("onclick", "SalesOrderService.replylist('" + feedbackId + "','${order.quarter}');")
            btnFeeback.html(title);
        }
    }

    function reloadComplain() {
        SalesOrderService.showComplainList('${order.id}','${order.quarter}');
    }
	//申诉成功回调方法
        ...
        layer.confirm(
            ...
            ,{icon: 3, title:'系统确认',success: function(layro, index) {
                    $(document).on('keydown', layro, function(e) {
                        if (e.keyCode == 13) {
                            SalesOrderService.doCancelComplain('${order.id}',complainId,quarter);
...
<body>
	...
	<form:form id="inputForm" action="#" method="post" class="form-horizontal">
		...
		<div class="accordion-group" style="margin-top:2px;">
			<div class="accordion-heading">
				...
				<a class="btn btn-primary btn-mini" id="btnFinishPhoto" href="javascript:;" onclick="SalesOrderService.browsePhotolist('${order.id}','${order.quarter}');" title="点击浏览完成照片">完成照片</a>
				</c:if>
				<!-- 问题反馈-->
				<c:choose>
					<c:when test="${order.orderCondition.feedbackId >0 }">
						<a class="btn btn-warning btn-mini" id="btnReply" style="width: 150px;" href="javascript:;" onclick="SalesOrderService.replylist('${order.orderCondition.feedbackId}','${order.quarter}','${order.orderNo}');" title="点击查看反馈内容">${fns:abbr(order.orderCondition.feedbackTitle,25)}</a>
					</c:when>
					<c:otherwise>
						<a class="btn btn-warning btn-mini" id="btnFeedback" style="width: 150px;" href="javascript:;" onclick="SalesOrderService.feedback('${order.id}','${order.quarter}',orderdetail_index);" title="点击添加反馈内容">问题反馈</a>
					</c:otherwise>
				</c:choose>
	...
	<div class="tabbable" style="margin:0px 20px">
		<ul class="nav nav-tabs">
			...
			<li>
                <a href="#tabTracking" data-toggle="tab" id="tab4link" onclick="SalesOrderService.showTrackingLogs('${order.id}','${order.quarter}','${isCustomer}');">跟踪进度</a>
            </li>
			<%--<shiro:hasPermission name="sd:orderdetail:exception">
				<li><a href="#tabException" data-toggle="tab" id="tab5link" onclick="SalesOrderService.showExceptLogs('${order.id}','${order.quarter}');">异常处理</a></li>
			</shiro:hasPermission>--%>
			<c:if test="${order.orderCondition.feedbackId != 0}">
			<li><a href="#tabFeedback" data-toggle="tab" onclick="SalesOrderService.showFeedbackLogs('${order.orderCondition.feedbackId}','${order.quarter}');">反馈处理</a></li>
			</c:if>
			<c:if test="${order.writeOff == 1 or order.writeOff == 3}">
                <c:set var="tabCount" value="${tabCount+1}"/>
				<li>
                    <a href="#tabCustomerReturn" data-toggle="tab" onclick="SalesOrderService.showCustomerReturnAndAdditionalList('${order.id}','${order.orderNo}','${order.quarter}');">退补</a>
                </li>
			</c:if>
			<%-- 投诉转移到orderStatus--%>
			<c:if test="${order.orderStatus.complainFlag>0}">
				...
				<li id="litabComplain">
					<a href="#tabComplain" data-toggle="tab" id="lnktabComplain" onclick="SalesOrderService.showComplainList('${order.id}','${order.quarter}');">投诉</a></li>
				</li>
			</c:if>
            <c:if test="${order.orderCondition.rushOrderFlag!=0}">
			  <%--  <shiro:hasAnyPermissions name="sd:orderdetail:crush,sd:orderCrush:edit,sd:ordercrush:view">
                    <c:set var="tabCount" value="${tabCount+1}"/>
					<li id="litabCrush">
						<a href="#tabOrderCrush" data-toggle="tab" id="lnktabCrush" onclick="SalesOrderService.crush_showList('${order.id}','${order.quarter}');">突击单</a></li>
					</li>
            ...
            <c:if test="${order.orderStatus.reminderStatus > 0}">
                ...
                <li id="litabReminder">
                    <a href="#tabReminder" data-toggle="tab" id="lnktabReminder" onclick="SalesOrderService.showReminderListForCustomer('${order.id}','${order.quarter}',undefined);">催单</a></li>
                </li>
            </c:if>
			<c:if test="${hasAuxiliaryMaterils == 1}">
				<c:set var="tabCount" value="${tabCount+1}"/>
				<li id="litabAuxiliaryMaterials" class="${tabCount==1?'active':''}">
					<a href="#tabAuxiliaryMaterials" data-toggle="tab" id="lnktabAuxiliaryMaterials" onclick="SalesOrderService.auxiliaryMaterial_showDetailInfo('${order.id}','${order.quarter}');">辅材收费</a></li>
				</li>
			</c:if>
		...
		<script type="text/javascript">
            ...
            function showPraise(orderId,quarter,servicePointId) {
                ...
                $.ajax({
                    cache: false,
                    type: "GET",
                    url: "${ctx}/sales/sd/orderInfo/ajax/getPraiseInfo?orderId=" + orderId +"&quarter=" + (quarter || '') + "&servicePointId=" + servicePointId,
                    dataType: 'json',
