...
<ul id="navtabs" class="nav nav-tabs">
    <c:forEach items="${dataSourceList}" var="dict">
        <c:choose>
            ...
            <c:otherwise>
                <li><a href="${ctx}/sales/sd/b2bOrder/b2bOrderAllList?dataSource=${dict.value}">${dict.label}</a>
            </c:otherwise>
        </c:choose>
        </li>
    </c:forEach>
...
<script type="text/javascript">
    function transferManual(dataSource, b2bOrderId, b2bOrderNo, quarter) {
        ...
        var planIndex = top.layer.open({
            ...
            zIndex: 19891016,
            title: 'B2B订单人工处理',
            content: "${ctx}/sales/sd/b2bOrder/manual?dataSource=" + dataSource + "&b2bOrderId=" + (b2bOrderId || '0') + "&b2bOrderNo=" + (b2bOrderNo || '') + "&quarter=" + (quarter || ''),
    ...
    function cancelOrderTransition(dataSource, b2bOrderId, b2bOrderNo, quarter, comment) {
        var candelIndex = top.layer.open({
            ...
            zIndex: 19891015,
            title: '取消订单',
            content: "${ctx}/sales/sd/b2bOrder/cancelOrderTransitionForm?dataSource=" + dataSource + "&b2bOrderId=" + (b2bOrderId || '0') + "&b2bOrderNo=" + (b2bOrderNo || '') + "&quarter=" + (quarter || '') + "&comment=" + (comment || ''),
            area: ['650px', '300px'],
            shade: 0.3,
            maxmin: false,
            success: function (layero, index) {
    ...
    function directlyCancelOrderTransition(dataSource, b2bOrderId, b2bOrderNo) {
        top.layer.confirm('确定要取消工单吗?', {icon: 3, title: '系统确认'}, function (index) {
            ...
            $.ajax({
                cache: false,
                type: "POST",
                url: "${ctx}/sales/sd/b2bOrder/directlyCancelOrderTransition",
    ...
    function ignoreOrderTransition(dataSource, b2bOrderId, b2bOrderNo) {
        top.layer.confirm('确定要忽略工单吗?', {icon: 3, title: '系统确认'}, function (index) {
            ...
            $.ajax({
                cache: false,
                type: "POST",
                url: "${ctx}/sales/sd/b2bOrder/ignoreOrderTransition",
                data: {dataSource: dataSource, b2bOrderId: b2bOrderId, b2bOrderNo:b2bOrderNo},
                success: function (data) {
                    top.layer.close(loadingIndex);
                    if (ajaxLogout(data)) {
    ...
    $('#btnTransfer').click(function () {
        ...
        top.layer.confirm('确定转换所选的订单吗?', {icon: 3, title: '系统确认'}, function (index) {
            ...
            $.ajax({
                cache: false,
                type: "POST",
                url: "${ctx}/sales/sd/b2bOrder/transfer",
    ...
    $('#btnCancelBatch').click(function () {
        ...
        top.layer.confirm('确定取消所选的订单吗?', {icon: 3, title: '系统确认'}, function (index) {
            ...
            $.ajax({
                cache: false,
                type: "POST",
                url: "${ctx}/sales/sd/b2bOrder/cancelOrderTransitionBatch",
                data: JSON.stringify(orders),
                dataType: 'json',
                contentType: "application/json;charset=utf-8",
                success: function (data) {
    ...
    $('#btnUpdateOrderAbnormalFlagBatch').click(function () {
        ...
        top.layer.confirm('确定更新所有订单的状态吗?', {icon: 3, title: '系统确认'}, function (index) {
            ...
            $.ajax({
                cache: false,
                type: "POST",
                url: "${ctx}/sales/sd/b2bOrder/updateOrderAbnormalFlagBatch?dataSource="+datasource,
                dataType: 'json',
                contentType: "application/json;charset=utf-8",
                success: function (data) {
                    if (ajaxLogout(data)) {
