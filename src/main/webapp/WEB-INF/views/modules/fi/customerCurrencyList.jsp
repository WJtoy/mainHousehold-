...
<head>
	...
	<%--<link href="${ctxStatic}/grid/list.css" type="text/css" rel="stylesheet" />--%>
	<%--<script src="${ctxStatic}/grid/BoxSelect.js" type="text/javascript"></script>--%>
	<%--<script src="${ctxStatic}/grid/Grid.js" type="text/javascript"></script>--%>
	...
	<script type="text/javascript">
		$(document).ready(function() {
            ...
            $("#btnSubmit").on("click", function(){
                ...
                var url = "${ctx}/fi/customercurrency/list";
...
<sys:message content="${message}"/>
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="fi:blockamountlist:view"><li><a href="${ctx}/fi/customercurrency/blockamountlist">冻结查询</a></li></shiro:hasPermission>
	...
	<form:form id="searchForm" modelAttribute="customerCurrency" action="${ctx}/fi/customercurrency/list" method="post" class="breadcrumb form-search">
	...
	<table id="treeTable" class="table table-striped table-bordered table-condensed table-hover">
		...
		<tr>
			...
			<th>创建时间</th>
			<th>修改时间</th>
			<shiro:hasPermission name="fi:customercurrency:view">
		...
		<c:forEach items="${page.list}" var="customerCurrency">
			...
			<c:if test="${!(customerCurrency.customer.id eq customerId) }">
				...
				<tr id="${customerCurrency.customer.id}" pId="">
					...
					<td>
						<shiro:hasPermission name="fi:customercurrency:charge">
							<a href="${ctx}/fi/customercurrency/form?customer.id=${customerCurrency.customer.id}">充值</a>
						</shiro:hasPermission>
					</td>
					<td></td>
				</tr>
			...
			<tr id="${customerCurrency.id}" pId="${customerId}">
				...
				<shiro:hasPermission name="fi:customercurrency:view"><td>
    				<a href="${ctx}/fi/customercurrency/form?id=${customerCurrency.id}">查看</a>
				</td>
				</shiro:hasPermission>
			</tr>
		</c:forEach>
