...
<head>
    ...
    <script type="text/javascript" language="javascript">
        ...
        function joyoungCompletedRetry(id,b2bInterfaceId) {
            var self = this;
            var canboResend = top.layer.open({
                type: 2,
                id: 'layer_joyoung_comlete_retry',
                zIndex: 19891015,
                title: '九阳完工重发',
                content: "${ctx}/b2b/rpt/processlog/joyoungFailLogRetryForm?id=" + id + "&b2bInterfaceId=" + (b2bInterfaceId || ''),
        ...
        function joyoungCancelRetry(id,b2bInterfaceId){
            var self = this;
            var canboResend = top.layer.open({
                type: 2,
                id:'layer_joyoung_cancel_retry',
                zIndex:19891015,
                title:'九阳取消重发',
                content: "${ctx}/b2b/rpt/processlog/joyoungCancelForm?id=" + id + "&b2bInterfaceId=" + (b2bInterfaceId || ''),
                area: ['1255px', screen.height-200+'px'],
                shade: 0.3,
                shadeClose:true,
                maxmin: false,
...
<ul class="nav nav-tabs">
    ...
    <li class="active"><a href="javascript:void(0);">九阳</a></li>
    <li>
        <a href="${ctx}/b2b/rpt/processlog/viomiFailLog">云米</a>
    </li>
</ul>
<form:form id="searchForm" modelAttribute="searchModel" action="${ctx}/b2b/rpt/processlog/joyoungFailLog"
           method="post" class="breadcrumb form-search">
    <input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
    <input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
    <div>
        <label class="label-search">工单号：</label>
        <input type=text class="input-small" id="kklOrderNo" name="kklOrderNo" value="${searchModel.kklOrderNo}" maxlength="20" />
        &nbsp;
        <label class="label-search">客户单号：</label>
        <input type=text class="input-small" id="b2bOrderNo" name="b2bOrderNo" value="${searchModel.b2bOrderNo}" maxlength="30" />
        &nbsp;
        <label>操作：</label>
        ...
        <select id="b2bInterfaceId" name="b2bInterfaceId" class="input-small" style="width:240px;">
            <c:forEach items="${b2bInterfaceId}" var="dict">
                <option value="${dict.value}" <c:out
                        value="${(dict.value == searchModel.b2bInterfaceId)?'selected=selected':''}"/> >${dict.label}</option>
            </c:forEach>
        </select>
        &nbsp;&nbsp;
        <label>传送时间：</label>
