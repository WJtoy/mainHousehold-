...
<head>
    ...
    <script type="text/javascript">
        ...
        function userViewNotice(id,title){
            top.layer.open({
                type: 2,
                id:'layer_viewNotice',
                zIndex:19891015,
                title:title || '通知详情',
                content: "${ctx}/im/notice/user/view?id=" + id,
                    area: ['750px','90%'],
                shade: 0.3,
                maxmin: false,
    ...
    <table id="contentTable" class="table table-bordered table-condensed" style="table-layout:fixed;">
        ...
        <c:forEach items="${page.list}" var="item">
            <tr>
                ...
                <td>
                <c:choose>
                    <c:when test="${item.isReaded == 1}">${item.notice.title}</c:when>
                    <c:otherwise><a href="javascript:void(0);" style="color: red;"><abbr title="未读">${item.notice.title}</abbr></a></c:otherwise>
                </c:choose>
                </td>
                <td>${item.notice.subTitle}</td>
                <td>
                    <a href="javascript:" >${fns:abbr(item.notice.content,150)}</a>
                </td>
                <td>
                    ${fns:formatDateString(item.notice.createAt)}
                </td>
                <td>
                    <c:if test="${item.isReaded == 1}">
                        ${fns:formatDateString(item.readAt)}
                    </c:if>
                </td>
                <td>
                    <a href="javascript:void(0);" class="btn btn-mini btn-primary"
                       onclick="javascript:userViewNotice('${item.id}','${item.notice.title}');">查看</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
