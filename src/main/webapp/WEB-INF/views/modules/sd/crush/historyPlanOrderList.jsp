...
<body>
<form:form id="searchForm" modelAttribute="orderCrush" method="post" action="${ctx}/sd/order/crush/historyPlanList" class="form-inline">
    ...
    <div class="control-group">
        ...
        <%--
        <sys:treeSelectAnyArea id="area" name="area.id" value="${orderCrush.area.id}"
                               levelValue="${orderCrush.areaLevel}" parentValue="${orderCrush.area.parent.id}"
                               labelName="area.name" labelValue="${orderCrush.area.name}"
                               title="区域" clearIdValue="0" allowClear="false"
                               url="/sys/area/treeData"  nodesLevel="2" nameLevel="4" canSelectLevel="2"/>
                               --%>

        <sys:treeselectareanew id="area" name="area.id" value="${orderCrush.area.id}" levelValue="${orderCrush.areaLevel}"  parentValue="${orderCrush.area.parent.id}"
...
<table id="contentTable" class="table table-striped table-bordered table-condensed table-hover">
    ...
    <c:forEach items="${page.list}" var="order">
        ...
        <tr>
            ...
            <td rowspan="${rowspan}"><a href="javascript:void(0);" onclick="Order.viewOrderDetail('${order.orderId}','${order.quarter}');">${order.orderNo}</a></td>
            <td rowspan="${rowspan}">
