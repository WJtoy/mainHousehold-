...
<ul id="navtabs" class="nav nav-tabs">
    <shiro:hasPermission name="sd:reminder:view">
        ...
        <li class="active"><a href="javascript:void(0);" title="超48小时未完成">超48小时未完成</a></li>
        <li><a href="${ctx}/sd/kefu/reminder/list/all" title="所有催单列表">所有</a></li>
    </shiro:hasPermission>
</ul>
<c:set var="currentuser" value="${fns:getUser() }"/>
<form:form id="searchForm" modelAttribute="reminder" action="${ctx}/sd/kefu/reminder/list/moreThan48HoursUnfinishedList" method="post" class="breadcrumb form-search">
    ...
    <div style="margin-top: 8px">
        <%--<label class="label-search">创建日期：</label>
        ...
        <input id="endDate" name="endDate" type="text" readonly="readonly" style="width:97px" maxlength="20"
               ...
               onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
       &lt;%&ndash; &nbsp;&nbsp;&ndash;%&gt;
        &lt;%&ndash;<label>区域</label>
        <sys:treeSelectAnyArea id="area" name="areaId" value="${reminder.areaId}" levelValue="${reminder.areaLevel}"
                               labelName="areaName" labelValue="${reminder.areaName }" title="区域" clearIdValue="0"
                               url="/sys/area/treeData?kefu=${currentuser.id}" allowClear="true" nodesLevel="-1" nameLevel="4"/>&ndash;%&gt;
        &nbsp;&nbsp;--%>
        ...
        <sys:treeselect id="customer" name="customerId" value="${reminder.customerId}" labelName="customerName"
                        labelValue="${reminder.customerName}" cssStyle="width:205px;"
    ...
    <table id="contentTable" class="table table-striped table-bordered table-condensed table-hover" style="table-layout:fixed;">
        ...
        <c:forEach items="${page.list}" var="model">
            ...
            <tr>
                ...
                <td>
                    <c:choose>
                        <c:when test="${model.status==1}">
                            <a href="javascript:void(0);" onclick="Order.reminderToProcess('${model.id}','${model.quarter}','0');">${model.reminderNo}</a>
                        </c:when>
                        <c:otherwise>
                ...
                <td>
                    <p style="color: red">超时:${model.completeTimelinessLabel}</p>
                </td>
            </tr>
        </c:forEach>
        </tbody>
