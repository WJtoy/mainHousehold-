...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#inputForm").validate({
                rules: {
                    ...
                    materialCharge:{min: 0,max: 10000},
                    travelCharge:{min: 0,max:500},
                    otherCharge:{min: 0,max:1000},
                    expressCharge:{min: 0,max:500},
                    engineerMaterialCharge:{min: 0,max: 10000},
                ...
                submitHandler: function(form){
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        type: "POST",
                        url: "${ctx}/sd/pending/saveservice?"+ (new Date()).getTime(),
                        data:$(form).serialize(),
                        beforeSend: function () {
        ...
        function serviceproduct_callback(data){
            ...
            // $("[name='serviceCategory.value']").val(data.orderservietype);
            $("[name='serviceCategory.value']:checked").trigger('change');
            // $("#brand").focus();
        }
...
<form:form id="inputForm" modelAttribute="item" method="post" class="form-horizontal">
    ...
    <c:if test="${canAction}">
        ...
        <input type="hidden" id="areaRemoteFee" name="areaRemoteFee" value="1"/>
        <table border="0">
            <tr>
                <td colspan="2">
                    <div class="span6">
                                                ...
                                                <c:otherwise>
                                                    <c:choose>
                                                        <c:when test="${ (!empty item.id && i.index == item.serviceTimes) || (empty id && i.index == item.orderServiceTimes) }">
                                                            ...
                                                            <label for="serviceTims${i.index}">第${i.index}次</label>
                                                        </c:when>
                                                        <c:otherwise>
            ...
            <c:if test="${limitRemoteCharge != null }">
                <td>
                        ...
                        <div class="controls">
                            <c:choose>
                                <c:when test="${limitRemoteCharge != null}">
                                    <%--远程费和其他费用合计限制品类--%>
                                    <input type="text" id="otherCharge" name="otherCharge" readonly="readonly" value="0" class="input-block-level required number" />
                                </c:when>
                                <c:otherwise>
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            ...
                            <c:if test="${order.orderFee.planTravelCharge ne null}">
                                <label>补助费用:${order.orderFee.planTravelCharge }</label></c:if>
                        </div>
                    </div>
                </div>
                </td>
            ...
            <tr>
                <td><div class="span6">
                    <div class="control-group">
                        <label class="control-label">远程审核单号：</label>
                        <div class="controls">
                            <c:choose>
                                <c:when test="${limitRemoteCharge != null && limitRemoteCharge <= 0}">
                                    <form:input path="travelNo" readonly="true" htmlEscape="false" type="text" maxlength="11" cssClass="input-block-level" />
                                </c:when>
                                <c:otherwise>
                                    <form:input path="travelNo" htmlEscape="false" type="text" maxlength="11" cssClass="input-block-level" />
                                </c:otherwise>
                            </c:choose>
                        </div>
                    </div>
            ...
            <tr>
                <td colspan="2">
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <shiro:hasPermission name="sd:order:service"><input id="btnSubmit" class="btn btn-primary" type="button" value="保 存"/>&nbsp;</shiro:hasPermission>
                        </div>
                    </div>
                </td>
            </tr>
