...
<ul id="navtabs" class="nav nav-tabs">
    ...
    <li class="active"><a href="javascript:void(0);" title="待接派的工单列表">待接派单<span id="spn_order_count" class="badge badge-info">${page !=null ?page.count:''}</span></a></li>
...
<form:form id="searchForm" modelAttribute="order" method="post" class="form-inline">
    ...
    <div style="margin-top:0px;" class="control-group">
        ...
        <c:set var="statusList" value="${fns:getDictInclueListFromMS('order_status','20,30')}"/>
        ...
        <tr>
            ...
            <td rowspan="${rowspan}">
                <c:choose>
                    <c:when test="${order.isNotAccept(30)}">
                        <a href="javascript:void(0);" style="color: red;" onclick="Order.showKefuOrderDetailForPlan('${order.id}','${order.quarter}',${order.orderCondition.orderServiceType});">
                    ...
                    <c:when test="${not empty order.repeateNo}">
                        <a href="javascript:void(0);" onclick="Order.showKefuOrderDetailForPlan('${order.id}','${order.quarter}',${order.orderCondition.orderServiceType});">
            ...
            <td rowspan="${rowspan}">
                <c:if test="${order.orderCondition.partsFlag == 1}">
                    <i class="icon-cogs" title="已申请配件" style="cursor: pointer;" onclick="javascript:;"></i>
                </c:if>
            </td>

            <td rowspan="${rowspan}">
                <c:if test="${order.orderCondition.finishPhotoQty>0 }">
                    <i class="icon-camera" title="已上传完成照片" style="cursor: pointer;" onclick="javascript:;"></i>
                </c:if>
            </td>
        </tr>
