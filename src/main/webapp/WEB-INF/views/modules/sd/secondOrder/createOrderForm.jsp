...
<ul class="nav nav-tabs">
	<li ><a href="${ctx}/sd/order/form">${empty createOrderConfig.firstType.title?'下单':createOrderConfig.firstType.title }</a></li>
	<c:if test="${not empty createOrderConfig.secondType.title}">
		<li class="active"><a href="javascript:void(0);">${createOrderConfig.secondType.title}</a></li>
	</c:if>
</ul>
<input type="hidden" id="hasRepeatOrder" name="hasRepeatOrder" value="false" />
<sys:message content="${message}"/>
<c:set var="url" value="${ctx}/sd/order/second/add" />
...
<form:form id="inputForm" modelAttribute="order" action="${url}" method="post" class="form-horizontal">
    ...
    <input type="hidden" id="action" name="action" value="newv2" />
	<!-- order head -->
				...
				<div class="controls">
					<c:choose>
						<c:when test="${currentuser.isCustomer()}">
							<input type="hidden" id="customer.id" name="customer.id" value="${order.customer.id}" />
							<input type="text" id="customer.name" name="customer.name" readonly="readonly" value="${order.customer.name}" class="input-block-level" />
						</c:when>
						<c:otherwise>
							<input type="hidden" id="customer.id" name="customer.id" value="${order.customer.id}" />
							<input type="hidden" id="customer.name" name="customer.name" value="${order.customer.name}" />
							<select id="customerId" name="customerId" class="input-block-level">
		...
		<div class="span12">
			...
			<c:if test="${currentuser.isCustomer()}">
				<shiro:hasPermission name="fi:customercurrency:chargeonline">
					&nbsp;&nbsp;<a class="btn btn-mini btn-primary" target="_blank" href="${ctx}/fi/customercurrency/chargeonline">充值</a>
				</shiro:hasPermission>
			</c:if>
		</div>
	</div>
...
<%--					<a id="btnItemSubmit" class="btn btn-primary" style="margin-right: 10px;" href="javascript:;">确认</a>--%>
                    <input id="btnItemSubmit" name="btnItemSubmit" class="btn btn-primary" style="margin-right: 10px;" type="button" value="确认"/>
		...
		<div class="span8 divUrgent" id="divUrgent" style="width: auto;<c:if test="${order.urgentFlag == 0}">display:none;</c:if>">
			<div class="row">
				...
				<label class="checkbox inline">
					<form:radiobutton path="urgentLevel.id" value="0" /> 不加急
				</label>
				<c:if test="${order.urgentFlag == 1 && !empty urgentLevels}">
					<c:forEach items="${urgentLevels}" var="urgentItem">
					<label class="checkbox inline">
						<form:radiobutton path="urgentLevel.id" value="${urgentItem.id}" />${urgentItem.remarks}
					</label>
					</c:forEach>
				</c:if>
				<span class="help-inline alert-input" style="padding: 3px 3px;margin-left:10px;display:${showReceive?'':'none'}">
						加急费：<label id="lblUrgent" style="min-width:30px;display:inherit;text-align: right;">0.0</label> 元
					</span>
			</div>
			...
			<shiro:hasPermission name="sd:order:add">
				<c:choose>
					...
					<c:when test="${(order.customerBalance + order.customerCredit - order.customerBlockBalance - order.expectCharge - order.blockedCharge) <= 0}">
						<input id="btnSubmit" name="btnSubmit" class="btn btn-danger" type="submit" disabled="disabled" style="margin-right: 3px;"  value="下单"></button>
					</c:when>
					<c:otherwise>
						<input id="btnSubmit" name="btnSubmit" class="btn btn-primary" type="submit" value="下单"/>
					</c:otherwise>
				</c:choose>
			</shiro:hasPermission>
...
<script class="removedscript" id="sc_AddItemRows" type="text/javascript">
    $(document).ready(function() {
        ...
        SecondOrder.loadCustomerInfoWhenCreate(${order.customer.id});
        </c:if>
    });
</script>
