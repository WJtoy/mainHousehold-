...
<ul id="navtabs" class="nav nav-tabs">
    ...
    <shiro:hasPermission name="sd:complain:complete">
        <li class="active"><a href="javascript:void(0);" title="待完成投诉单列表">待完成</a></li>
...
<form:form id="searchForm" modelAttribute="complain" action="${ctx}/sd/complain/completelist" method="post" class="form-inline">
                ...
                <td>
                    <c:choose>
                        <c:when test="${model.status.value eq '1' and not empty model.judgeDate}">
                            <shiro:hasPermission name="sd:complain:complete">
                            <a href="javascript:void(0);" class="btn btn-mini btn-warning" onclick="Order.complain_complete('${model.id}','${model.quarter}');">完成</a>
                            </shiro:hasPermission>
                        </c:when>
                        <c:otherwise>
