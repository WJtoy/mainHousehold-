...
<body>
	<ul id="navtabs" class="nav nav-tabs">
		...
		<li><a href="${ctx}/sd/order/engineer/appointedList" title="预约中的订单列表">预约中</a></li>
	...
	<form:form id="searchForm" modelAttribute="order" action="${ctx}/sd/order/engineer/list" method="post" class="breadcrumb form-search">
		...
		<div>
			...
			<label>安维姓名：</label>
				<input type=text class="input-mini" id="engineerName" name="engineer.name" value="${order.engineer.name}" maxlength="20" />
		</div>
		<div style="margin-top: 8px;">
			...
			<label style="width:45px;text-align: center;"> ~ </label>&nbsp;&nbsp;&nbsp;<input id="acceptEndDate" name="acceptEndDate"
		...
		<tbody>
			...
			<c:forEach items="${page.list}" var="order">
				...
				<tr>
					...
					<shiro:hasPermission name="sd:order:engineeraccept">
						<td>
							<c:if test="${ order.canPlanOrder()}"><%-- <c:if test="${ order.canPlanOrder() && currentuser.isEngineer()}"> --%>
