...
<body>
    <ul class="nav nav-tabs">
        ...
        <li class="active"><a href="javascript:void(0);">樱雪</a></li>
</ul>
<form:form id="searchForm" modelAttribute="inseSearchModel" action="${ctx}/b2b/rpt/processlog/inseorder" method="post" class="breadcrumb form-search">
    ...
    <div>
        ...
        <select id="b2bInterfaceId" name="b2bInterfaceId" class="input-small" style="width:240px;">
            ...
            <c:forEach items="${b2bInterfaceId}" var="dict">
                <option value="${dict.value}" <c:out value="${(dict.value == inseSearchModel.b2bInterfaceId)?'selected=selected':''}" /> >${dict.label}</option>
            </c:forEach>
        </select>
        &nbsp;&nbsp;
        <label>状 态：</label>
        <select id="processFlag" name="processFlag" class="input-small" style="width:225px;">
            ...
            <option value="" <c:out value="${(empty inseSearchModel.processFlag)?'selected=selected':''}" />>所有</option>
            <c:forEach var="type" items="${processFlags}">
                <option value="${type.value}" <c:out value="${(type.value == inseSearchModel.processFlag)?'selected=selected':''}" /> >${type.label}</option>
            </c:forEach>
        </select>
        &nbsp;&nbsp;
        ...
        <input id="beginDate" name="beginDate" type="text" readonly="readonly" style="width:99px;margin-left:4px" maxlength="20" class="input-small Wdate"
               value="<fmt:formatDate value='${inseSearchModel.beginDate}' pattern='yyyy-MM-dd' type='date'/>"
        ...
        <input id="endDate" name="endDate" type="text" readonly="readonly" style="width:98px" maxlength="20" class="input-small Wdate"
               value="<fmt:formatDate value='${inseSearchModel.endDate}' pattern='yyyy-MM-dd' type='date'/>"
               onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
        &nbsp;&nbsp;
