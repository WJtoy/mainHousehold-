...
<ul class="nav nav-tabs">
    ...
    <li class="active"><a href="javascript:void(0);">同望</a></li>
    <li>
...
<form:form id="searchForm" modelAttribute="canboSearchModel" action="${ctx}/b2b/rpt/processlog/canboauston"
    ...
    <div>
        ...
        <select id="dataSource" name="dataSource" class="input-small" style="width:240px;">
                <%--            <option value="">所有</option>--%>

            <option value="3" <c:out value="${(3 == canboSearchModel.dataSource)?'selected=selected':''}"/>>康宝</option>
            <option value="4" <c:out value="${(4 == canboSearchModel.dataSource)?'selected=selected':''}"/> >阿诗丹顿
            </option>
                    ...
                    <c:choose>
                        <c:when test="${canboSearchModel.b2bInterfaceId eq 2006}">
                            <a style="line-height: 30px" class="input-block-level" href="javascript:void(0);"
                               onclick="Order.canboCompleteOrderResend('${item.id}','${canboSearchModel.b2bInterfaceId}')"
                               data-toggle="tooltip">${fns:abbr(item.infoJson,40)}</a>
                        </c:when>
                        <c:when test="${canboSearchModel.b2bInterfaceId eq 2005}">
                            <a style="line-height: 30px" class="input-block-level" href="javascript:void(0);"
                               onclick="Order.canboCancelOrderResend('${item.id}','${canboSearchModel.b2bInterfaceId}')"
                               data-toggle="tooltip">${fns:abbr(item.infoJson,40)}</a>
                        </c:when>
                        <c:otherwise>
                            <a style="line-height: 30px" class="input-block-level" href="javascript:void(0);"
                               onclick="Order.canboAppointmentOrderResend('${item.id}','${canboSearchModel.b2bInterfaceId}')"
                               data-toggle="tooltip">${fns:abbr(item.infoJson,40)}</a>
                        </c:otherwise>
                    </c:choose>
