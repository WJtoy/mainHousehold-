    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function () {
            ...
            $("#btnSubmit").click(function () {
                $("#pageNo").val(1);
                $("#searchForm").attr("action", "${ctx}/servicePoint/rpt/spDetailedReconciliation/getNetworkReconciliationNew");
                $("#searchForm").submit();
            });
            ...
            $("#btnExport").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "${ctx}/servicePoint/rpt/spDetailedReconciliation/checkExportTaskNew?" + (new Date()).getTime(),
                        ...
                        if (data && data.success == true) {
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/servicePoint/rpt/spDetailedReconciliation/exportNew?" + (new Date()).getTime(),
                                        data: $(searchForm).serialize(),
                                        success: function (data) {
                                            if (ajaxLogout(data)) {
                                                return false;
...
<form:form id="searchForm" modelAttribute="rptSearchCondition" action="${ctx}/servicePoint/rpt/spDetailedReconciliation/getNetworkReconciliationNew" method="post" class="breadcrumb form-search">
    ...
    <div style="width: 90%">
        ...
        &nbsp;&nbsp;
        <shiro:hasPermission name="rpt:servicePoint:getNetworkReconciliationNew:view"><input id="btnSubmit" class="btn btn-primary" type="button" value="查询"/></shiro:hasPermission>
        &nbsp;&nbsp;
        <shiro:hasPermission name="rpt:servicePoint:getNetworkReconciliationNew:export"><input id="btnExport" class="btn btn-primary" type="button" value="导出"/></shiro:hasPermission>
    </div>
    ...
    <table id="contentTable" class="table  table-bordered table-condensed" style="table-layout:fixed;">
        ...
        <c:if test="${page.list[0].list.size()>0}">
            <tr>
                ...
                <td style="color: red;">${engineerDeposit}</td>
