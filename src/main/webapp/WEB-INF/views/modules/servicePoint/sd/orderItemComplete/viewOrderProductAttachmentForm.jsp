    ...
    <script type="text/javascript">
        ...
        function uploadfile($obj1,obj2,index,type) {
            ...
            $.ajaxFileUpload({
                ...
                success: function (data, status) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "${ctx}/servicePoint/sd/orderItemComplete/saveOrderCompletePic",
                        data: {
                            filePath: filePath,
        ...
        function deletePic(fieldName,idIndex,imageId,uploadImageId) {
            ...
            top.layer.confirm('确定要删除该附件吗?', {icon: 3, title:'系统确认'}, function(index){
                ...
                $.ajax({
                    type:"post",
                    url: "${ctx}/servicePoint/sd/orderItemComplete/deletePic",
                    data: {
        ...
        function editUnitCode(index){
            ...
            top.layer.open({
                ...
                zIndex:19891015,
                title:'修改条码',
                content: "${ctx}/servicePoint/sd/orderItemComplete/editBarcode?id="+ (completePicId || '')
        ...
        function getDeleteList(orderId,quarter) {
            ...
            top.layer.open({
                ...
                zIndex:19891015,
                title:'查看已删除记录',
                content: "${ctx}/servicePoint/sd/orderItemComplete/getDelListByOrderId?orderId="+ (orderId || '') + "&quarter=" + (quarter || ''),
...
<c:if test="${addattach == 1}">
    <ul class="nav nav-tabs" style="margin-left: 24px;margin-top: 24px">
        ...
        <li><a href="${ctx}/servicePoint/receipt/praise/praiseForm?orderId=${order.id}&customerId=${order.orderCondition.customer.id}&quarter=${order.quarter}">好评图片</a></li>
    </ul>
</c:if>
<%--<legend style="margin: 0px 5px 0px 5px;position:relative">上传新的完成照片   (<a href="javascript:ServicePointOrderService.photoList('${order.id}','${order.quarter}');">老照片</a> )<label style="position:absolute;right: 5%;top: 5px;"><a href="javascript:getDeleteList('${order.id}','${order.quarter}')">查看删除记录</a></label></legend>--%>
                                ...
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                        ...
                                        <c:if test="${addattach eq 1}">
                                            <input id="btnSubmit${i.index}" class="btn btn-primary" style="margin-top: 19px" type="submit" onclick="editUnitCode('${i.index}')" value="编辑" />
                                        </c:if>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="span12">
                                <div class="control-group">
                                   ...
                                   <%-- <label class="control-label">${map.completePic.product.name}</label>--%>
                                    <div class="controls">
                                        <div class="upload_warp">
                                            <div class="alert alert-false"><button data-dismiss="alert" class="close">×</button>${map.completePic.product.name}还没有配置产品完成图片</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
