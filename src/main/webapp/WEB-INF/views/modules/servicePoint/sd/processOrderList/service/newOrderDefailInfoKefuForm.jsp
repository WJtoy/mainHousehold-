...
<head>
    ...
    <%@ include file="/WEB-INF/views/modules/servicePoint/sd/processOrderList/service/tpl/newOrderDetailKefu.html" %>
        ...
        <ul class="nav nav-tabs">
            ...
            <c:if test="${status>=50 && status<=100}">
                <li id="litabInsurance">
                    <a href="#tabInsurance" data-toggle="tab" id="lnktabInsurance" onclick="getInsurance('${order.id}','${order.quarter}')">互助基金</a></li>
                </li>
            </c:if>
        </ul>
        <!-- tab content -->
...
<script type="text/javascript">
    ...
    //互助基金
    function getInsurance(orderId,quarter) {
        var parentLayerIndex = top.layer.getFrameIndex('layer_orderdetail');
        ...
        $.ajax({
            cache: false,
            type: "GET",
            url: "${ctx}/servicePoint/sd/processOrderList/getOrderInsurance?strOrderId=" + orderId + "&quarter=" + (quarter || ''),
            dataType: 'json',
            success: function (data) {
                top.layer.close(loadingIndex);
                if(ajaxLogout(data)){
