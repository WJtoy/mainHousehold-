...
//			$("#iframe",window).load(pagestyle);
        ...
        function recoverPrice(type) {
            var text = "恢复标准价";
            var url = "${ctx}/fi/md/servicePointPrice/recover";
            top.layer.open({
                type: 2,
                id:"priceRecover",
                zIndex:19891015,
        ...
        function editPrice(sid, pid){
            var serviceRemotePriceFlag = $("#serviceRemotePriceFlag").val();
            var text = "师傅价格";
            var url = "${ctx}/fi/md/servicePointPrice/product?servicePoint.id="+sid+"&product.id="+pid + "&serviceRemotePriceFlag=" + serviceRemotePriceFlag;
            top.layer.open({
                type: 2,
        ...
        <c:forEach items="${servicePoint.servicePointPriceList}" var="servicePointPrice">
            ...
            <tr id="${servicePointPrice.productId}" pId="${servicePoint.servicePointId}">
                <td style="text-align: center">
                    <c:choose>
                        <c:when test="${serviceRemotePriceFlag == 1}">
                            <c:choose>
