    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function() {
            ...
            $("#btnSubmit").click(function(){
                ...
                $("#pageNo").val(1);
                $("#searchForm").attr("action","${ctx}/finance/rpt/customerCharge/customerChargeCompleteReport");
                $("#searchForm").submit();
            });
            $("#btnExport").click(function () {+
                top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                    if (v == "ok") {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "${ctx}/finance/rpt/customerCharge/export?" + (new Date()).getTime(),
...
<ul class="nav nav-tabs">
    <li>
        <a href="${ctx}/finance/rpt/customerCharge/customerChargeSummaryRpt">订单|消费|充值</a>
    </li>
    <li class="active"><a href="javascript:void(0);">完工单</a></li>
    <li>
        <a href="${ctx}/finance/rpt/customerCharge/returnedOrderCancelledRpt">退单/取消单</a>
    </li>
    <li>
        <a href="${ctx}/finance/rpt/customerCharge/customerChargeWriteOffRpt">退补单</a>
    </li>
...
<form:form id="searchForm" modelAttribute="rptSearchCondition" method="post" class="breadcrumb form-search" onsubmit="return validate(this)">
    ...
    <div style="90%">
        ...
        &nbsp;&nbsp;
        <shiro:hasPermission name="rpt:finance:customerCharge:view"><input id="btnSubmit" class="btn btn-primary" type="button" value="查询" /></shiro:hasPermission>
        &nbsp;&nbsp;
        <shiro:hasPermission name="rpt:finance:customerCharge:export"><input id="btnExport" class="btn btn-primary" type="button" value="导出" /></shiro:hasPermission>
    </div>
        ...
        <div style="position: relative" class="parent">
            ...
            <div style="text-align:right;position: absolute;width: 250px;height: 100px;right:1px;top:31px;" class="target">
                <div style="text-align:left;background-color: #1B1E24;border-radius: 5px;opacity: 0.8;padding: 10px;font-size:14px;color:white;min-width:100px;max-width:400px;">
                    ...
                    统计方式：审单时间<br/>
                    栏位说明：<br/>
                    【应收客户货款】应收合计=服务费+配件费+远程费+快递费+其他费用+厂商时效+加急费+好评费
                </div>
            </div>
        ...
        if($("#contentTable tbody>tr").length>0) {
            ...
            $("#contentTable").toSuperTable({
                ...
                colWidths:
                    [40,
                        140,140,120,
                        140,140, 70, 80, 80, 70, 60,140, 90, 200, 80, 110, 200,
                        90, 90 ,120,90,
...
<div id="divGrid" style="overflow: auto;">
    <table id="contentTable" class="table  table-bordered table-condensed table-hover" style="table-layout:fixed; margin-top: 0px;border-top-width: 0px;">
        ...
        <c:set var="totalDetailCharge"/>
        <c:set var="totalTimelinessCharge"/>
        <c:set var="totalCharge"/>
        <c:set var="totalUrgentCharge"/>
        <c:set var="totalPraiseFee"/>
        ...
        <c:forEach items="${page.list}" var="item">
            ...
            <c:forEach begin="0" end="${item.maxRow-1}" var="i">
                ...
                <c:if test="${i eq 0}">
                    ...
                    <td rowspan="${item.maxRow}"><fmt:formatDate value="${item.arrivalDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                    <td rowspan="${item.maxRow}"><fmt:formatDate value="${item.appCompleteDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                ...
                <c:choose>
                    <c:when test = "${i lt item.details.size()}">
                        ...
                        <td class="autocut">${item.details.get(i).product.name}</td>
                        <td class="autocut">${item.details.get(i).productSpec}</td>
                        <td class="autocut">${item.details.get(i).brand}</td>
                        <td>${item.details.get(i).qty}</td>
                        <c:set var="totalItem" value="${item.details.get(i).serviceCharge+item.details.get(i).materialCharge+item.details.get(i).travelCharge+item.details.get(i).expressCharge+item.details.get(i).otherCharge}"/>
                        <c:set var="totalActualCount" value="${totalActualCount+item.details.get(i).qty}" />
                        <c:set var="totalDetailCharge" value="${totalDetailCharge+totalItem}"/>
                ...
                <c:if test="${i eq 0}">
                    <c:set var="totalUrgentCharge" value="${totalUrgentCharge+item.customerUrgentCharge}"/>
                    <c:set var="totalTimelinessCharge" value="${totalTimelinessCharge+item.customerTimelinessCharge}"/>
                    <c:set var="totalPraiseFee" value="${totalPraiseFee+item.customerPraiseFee}"/>
                    ...
                    <td rowspan="${item.maxRow}" style="color:green;">${item.customerTotalCharge}</td>
                    <c:set var="totalInCharge" value="${totalInCharge+item.customerTotalCharge}" />
                    <td rowspan="${item.maxRow}"><span class="label status_Completed">完成</span></td>
                ...
                <c:choose>
                    <c:when test = "${i lt item.details.size()}">
                        <td>${item.details.get(i).errorType.label}</td>
                        <td>${item.details.get(i).errorCode.label}</td>
                        <td>${item.details.get(i).actionCode.label}</td>
                    </c:when>
                    <c:otherwise>
                        <td></td>
                        <td></td>
                ...
                <c:if test="${i eq 0}">
                    <td rowspan="${item.maxRow}"><fmt:formatDate value="${item.chargeDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                    </tr>
                </c:if>
            </c:forEach>
        ...
        <c:if test="${page.list.size()>0}">
            <tr>
                ...
                <th colspan="5"></th>
                <th><B>${totalDetailCharge}</B></th>
                <th><B>${totalTimelinessCharge}</B></th>
                <th><B>${totalUrgentCharge}</B></th>
                <th><B>${totalPraiseFee}</B></th>
                <th style="color:green;"><B>${totalInCharge}</B></th>
                <th colspan="6"></th>
            </tr>
        </c:if>
        </tbody>
