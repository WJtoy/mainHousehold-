        ...
        var tree, setting = {view:{selectedMulti:false,dblClickExpand:false},check:{enable:"${checked}",nocheckInherit:true},
            ...
            data:{simpleData:{enable:true}},callback:{
                onClick:function(event, treeId, treeNode){
                    ...
                    // console.log("我被你点击了.名称:"+treeNode.name+",类型："+treeNode.type);
...
//                var nodesLevel=$("#nodesLevel").attr("value");
        ...
        function selectCheckNode(){
            ...
            if(areaLevel === "3"){
                ids = "${selectParentId}".split(",");
            }
            // console.log("默认选择节点。"+ids);
            for(var i=0; i<ids.length; i++) {
                ...
                if(node && node.level === 2 && areaLevel === "3"){
                    treeArea.obtainTownDataByDistrict(tree,node,"${selectIds}");
                    <%--treeArea.fillTownDataByTownId(tree,node,ids[i],"${checked}");--%>
                }
            }
        }
        function focusKey(e) {
