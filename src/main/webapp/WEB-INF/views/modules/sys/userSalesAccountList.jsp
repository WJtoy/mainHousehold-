...
<head>
    ...
    <script type="text/javascript">
        ...
        function search() {
            $("#pageNo").val(1);
            $("#searchForm").attr("action", "${ctx}/sys/userSales/list");
            $("#searchForm").submit();
            return false;
        }
        ...
        function editUser(id) {
            ...
            var text = "添加业务";
            var url = "${ctx}/sys/userSales/form";
            var height = (h-200);
            var area = ['900px',(h-200)+'px'];
            if (id != null){
                text = "修改";
                url = "${ctx}/sys/userSales/form?id=" + id;
                area = ['900px',(h-200)+'px'];
            }
            top.layer.open({
                type: 2,
        ...
        function removeUser(userId,userName){
            var typeName = ('业务员');
            layer.confirm(
                '确认要删除业务员' +'<label style="color:#63B9E6">'+ userName +'</label>吗？',
                {
                    btn: ['确定','取消'], //按钮
                    title:'提示'
                }, function(index){
...
<ul class="nav nav-tabs">
    <li class="active"><a href="javascript:void(0);">业务</a></li>
</ul>
<form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/userSales/list" method="post"
           class="form-inline">
    ...
    <div class="control-group" style="height: 45px;margin-top: 20px;border-bottom: 1px solid #EEEEEE;">
        ...
        <label class="margin_line">业务名称：</label>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed table-hover">
    ...
    <tr>
        <th>序号</th>
        <th class="sort-column name">业务名称</th>
        <th class="sort-column login_name">登录帐号</th>
        <th>联系电话</th>
        <th>座机</th>
        <th>QQ</th>
        <th>部门</th>
    ...
    <c:forEach items="${page.list}" var="user">
        ...
        <tr id="tr_${user.id}">
            ...
            <td>
                    <c:choose>
                        <c:when test="${user.subFlag == 1}">业务</c:when>
                        <c:when test="${user.subFlag == 2}">跟单</c:when>
                        <c:when test="${user.subFlag == 3}">业务主管</c:when>
                    </c:choose>
            </td>
