...
<head>
    ...
    <script type="text/javascript">
        ...
        function search() {
            $("#pageNo").val(1);
            $("#searchForm").attr("action", "${ctx}/sys/customerComplaint/list");
            $("#searchForm").submit();
            return false;
        }
        ...
        function editUserKeFu(id) {
            ...
            var text = "添加客诉";
            var url = "${ctx}/sys/customerComplaint/form";
            var height = (h-200);
            var area = ['1000px', (h-200)+'px'];
            if (id != null){
                text = "修改";
                url = "${ctx}/sys/customerComplaint/form?userId=" + id;
            }
            top.layer.open({
                type: 2,
                id:"userKeFu",
        ...
        function removeUser(userId,userName){
            ...
            layer.confirm(
                '确认要删除客诉' +'<label style="color:#63B9E6">'+ userName +'</label>吗？',
                {
                    btn: ['确定','取消'], //按钮
                    title:'提示'
                }, function(index){
                    ...
                    $.ajax({
                        ...
                        success:function (data) {
                            ...
                            if (data.success) {
                                ...
                                layerError("删除客诉失败:" + data.message, "错误提示");
                            }
                            return false;
                        },
                        error: function (data) {
        ...
        function enableUser(userId,loginName,obj){
            ...
            layer.confirm(
                '确认要'+ title+'<label style="color:#63B9E6">'+loginName +'</label>登录账号吗？',
                {
                    btn: ['确定','取消'], //按钮
                    title:'提示',
...
<body>
    <ul class="nav nav-tabs">
        <li class="active"><a href="javascript:void(0);">客诉</a></li>
    </ul>
    <form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/customerComplaint/list" method="post"
               class="form-inline">
        ...
        <div class="control-group" style="height: 45px;margin-top: 20px;border-bottom: 1px solid #EEEEEE;">
            <label class="margin_line" style="margin-left: 5px">登录帐号：</label>
            <form:input path="loginName" htmlEscape="false" maxlength="50" class="input-medium"/>

            &nbsp;&nbsp;
            <label class="margin_line">客诉名称：</label>
    ...
    <shiro:hasPermission name="sys:customerComplaint:edit">
        <button style="margin-bottom: 15px;border-radius: 4px;border:1px solid;border-color:#C0C0C0;background-color: rgb(238,238,238);width: 110px;height: 32px" onclick="editUserKeFu()">
            <i class="icon-plus-sign"></i>&nbsp;添加客诉
