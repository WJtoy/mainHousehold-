...
<head>
    ...
    <script type="text/javascript" language="javascript">
        ...
        function viomiRetry(id) {
            var self = this;
            top.layer.open({
                type: 2,
                id: 'layer_viomi_retry',
                zIndex: 19891015,
                title: '云米异常重发',
                content: "${ctx}/b2b/rpt/processlog/viomiFailLogRetryForm?id="+id,
...
<ul class="nav nav-tabs">
    ...
    <li class="active"><a href="javascript:void(0);">云米</a></li>

</ul>
<form:form id="searchForm" modelAttribute="searchModel" action="${ctx}/b2b/rpt/processlog/viomiFailLog"
    ...
    <div>
        ...
        <input type=text class="input-small" id="orderNumber" name="orderNumber" value="${searchModel.orderNumber}" maxlength="30" />
        &nbsp;
        <label>传送时间：</label>
    ...
    <table id="contentTable"
        ...
        <c:forEach items="${page.list}" var="item">
            ...
            <tr>
                ...
                <td>  <a  class="input-block-level"  href="javascript:void(0);" onclick="viomiRetry('${item.id}')">${item.kklOrderNo}</a>
                        </td>
                <td>  <a  class="input-block-level"  href="javascript:void(0);" onclick="viomiRetry('${item.id}')">${item.orderNumber}</a>
                </td>
                <td>${item.contacts}</td>
                <td>${item.contactsPhone}</td>
                <td><a href="javascript:showResultJson(${index-1});" data-toggle="tooltip"
                       data-tooltip='${item.address}'>${fns:abbr(item.address,40)}</a></td>
                <td>${item.viomiStatus}</td>
