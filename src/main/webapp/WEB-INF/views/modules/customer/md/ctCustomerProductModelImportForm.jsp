    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $("#btnImportSubmit").click(function () {
                ...
                $("#searchForm").attr("action", "${ctx}/customer/md/customerProductModel/read");
            ...
            $("#btnExport").click(function () {
                ...
                layer.confirm('确认要导出异常明细吗?', {icon: 3, title: '系统确认'}, function (index, layero) {
                    ...
                    jQuery('<form id="exportForm" action="${ctx}/customer/md/customerProductModel/errorlist" method="post"></form>')
            ...
            $("#btnTransfer").click(function () {
                ...
                $.ajax({
                    cache: false,
                    type: "POST",
                    url: "${ctx}/customer/md/customerProductModel/importSave",
                    data: data,
                    beforeSend: function () {
                    ...
                    success: function (data) {
                        ...
                        if (data.success) {
                            ajaxSuccess = 1;
                            layerMsg('订单保存成功');
                            location.href = "${ctx}/customer/md/customerProductModel/importForm";
                        }
                        else {
            ...
            $("#btnClear").click(function () {
                ...
                $.ajax({
                    cache: false,
                    type: "POST",
                    url: "${ctx}/customer/md/customerProductModel/importClear",
                    data: data,
                    beforeSend: function () {
...
<ul class="nav nav-tabs">
    <li><a href="/customer/md/customerProductModel/getList">列表</a></li>
    <li class="active"><a href="javascript:void(0);">批量添加</a></li>
</ul>
<c:set var="currentuser" value="${fns:getUser()}"/>
<form:form id="searchForm" modelAttribute="customerProductModel" action="${ctx}/customer/md/customerProductModel/importForm" method="post" class="form-search"
           cssStyle="border-bottom: 1px solid #EEEEEE;padding-bottom: 15px;margin-top: 15px;">

    <c:choose>
        <c:when test="${currentuser.isCustomer()==true}">
            <input id="customerId"  name="customerId" value="${currentuser.company.id}" type="hidden" />
        </c:when>
    ...
    <input id="btnExport" class="btn btn-danger" type="button" value="导出异常"/>
    <a id="btnClear" href="javascript:" class="btn">清空</a>
    &nbsp&nbsp&nbsp&nbsp
    <c:if test="${currentuser.isCustomer() }"><a href="${ctxStatic}/doc/快可立批量添加产品型号模板.xls">下载产品型号模板</a></c:if>
    <c:if test="${currentuser.isCustomer()==false}"><a
            href="${ctxStatic}/doc/快可立批量添加产品型号模板.xls">下载产品模板</a></c:if>
