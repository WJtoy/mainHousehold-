    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function () {
            ...
            $("#btnSubmit").click(function () {
                ...
                $("#searchForm").attr("action", "${ctx}/customer/rpt/customerFrozenDailyRpt/customerFrozenDailyRptData");
                $("#searchForm").submit();
            });
            $("#btnExport").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "${ctx}/customer/rpt/customerFrozenDailyRpt/checkExportTask?"+ (new Date()).getTime(),
                        ...
                        if(data && data.success == true){
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/customer/rpt/customerFrozenDailyRpt/export?"+ (new Date()).getTime(),
                                        data:$(searchForm).serialize(),
                                        success: function (data) {
                                            if(ajaxLogout(data)){
                                                return false;
...
<ul class="nav nav-tabs">
    <li>
        <a href="${ctx}/customer/rpt/customerNewOrderDaily/getList">客户每日下单</a>
    </li>
    <li class="active"><a href="javascript:void(0);">每日冻结明细</a></li>

    <li><a href="${ctx}/customer/rpt/customerNewOrderMonth/getMonthList">客户每月下单</a></li>

    <li>
        <a href="${ctx}/customer/rpt/customerFrozenMonthRpt/customerFrozenMonthRptData">每月冻结明细</a>
    </li>
</ul>
<c:set var="currentuser" value="${fns:getUser() }"/>
<form:form id="searchForm" modelAttribute="rptSearchCondition"
           action="${ctx}/customer/rpt/customerFrozenDailyRpt/customerFrozenDailyRptData" method="post" class="breadcrumb form-search">
...
<%--                &lt;%&ndash;<input type="text" readonly="true" id="customer.name" name="customer.name" value="${currentuser.customerAccountProfile.customer.name}" />&ndash;%&gt;--%>
...
<%--                        &lt;%&ndash;<option value="" <c:out value="${(empty rptSearchCondition.customerId)?'selected=selected':''}"/>>所有</option>&ndash;%&gt;--%>
...
<%--        </c:choose>--%>
        &nbsp;&nbsp;
        <shiro:hasPermission name="rpt:customer:customerNewOrderDaily:view"><input id="btnSubmit" class="btn btn-primary" type="button" value="查询"/></shiro:hasPermission>
        &nbsp;&nbsp;
        <shiro:hasPermission name="rpt:customer:customerNewOrderDaily:export"><input id="btnExport" class="btn btn-primary" type="button" value="导出"/></shiro:hasPermission>
    </div>
...
<div id="divGrid" style="overflow: auto;">
    <table id="contentTable" class="table table-bordered table-condensed table-hover "
        ...
        <c:forEach items="${page.list}" var="item">
            ...
            <td rowspan="${item.orderItems.size()}">${rowIndex+(page.pageNo-1)*page.pageSize}</td>
            <td rowspan="${item.orderItems.size()}">${item.orderNo}</td>
            <td rowspan="${item.orderItems.size()}">${item.shopName}</td>
            <td rowspan="${item.orderItems.size()}">${item.parentBizOrderId}</td>
            <td rowspan="${item.orderItems.size()}">${item.orderCreateByName}</td>
              <c:forEach begin="0" end="${item.orderItems.size()-1}" var="i">
                <c:if test="${i ne 0}">
                    <tr>
                </c:if>
                <c:choose>
                ...
                <c:if test="${i eq 0}">
                    <c:choose>
                        <c:when test="${item.currencyType == '10' }">
                            <td rowspan="${item.orderItems.size()}">${item.blockAmount}</td>
                            <c:set var="totalBlockAmount" value = "${totalBlockAmount+item.blockAmount}"/>
                        </c:when>
                        <c:otherwise>
                            <td rowspan="${item.orderItems.size()}" >0</td>
                            <c:set var="totalBlockAmount" value = "${totalBlockAmount+0}"/>
                        </c:otherwise>
                    </c:choose>
                    <td rowspan="${item.orderItems.size()}">${item.remarks}</td>
                    <c:choose>
