...
<%@ attribute name="levelValue" type="java.lang.String" required="true" description="输入框值（Name）"%>
...
<%@ attribute name="showMaxLevel" type="java.lang.String" required="false" description="显示最大区域级别(如到4级为街道/乡/镇)"%>
...
<div class="input-append">
    ...
    <input id="${id}ShowMaxLevel" name="${id}ShowMaxLevel" type="hidden" value="${empty showMaxLevel?"3":showMaxLevel}" />
</div>
<link href="${ctxStatic}/area/area.css" rel="stylesheet" />
<script type="text/javascript">
    var area;
    $(function(){
        area = AreaFourLevelUtils;
        var showMaxLevel = ${empty showMaxLevel?"3":showMaxLevel};
        area.initComponent(document, showMaxLevel);
