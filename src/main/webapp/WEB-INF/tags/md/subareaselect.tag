...
<%@ attribute name="mustSelectCounty" type="java.lang.Boolean" required="true" description="只能选择到城市级"%>
...
<%@ attribute name="areaId" type="java.lang.String" required="false" description="区县Id"%>
...
<script src="${ctxStatic}/area/SubArea.js" type="text/javascript"></script>
<script type="text/javascript">
    var area;
    $(function(){
		area = SubAreaUtils;
        area.initComponent(document,${areaId});
        $(document).on("click",".btnCity",function(event){
            <c:choose>
            <c:when test="${empty callback}">
            area.open(this,event,${areaId});
            </c:when>
			<c:otherwise>
            area.opencallback(this,event,${callback},${areaId});
			</c:otherwise>
			</c:choose>
        });
    });
