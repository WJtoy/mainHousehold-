        ...
        $(document).ready(function(){
            $("#btnSubmit").click(function(){
				...
				if(val==3){
                    ...
                    entity.areaId=$obj.data("areaid");
                    entity.receiveName = $obj.data("receivename");
                    entity.receivePhone = $obj.data("receivephone");
                    entity.detailAddre = $obj.data("detailaddre");
                    entity.address = $obj.data("address");
            ...
            $("#inputForm").validate({
                submitHandler: function(form){
                    ...
                    $.ajax({
                        url:"${ctx}/sd/material/saveCustomerAddress",
                        type:"POST",
                        data:$(form).serialize(),
                        dataType:"json",
                        success: function(data){
                            ...
                            if (data.success) {
                                ...
                                entity.materialId = $("#materialId").val();
                                if(parentIndex && parentIndex != undefined && parentIndex != ''){
                                    var layero = $("#layui-layer" + parentIndex,top.document);
                                    var iframeWin = top[layero.find('iframe')[0]['name']];
                                    iframeWin.setReceiveData(entity);
						...
						<div class="control-group">
							<div class="controls" style="margin-left: 10px">
								<div style="margin-top: 10px">
									<c:choose>
										<c:when test="${materialReceives[0].areaId==null || materialReceives[0].areaId==0 || empty materialReceives[0].address}">
											...
											<label for="engineer_1">公司地址</label>
								...
								<div style="margin-top: 5px;margin-bottom: 10px" >
									...
									<%--<c:if test="${empty materialReceives[2].id || materialReceives[2].id<=0}">
										<span style="margin-left: 12px;">${returnAddress}</span>
									</c:if>--%>
								</div>
							</div>
						</div>
				...
				<div class="span6">
					<div class="control-group">
						<div class="controls" style="margin-left: -5px">
							<input name="address" id="address" class="input-block-level required fromInput" value="${materialReceives[2].address}" placeholder="详情地址，如xxx大厦1层101室">
						</div>
					</div>
				</div>
			</div>
