    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function () {
            ...
            $("#btnSubmit").click(function () {
                ...
                $("#searchForm").attr("action", "${ctx}/rpt/provider/gradedOrder/cityCompleteOrderRpt");
                $("#searchForm").submit();
            });
                        ...
                        if(data && data.success == true){
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/rpt/provider/gradedOrder/cityCompletedExport?"+ (new Date()).getTime(),
                                        data:$(searchForm).serialize(),
                                        success: function (data) {
                                            if(ajaxLogout(data)){
                                                return false;
...
<ul class="nav nav-tabs">
    <li>
        <a href="${ctx}/rpt/provider/gradedOrder/provinceCompleteOrderRpt">省每日完成统计</a>
    </li>
    <li class="active"><a href="javascript:void(0);">市每日完成统计</a></li>
    <li>
...
<form:form id="searchForm" modelAttribute="rptSearchCondition"
           action="${ctx}/rpt/provider/gradedOrder/cityCompleteOrderRpt" method="post" class="breadcrumb form-search">
    <div style="width: 90%;">
                ...
                <c:choose>
                    <c:when test="${rowIndex < list.size()}">
                        <tr>
                            ...
                            <td>
                                <a href="${ctx}/rpt/provider/gradedOrder/areaCompleteOrderRpt?cityId=${cItem.cityId}&selectedYear=${rptSearchCondition.selectedYear}&selectedMonth=${rptSearchCondition.selectedMonth}&areaName=${cItem.cityName}">
                                        ${cItem.cityName}</a>
                            </td>
                            <c:forEach var="i" begin="1" end="${rptSearchCondition.days}" step="1">
