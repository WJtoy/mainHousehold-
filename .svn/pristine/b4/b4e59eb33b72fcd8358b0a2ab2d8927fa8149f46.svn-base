    ...
    private OrderSearchModel setSearchModel(User user,OrderSearchModel searchModel,Model model ,
        ...
        if(user.isKefu()) {
            if (user.getSubFlag() == KefuTypeEnum.VIPKefu.getCode()) {
                ...
                //突击客服，只看自己负责的单
                searchModel.setCustomerType(null); //可查看vip的工单
                searchModel.setRushType(1);//查看突击区域订单
            }else if(user.getSubFlag() == KefuTypeEnum.AutomaticKefu.getCode()){
                //自动客服
                searchModel.setCustomerType(null);//可查询vip客户订单
                searchModel.setRushType(0);//不能查看突击区域订单
            }else if(user.getSubFlag() == KefuTypeEnum.COMMON_KEFU.getCode()){ //大可客看所有客服
                searchModel.setCustomerType(null);//可查询vip客户订单
                searchModel.setRushType(0);//不能查看突击区域订单
            }else {
       ...
       /* if (isServiceSupervisor) {
        ...
        if(user.isKefu()) {
            if (user.getSubFlag() == KefuTypeEnum.VIPKefu.getCode()) {
                ...
                //普通客服
                searchEntity.setCustomerType(null);//不能查询vip客户订单
                searchEntity.setRushType(0);//不能查看突击区域订单
    ...
    public String tmallServiceMonitorList(TmallServiceMonitorSearchVM tmallServiceMonitor, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if(user.isKefu()) {
            if (user.getSubFlag() == KefuTypeEnum.VIPKefu.getCode()) {
                ...
                tmallServiceMonitor.setCustomerType(null);//查询所有客户订单
                tmallServiceMonitor.setRushType(0);//不能查看突击区域订单
            }else if(user.getSubFlag() == KefuTypeEnum.COMMON_KEFU.getCode()){
                tmallServiceMonitor.setCustomerType(null);//查询所有客户订单
                tmallServiceMonitor.setRushType(0);//不能查看突击区域订单
        ...
        if (orderId <= 0) {
            ...
            if (order == null || order.getOrderCondition() == null) {
                ...
                if (order.getOrderCondition().getServicePoint() != null && order.getOrderCondition().getServicePoint().getId() != null
                    ...
                    MDDepositLevel depositLevel = msDepositLevelService.getById(order.getOrderCondition().getServicePoint().getMdDepositLevel().getId());
                    ServicePointFinance servicePointFinance = servicePointService.getDepositFromMasterById(order.getOrderCondition().getServicePoint().getId());
                    if(servicePointFinance!=null){
                        if(order.getOrderCondition().getServicePoint().getFinance()!=null){
                            order.getOrderCondition().getServicePoint().getFinance().setDeposit(servicePointFinance.getDeposit());
                            order.getOrderCondition().getServicePoint().getFinance().setDepositRecharge(servicePointFinance.getDepositRecharge());
                        }else{
                            order.getOrderCondition().getServicePoint().setFinance(servicePointFinance);
                        }
                    }
                    if(depositLevel!=null){
                        order.getOrderCondition().getServicePoint().setMdDepositLevel(depositLevel);
                    }
                    if (engineer != null) {
...
//                        int appLoged = servicePoint !=null && servicePoint.getPrimary()!=null && servicePoint.getPrimary().getAppLoged() != null? servicePoint.getPrimary().getAppLoged() : 0;
     ...
     * @return
     */
    @RequiresPermissions(value ="sd:order:insurance")
    ...
    public AjaxJsonEntity getOrderInsurance(String strOrderId,String quarter){
        ...
        try {
            ...
            List<OrderInsurance> list = kefuOrderListService.getOrderInsurance(orderId,quarter);
