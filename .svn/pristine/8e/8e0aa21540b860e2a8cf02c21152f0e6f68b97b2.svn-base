...
<head>
	...
	<script type="text/javascript">
        ...
        function showServicePointAdd(areaId,productCategoryId,address,subAreaId,canRush) {
		    var layerIndex = $("#layerIndex").val();
            var parentLayerIndex = $("#parentLayerIndex").val();
            window.location.href="${ctx}/md/servicepoint/addServicePointForPlanForm?dialogType=layer&area.id=" +areaId + '&productCategoryId='+ productCategoryId +"&address=" + encodeURI(address) +"&layerIndex=" + layerIndex + "&parentLayerIndex=" +parentLayerIndex +"&subArea.id=" + subAreaId + "&canRush="+canRush;
        }
        ...
        function enableServicePoint(servicePointId,name,item) {
            top.layer.open({
                ...
                yes:function (index,layero) {
                    ...
                    $.ajax({
                        url:"${ctx}/md/servicepoint/updateStatusForPlan",
                        type:"POST",
...
<body>
	<ul id="navtabs" class="nav nav-tabs">
		...
		<c:forEach items="${fns:getDictListFromMS('degreeType')}" var="dict">
            <li><a href="${ctx}/md/servicepoint/selectForPlan?dialogType=layer&area.id=${servicePointModel.area.id}&subArea.id=${servicePointModel.subArea.id}&productCategoryId=${servicePointModel.productCategoryId}&address=${fns:urlEncode(servicePointModel.address)}&degree=${dict.value}&layerIndex=${servicePointModel.layerIndex}&parentLayerIndex=${servicePointModel.parentLayerIndex}&canRush=${servicePointModel.canRush}" style="width: 70px;text-align: center" title="${dict.label}">${dict.label}</a></li>
		</c:forEach>
		<li  class="active"><a href="javascript:void(0)">网点找回</a></li>
		<li><a href="javascript:showServicePointAdd('${servicePoint.area.id}','${servicePoint.productCategoryId}','${servicePoint.address}','${servicePoint.subArea.id}','${servicePoint.canRush}')">添加网点</a></li>
	</ul>
	<c:set var="currentuser" value="${fns:getUser()}" />
	<div style="margin-left:3px;margin-right:3px;">
		<form:form id="searchForm" modelAttribute="servicePoint"  action="${ctx}/md/servicepoint/unableSelectForPlan" method="post" class="breadcrumb form-inline">
