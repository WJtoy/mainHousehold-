...
<ul id="navtabs" class="nav nav-tabs">
    <shiro:hasPermission name="sd:abnormal:view">
        ...
        <li><a href="${ctx}/servicePoint/sd/abnormalForm/smsAbnormalList" title="短信异常">短信异常</a></li>
        <li class="active"><a href="javascript:void(0);" title="旧app异常">旧app异常</a></li>
        <li><a href="${ctx}/servicePoint/sd/abnormalForm/processedList" title="异常单已处理">已处理</a></li>
    </shiro:hasPermission>
</ul>
<c:set var="currentuser" value="${fns:getUser() }"/>
<form:form id="searchForm" modelAttribute="abnormalForm" action="${ctx}/servicePoint/sd/abnormalForm/oldAppAbnormalList" method="post" class="form-inline">
    ...
    <div class="control-group">
        ...
        <form:select path="subType" cssClass="input-small" cssStyle="width:180px;">
            ...
            <form:option value="1" label="APP操作异常"/>
            <form:option value="2" label="由于用户原因，申请取消"/>
            <form:option value="3" label="异常反馈"/>
            <form:option value="4" label="异常反馈"/>
            <form:option value="5" label="师傅申请改派或取消"/>
            <form:option value="6" label="无法联系用户"/>
        </form:select>&nbsp;
        <label>所属网点：</label>
    ...
    <div class="control-group">
        <label class="label-search">反馈日期：</label>
        ...
        <c:forEach items="${page.list}" var="model">
            ...
            <tr>
               ...
               <%-- <td>${model.feedBackTimeliness}小时</td>--%>
                ...
                <shiro:hasPermission name="sd:abnormal:close">
                    <td>
                        <input id="abnormalClose" class="btn btn-warning" type="button" onclick="closeAbnormal('${model.id}','${model.orderId}','${model.quarter}','${model.createDt}','${model.formType}')" value="解除异常"/>
                    </td>
                </shiro:hasPermission>
            </tr>
        </c:forEach>
...
<script type="text/javascript">
    ...
    var locked = false;
    function closeAbnormal(id,orderId,quarter,createDt,formType) {
        var index = layer.confirm('<span style="position: relative;font-size: 20px;top: 50px;left: 60px">确定该工单异常已经完成了吗？</span>', {
            area:["450px","240px"],
            title:"标记异常单-操作",
            btn: ['确定,解除异常','取消'], //按钮
           ...
           if(!locked){
               ...
               $.ajax({
                   type: "POST",
                   url: "${ctx}/sd/abnormalForm/closeAbnormalForKefu",
                   data: {id:id,orderId:orderId,quarter:quarter,createDt:createDt,formType:formType},
