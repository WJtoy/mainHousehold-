...
<ul id="navtabs" class="nav nav-tabs">
    <shiro:hasPermission name="sd:reminder:view">
        <li><a href="${ctx}/sd/kefu/reminder/list/process" title="待客服回复列表">待回复</a></li>
        <li><a href="${ctx}/sd/kefu/reminder/list/haveRepliedList" title="客服已回复列表">已回复</a></li>
        <li><a href="${ctx}/sd/kefu/reminder/list/moreThan24HoursUnfinishedList" title="超24小时未完成">超24小时未完成</a></li>
        <li><a href="${ctx}/sd/kefu/reminder/list/moreThan48HoursUnfinishedList" title="超48小时未完成">超48小时未完成</a></li>
        <li class="active"><a href="javascript:void(0);" title="所有催单列表">所有</a></li>
    </shiro:hasPermission>
</ul>
<c:set var="currentuser" value="${fns:getUser() }"/>
<form:form id="searchForm" modelAttribute="reminder" action="${ctx}/sd/kefu/reminder/list/all" method="post" class="breadcrumb form-search">
    <input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
    <input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
    <div class="alert alert-block" style="padding-top:6px;padding-bottom:6px;">
        注:当用催单号,订单号和电话进行搜索查询时，不受用户、客户、创建日期、区域及品类等其他条件的限制。
    </div>
    <div>
        <label>区&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;域：</label>
        <%--
        ...
        <sys:treeselect id="customer" name="customerId" value="${reminder.customerId}" labelName="customerName"
                        ...
                        title="客户" url="/md/customer/treeData?kefu=${currentuser.id}" clearIdValue="0"
    ...
    <div style="margin-top: 8px">
        ...
        <form:select path="status" class="input-small" style="width:135px;">
            ...
            <form:option value="5" label="未完成" />
            <form:option value="4" label="已完成" />
        </form:select>
        &nbsp;&nbsp;
...
<div id="divGrid">
    <table id="contentTable" class="table table-striped table-bordered table-condensed table-hover" style="table-layout:fixed;">
        ...
        <tr>
            ...
            <th width="100">处理时效
                <a href="javascript:void(0);" style="text-decoration:none" data-toggle="tooltip" data-tooltip="已回复到已处理的时效">
        ...
        <c:forEach items="${page.list}" var="model">
            ...
            <tr>
                ...
                <td>
                    <c:choose>
                        <c:when test="${model.status==1}">
                            ...
                            <c:choose>
                                <c:when test="${model.processTimeLiness<0}">
                                    <c:set var="cutoffClass" value="timeout"/>
                                </c:when>
                                <c:when test="${model.processTimeLiness<0.33}">
                                    <c:set var="cutoffClass" value="intime"/>
                                </c:when>
                            </c:choose>
                            <p class="reminder_${cutoffClass}">${model.cutOffLabel}</p>
                        </c:when>
                        <c:otherwise>
                            <c:set var="cutoffClass" value="normal"/>
                            <c:choose>
                ...
                <td>
                    <c:if test="${model.status==3 || model.status==4 && model.handleTimeLiness>0}">
                        <c:set var="cutoffClass" value="normal"/>
                        <c:choose>
                            <c:when test="${model.handleTimeLiness>1}">
                                <c:set var="cutoffClass" value="timeout"/>
                            </c:when>
                        </c:choose>
                        <p class="reminder_${cutoffClass}">用时:${model.handleTimeLinessLabel}</p>
                    </c:if>
                </td>
                <td>
                    <c:choose>
                        <c:when test="${model.status==4}">
                            <c:set var="cutoffClass" value="normal"/>
                            <c:choose>
                                <c:when test="${model.orderTimeLiness>24}">
                                    <c:set var="cutoffClass" value="timeout"/>
                                </c:when>
                            </c:choose>
                            <p class="reminder_${cutoffClass}">用时:${model.completeTimelinessLabel}</p>
                        </c:when>
                        <c:otherwise>
                            <c:set var="cutoffClass" value="normal"/>
                            <c:choose>
