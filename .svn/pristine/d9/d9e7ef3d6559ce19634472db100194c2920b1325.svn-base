 ...
 * RPT微服务调用
 */
@FeignClient(name = "provider-rpt", fallbackFactory = MSCustomerChargeRptFeignFallbackFactory.class)
public interface MSCustomerChargeRptFeign {

    /**
     * 获取客户的工单数量与消费金额信息
     */
    @PostMapping("/customerCharge/getCustomerChargeSummaryMonthly")
     ...
     * 分页获取退单或取消工单明细
     */
    @PostMapping("/customerCharge/getCustomerWriteOffList")
