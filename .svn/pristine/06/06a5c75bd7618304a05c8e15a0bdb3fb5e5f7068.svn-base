...
<head>
    ...
    <style type="text/css">
        .table thead th, .table tbody td {  text-align: center;  vertical-align: middle; /*background: transparent;*/  }
    ...
    <script type="text/javascript">
        ...
        $(document).on("click", "#btnClearSearch", function () {
            ...
            $("#autoComplete").attr('checked', false);
            $("#remarks").val("");
            $("#dataSource").val("0");
...
<ul id="navtabs" class="nav nav-tabs">
    ...
    <li class="active"><a href="javascript:void(0);" title="已完成的工单">已完成</a></li>
...
<form:form id="searchForm" modelAttribute="order" method="post" class="form-inline">
    ...
    <div class="control-group">
        ...
        <label>用户电话：</label>
        <input type=text class="input-small" id="phone1" name="phone1" value="${order.phone1}" placeholder="用户电话 或 实际联络电话" maxlength="20" />
        <label>师傅：</label>
    ...
    <div style="margin-top:0px; display:none;" id="div_search_customer">
        ...
        <div style="margin-top:5px;">
           ...
           <%-- <label>地址：</label>
            ...
            <label>产品：</label>
            <form:select path="productId" cssClass="input-small" cssStyle="width:212px;">
            ...
            <label>自动完工：</label>
            <form:select path="orderAutoComplete" class="input-small" style="width:125px;">
                <form:option value="0" label=""/>
                <form:option value="5" label="所有自动完工"/>
                <form:option value="2" label="短信完工"/>
                <form:option value="3" label="语音完工"/>
                <form:option value="4" label="App完工"/>
            </form:select>
        </div>
        <div style="margin-top:5px;">
            ...
            <input id="replyFlagCustomer" name="replyFlagCustomer" <c:if test="${order.replyFlagCustomer==2}">checked="checked"</c:if> type="checkbox" value="2" class="input-small"/> &nbsp;&nbsp;
            <label>反馈内容：</label>&nbsp;
            <input type=text class="input-small" style="width: 150px;" id="remarks" name="remarks" value="${order.remarks }" maxlength="30" />
        </div>&nbsp;
    </div>
    ...
    <c:forEach items="${page.list}" var="order">
        ...
        <tr>
            ...
            <td rowspan="${rowspan}">
                    ...
                    <c:if test="${!empty order.orderStatus && order.orderStatus.chargeDate != null }">
                        <br><fmt:formatDate value="${order.orderStatus.chargeDate}" pattern="yyyy-MM-dd"/>
                    </c:if>
            ...
            <td rowspan="${rowspan}">
                ...
                <c:if test="${!empty order.orderCondition}">
                    ...
                    <c:if test="${!empty finance}">
                        <c:set var="tips" value="${tips.concat(order.orderCondition.servicePoint==null?'':order.orderCondition.servicePoint.servicePointNo).concat(' 账户余额:').concat(finance.balance).concat('元').concat('<br>')}"/>
                        <c:if test="${!empty finance.lastPayDate}">
                            <c:set var="tips" value="${tips.concat('上次付款时间:').concat(fns:formatDate(finance.lastPayDate,'yyyy.MM.dd')).concat('，付款金额').concat(finance.lastPayAmount).concat('元')}"/>
                        </c:if>
                    </c:if>
                </c:if>
                <a href="javascript:void(0);" data-toggle="tooltip" data-tooltip="${tips}">${order.orderCondition.engineer==null?'':order.orderCondition.engineer.name}</a>
