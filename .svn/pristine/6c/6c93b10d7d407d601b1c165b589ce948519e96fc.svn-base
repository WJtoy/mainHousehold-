...
public class CtCustomerProductModelController extends BaseController {
    @Autowired
    private CtCustomerProductModelService customerProductModelService;
    ...
    public String getList(CustomerProductModel customerProductModel, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if (customerProductModel.getCustomerId() == null || customerProductModel.getCustomerId() <= 0) {
            ...
            return "modules/customer/md/ctCustomerProductModelList";
        }
        if(erroFlag){
        ...
        page = customerProductModelService.getList(new Page<CustomerProductModel>(request, response), customerProductModel);
        ...
        for(CustomerProductModel entity:list){
            Product product = msProductService.getProductByIdFromCache(entity.getProductId());
            if(product!=null){
    ...
    public String form(CustomerProductModel customerProductModel, Model model) {
        ...
        if(user.isCustomer()){
            if (user.getCustomerAccountProfile() != null && user.getCustomerAccountProfile().getCustomer() != null) {
                ...
                return "modules/customer/md/ctCustomerProductModelForm";
            }
        }
        if (customerProductModel.getId() != null && customerProductModel.getId() > 0) {
            MSResponse<CustomerProductModel> msResponse = customerProductModelService.getById(customerProductModel.getId());
            ...
            if(entity !=null){
                ...
                if(entity.getBrandId()!=null && entity.getBrandId()>0){
                    entity.setBrandName(ctCustomerBrandService.getById(entity.getBrandId()).getBrandName());
                }
            }
        }
    ...
    public String save(CustomerProductModel customerProductModel, HttpServletRequest request, Model model, RedirectAttributes redirectAttributes) {
        ...
        if (user.getId() != null) {
            ...
            MSErrorCode mSResponse = customerProductModelService.save(customerProductModel);
        ...
        if(user.isSaleman()){
            return "redirect:" + adminPath + "/customer/md/customerProductModel/getList?repage&customerId=" + customerProductModel.getCustomerId();
        }
        return "redirect:" + adminPath + "/customer/md/customerProductModel/getList?repage";
    }*/
    ...
    public String delete(CustomerProductModel entity, RedirectAttributes redirectAttributes) {
        MSResponse<Integer> msResponse = customerProductModelService.delete(entity);
        if (msResponse.getCode() == 0) {
