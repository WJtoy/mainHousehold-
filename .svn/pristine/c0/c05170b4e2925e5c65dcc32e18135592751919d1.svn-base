    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function () {
            ...
            $("#btnSubmit").click(function () {
                $("#pageNo").val(1);
                $("#searchForm").attr("action", "${ctx}/servicePoint/rpt/spNewDetailedReconciliation/getNewNetworkReconciliation");
                $("#searchForm").submit();
            });
            ...
            $("#btnExport").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "${ctx}/servicePoint/rpt/spNewDetailedReconciliation/checkExportTaskNew?" + (new Date()).getTime(),
                        ...
                        if (data && data.success == true) {
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/servicePoint/rpt/spNewDetailedReconciliation/exportNew?" + (new Date()).getTime(),
                                        data: $(searchForm).serialize(),
                                        success: function (data) {
                                            if (ajaxLogout(data)) {
                                                return false;
...
<form:form id="searchForm" modelAttribute="rptSearchCondition" action="${ctx}/servicePoint/rpt/spNewDetailedReconciliation/getNewNetworkReconciliation" method="post" class="breadcrumb form-search">
    ...
    <div style="width: 90%">
        ...
        &nbsp;&nbsp;
        <shiro:hasPermission name="rpt:servicePoint:getNewNetworkReconciliation:view"><input id="btnSubmit" class="btn btn-primary" type="button" value="查询"/></shiro:hasPermission>
        &nbsp;&nbsp;
        <shiro:hasPermission name="rpt:servicePoint:getNewNetworkReconciliation:export"><input id="btnExport" class="btn btn-primary" type="button" value="导出"/></shiro:hasPermission>
    </div>
        ...
        <div style="position: relative" class="parent">
            ...
            <div style="text-align:right;position: absolute;width: 250px;height: 100px;right:1px;top:31px;" class="target">
                <div style="text-align:left;background-color: #1B1E24;border-radius: 5px;opacity: 0.8;padding: 10px;font-size:14px;color:white;min-width:100px;max-width:400px;">
                    ...
                    统计方式：审单时间<br/>
                    栏位说明：<br/>
                    【本月未付金额】上月未付金额+本月完工单金额-本月扣点-本月平台费-本月质保金额-本月已付金额<br/>
                    【合计金额】服务费+配件费+快递费+其他费+互助基金+时效奖励+厂商时效+加急费+好评费
                </div>
            </div>
...
<table id="treetable" class="fancyTable datatable table table-bordered table-condensed table-hover" style="table-layout:fixed"
    ...
    <c:if test="${page.list[0] != null}">
        <tr>
            ...
            <td><fmt:formatNumber pattern="0.00" value="${page.list[0].engineerDeposit}"/></td>
