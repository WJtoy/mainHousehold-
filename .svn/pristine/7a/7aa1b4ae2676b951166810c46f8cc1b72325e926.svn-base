    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function () {
            ...
            $("#btnSubmit").click(function () {
                $("#pageNo").val(1);
                $("#searchForm").attr("action", "${ctx}/rpt/provider/detailedReconciliationNew/getNetworkReconciliationNew");
                $("#searchForm").submit();
            });
            ...
            $("#btnExport").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "${ctx}/rpt/provider/detailedReconciliationNew/checkExportTask?" + (new Date()).getTime(),
                        ...
                        if (data && data.success == true) {
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/rpt/provider/detailedReconciliationNew/export?" + (new Date()).getTime(),
                                        data: $(searchForm).serialize(),
                                        success: function (data) {
                                            if (ajaxLogout(data)) {
                                                return false;
...
<ul class="nav nav-tabs">
    <li class="active"><a href="javascript:void(0);">网点明细对帐表</a></li>
</ul>
<form:form id="searchForm" modelAttribute="rptSearchCondition" action="${ctx}/rpt/provider/detailedReconciliationNew/getNetworkReconciliationNew" method="post" class="breadcrumb form-search" onsubmit="return validate(this)">
    ...
    <div>
        ...
        &nbsp;&nbsp;
        <shiro:hasPermission name="rpt:getNetworkReconciliationNew:view"><input id="btnSubmit" class="btn btn-primary" type="button" value="查询"/></shiro:hasPermission>
        &nbsp;&nbsp;
        <shiro:hasPermission name="rpt:getNetworkReconciliationNew:export"><input id="btnExport" class="btn btn-primary" type="button" value="导出"/></shiro:hasPermission>
    </div>
</form:form>
...
<table id="treetable" class="fancyTable datatable table table-bordered table-condensed table-hover" style="table-layout:fixed"
    ...
    <c:if test="${page.list[0] != null}">
        <tr>
            ...
            <td><fmt:formatNumber pattern="0.00" value="${page.list[0].engineerTaxFee}"/></td>
            <td><fmt:formatNumber pattern="0.00" value="${page.list[0].engineerInfoFee}"/></td>
...
<%--        <th width="80">远程费</th>--%>
            ...
            <c:forEach begin="0" end="${item.maxRow-1}" var="i">
                ...
                <c:choose>
                    <c:when test="${i lt item.details.size()}">
                        ...
                        <td>${item.details.get(i).engineerOtherCharge+item.details.get(i).engineerTravelCharge}</td>
                        ...
                        <c:set var="totalOtherCharge" value="${totalOtherCharge +item.details.get(i).engineerOtherCharge+item.details.get(i).engineerTravelCharge}"/>
                    </c:when>
                </c:choose>
                ...
                <c:if test="${i eq 0}">
                    ...
                    <td rowspan="${item.maxRow}">${item.engineerTotalCharge+item.engineerInsuranceCharge+item.engineerUrgentCharge+item.engineerTimelinessCharge+item.engineerCustomerTimelinessCharge+item.engineerPraiseFee}</td>
                    ...
                    <c:set var="totalCharge"
                           value="${totalCharge+item.engineerTotalCharge +item.engineerInsuranceCharge+item.engineerUrgentCharge+item.engineerTimelinessCharge+item.engineerCustomerTimelinessCharge+item.engineerPraiseFee}"/>
                </c:if>
        ...
        <c:if test="${page.list[0].list.size()>0}">
            <tr>
                ...
                <td style="color: red;">${totalServiceCharge}</td>
                <td style="color: red;">${totalMaterialCharge}</td>
                <td style="color: red;">${totalExpressCharge}</td>
                <td style="color: red;">${totalOtherCharge}</td>
                <td style="color: red;">${totalInsurance}</td>
                <td style="color: red;">${totalTimeLinessCharge}</td>
                <td style="color: red;">${totalCustomerTimeLinessCharge}</td>
                <td style="color: red;">${totalEngineerUrgentCharge}</td>
                <td style="color: red;">${engineerPraiseFee}</td>
                <td style="color: red;">${totalCharge}</td>
                <td></td>
                <td></td>
                <td colspan="2"></td>
            </tr>
