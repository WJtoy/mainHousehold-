...
<ul id="navtabs" class="nav nav-tabs">
    ...
    <li class="active"><a href="javascript:void(0);" title="未完成的工单">未完成<span id="spn_order_count" class="badge badge-info">${page !=null ?page.count:''}</span></a></li>
    <li><a href="${ctx}/sd/order/kefuOrderListNew/completedList" title="已完成的工单">已完成</a></li>
    <li><a href="${ctx}/sd/order/kefuOrderListNew/alllist" title="所有工单">所有</a></li>
</ul>
...
<form:form id="searchForm" modelAttribute="order" method="post" class="form-inline">
    ...
    <div class="control-group">
        ...
        <label>用户电话：</label>
        <input type=text class="input-small digits" id="phone1" name="phone1" value="${order.phone1}"
    ...
    <div style="margin-top:0px; display:none;" id="div_search_customer">
        <div>
            ...
            <label>区域：</label>
            <%--
            <sys:treeSelectAnyArea id="area" name="area.id" value="${order.area.id}" levelValue="${order.areaLevel}" parentValue="${order.area.parent.id}"
        ...
        <div style="margin-top:5px;">
            ...
            <label>状态：</label>
            <c:set var="statusList" value="${fns:getDictExceptListFromMS('order_status','10,80,85,90,100')}"/><%--切换为微服务--%>
            ...
            <form:select path="urgentLevel.id" class="input-small" style="width:125px;">
                ...
                <c:if test="${!empty order.urgentLevels}">
                    <form:option value="1" label="所有加急"/>
        ...
        <div style="margin-top:5px;">
            ...
            &nbsp;&nbsp;
            <label for="replyFlagCustomer" title="未回复反馈">反馈：</label>
            ...
            <input id="appAbnormalyFlag" name="appAbnormalyFlag"
                   ...
                   class="input-small"/> &nbsp;&nbsp;
            <label for="isBackApprove" title="师傅接单2小时内没有预约也没有联系用户">回收：</label>
            <input id="isBackApprove" name="isBackApprove"
                   <c:if test="${isBackApprove==true}">checked="checked"</c:if> type="checkbox" value="1"
                   class="input-small"/> &nbsp;&nbsp;
            <label>反馈内容：</label>&nbsp;
            <input type=text class="input-small" style="width: 150px;" id="remarks" name="remarks"
                   value="${order.remarks}" maxlength="30"/>
        </div>&nbsp;
    </div>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed table-hover">
    ...
    <c:forEach items="${page.list}" var="order">
        ...
        <tr>
            ...
            <td rowspan="${rowspan}">
                <c:set var="canFollowUp" value="${fns:canFollowUp(order.orderCondition.status.value,order.orderCondition.subStatus,order.orderCondition.pendingTypeDate)}" />
                <c:if test="${canFollowUp == 0}">
                <c:set var="canFollowUp" value="${order.isFollowUpFail()}" />
                </c:if>
                <c:choose>
                    <c:when test="${order.isNotAccept(30)}">
                        <c:choose>
                        <c:when test="${canFollowUp == 0}">
                        ...
                        <c:otherwise>
                            <a href="javascript:void(0);" style="color: red;" onclick="Order.showFollowUpFailOrderDetail('${order.id}','${order.quarter}',${order.orderCondition.orderServiceType});">
                        </c:otherwise>
                        </c:choose>
                    ...
                    <c:when test="${order.orderCondition.appAbnormalyFlag == 1}">
                        <%--<a href="javascript:void(0);" style="color: red;" onclick="Order.showKefuOrderDetail('${order.id}','${order.quarter}');">--%>
                        <c:choose>
                        <c:when test="${canFollowUp == 0}">
                        <a href="javascript:void(0);" style="color: red;" onclick="Order.showKefuOrderDetail('${order.id}','${order.quarter}',${order.orderCondition.orderServiceType});">
                        </c:when>
                    ...
                    <c:when test="${not empty order.repeateNo}">
                        <%--<a href="javascript:void(0);" onclick="Order.showKefuOrderDetail('${order.id}','${order.quarter}');">--%>
                    <c:choose>
                    <c:when test="${canFollowUp == 0}">
                        <a href="javascript:void(0);" style="color: red;" onclick="Order.showKefuOrderDetail('${order.id}','${order.quarter}',${order.orderCondition.orderServiceType});">
                    </c:when>
            ...
            <td>${item.qty}</td>
            <td rowspan="${rowspan}">
                <c:set var="engineerInvoceDate" value=""/>
                <c:if test="${!empty order.orderStatus.engineerInvoiceDate}">
                    <c:set var="engineerInvoceDate"
                           value="${engineerInvoceDate.concat('付款日期:').concat(fns:formatDate(order.orderStatus.engineerInvoiceDate,'yyyy-MM-dd'))}"/>
                </c:if>
                <a href="javascript:void(0);" data-toggle="tooltip"
                   data-tooltip="${order.orderCondition.servicePoint==null?'':order.orderCondition.servicePoint.servicePointNo} ${engineerInvoceDate}">${order.orderCondition.engineer==null?'':order.orderCondition.engineer.name}</a>
            </td>
                ...
                <c:choose>
                    <c:when test="${order.orderCondition.feedbackFlag eq 1}">
                        ...
                        <a href="javascript:void(0);"
                           ...
                           title="${order.orderCondition.feedbackTitle}" style="display: block;">
