...
<head>
	...
	<script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#inputForm").validate({
                rules: {
                    loginName: {remote: "${ctx}/md/customerAccount/checkLoginName?oldLoginName=" + encodeURIComponent('${!empty user.id?user.loginName:''}') +"&expectId=${empty user.id?'0':'' + user.id}"}
					,mobile: {remote:{
							type: "post",
							url: "${ctx}/md/customerAccount/checkMasterPhone",
							data: {
								id: function() {
									return $("#id").val();
								},
                ...
                submitHandler: function(form){
					...
					$.ajax({
						url: "${ctx}/md/customerAccount/save",
						type: "POST",
						data: $(form).serialize(),
						dataType: "json",
		...
		function editCustomerShop() {
			...
			var text = "关联客户店铺";
			var url = "${ctx}/md/customerAccount/customerShop?userId="+ userId +"&customerId=" + customerId + "&name=" + name + "&parentIndex=" + (orderdetail_index || '');
...
<form:form id="inputForm" modelAttribute="user" action="${ctx}/md/customerAccount/save" method="post" class="form-horizontal" style="margin: 0px;">
	<form:hidden path="id"/>
