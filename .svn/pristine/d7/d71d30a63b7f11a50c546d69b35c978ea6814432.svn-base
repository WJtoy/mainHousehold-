...
<form:form id="searchForm" modelAttribute="abnormalForm" action="${ctx}/servicePoint/sd/abnormalForm/praiseAbnormalList" method="post" class="breadcrumb form-search">
    <input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
    <input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
    <div class="alert alert-block" style="padding-top:6px;padding-bottom:6px;">
        注:当用工单号搜索查询时，不受当前时间等其他条件的限制。
    </div>
    <div class="control-group">
        <label>工单单号：</label>
        <input type=text class="input-small" id="orderNo" name="orderNo" value="${abnormalForm.orderNo}" maxlength="14" />&nbsp;
        <label>客    户：</label>
        <sys:treeselect id="customer" name="customerId" value="${abnormalForm.customerId}" labelName="customerName"
                        labelValue="${abnormalForm.customerName}" title="客户"
    ...
    <table id="contentTable" class="table table-bordered table-condensed table-hover">
        ...
        <c:forEach items="${page.list}" var="model">
            ...
            <tr>
                ...
                <td>${model.servicePoint.contactInfo1}</td>
...
<script type="text/javascript">
    function showPraiseInfo(id,quarter,servicePointId) {
        top.layer.open({
            type: 2,
            id: 'layer_praiseAbnormal',
            zIndex: 19891015,
            title: '好评驳回',
            content: "${ctx}/sd/abnormalForm/praiseInfoForKefu?orderId="+id + "&quarter=" + quarter +"&servicePointId=" + servicePointId,
            area: ['900px','720px'],
            shade: 0.3,
            shadeClose: true,
            maxmin: true
