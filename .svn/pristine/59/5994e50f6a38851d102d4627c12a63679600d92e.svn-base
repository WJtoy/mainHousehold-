    ...
    <script type="text/javascript">
        ...
        function enableUser(id,name,contactInfo,obj){
            ...
            if(delFlag == 0){
                title = "停用";
                delFlag = 1;
                url = "${ctx}/md/engineerForSP/delete?id="+id;
            }else {
                delFlag = 0;
                title = "启用";
                url = "${ctx}/md/engineerForSP/enable?id="+id +"&contactInfo=" + contactInfo;
            }
...
<form:form id="searchForm" modelAttribute="engineer" action="${ctx}/md/engineerForSP" method="POST" class="breadcrumb form-search">
    ...
    <ul class="ul-form">
        ...
        <li>
            <label>联系电话：</label>
            <form:input path="contactInfo" htmlEscape="false" maxlength="20" class="input-small" style="width: 186px"/>
        </li>
        <%--<li>--%>
            <%--<label>账号类型：</label>--%>
