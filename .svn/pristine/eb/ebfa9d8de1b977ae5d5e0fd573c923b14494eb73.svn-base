...
<html>
<head>
	<title>我的订单配件单(客户待发件 for 业务)</title>
...
<%--	<li><a href="${ctx}/sd/order/salesNew/cancellist" data-toggle="tooltipnorth" data-tooltip="取消单列表">取消单</a></li>--%>
	...
	<li><a href="${ctx}/sd/order/salesNew/reminderlist" data-toggle="tooltipnorth" data-tooltip="催单列表">催单</a></li>
</ul>
<c:set var="currentuser" value="${fns:getUser() }" />
<form:form id="searchForm" modelAttribute="order" action="${ctx}/sd/order/salesNew/materialList" method="post" class="breadcrumb form-search">
	...
	<div>
		<label>工单号：</label><input type=text class="input-small" id="orderNo" name="orderNo" value="${order.orderNo }" maxlength="20" />
		<label>客户:</label>
...
<%--		<input type="text" class="input-small" id="address" name="address" value="${order.address}" maxlength="100" />--%>
	...
	<div style="margin-top:5px">
		...
		<label>下单人：</label>
		<input type=text class="input-mini" id="creator" name="creator" value="${order.creator }" maxlength="20" style="width: 105px;"/>
		<label>下单日期：</label>
		<input
				...
				<td>
					<c:if test="${o.orderCondition.replyFlagKefu == 1 }">
						<a href="javascript:void(0);" style="color: red;"
						   onclick="SalesOrderService.viewOrderDetail('${o.id}','${o.quarter}');"><abbr title="问题反馈/回复未处理">${o.orderNo}</abbr> </a>
					</c:if>
					<c:if test="${o.orderCondition.replyFlagKefu != 1}">
						<a href="javascript:void(0);" onclick="SalesOrderService.viewOrderDetail('${o.id}','${o.quarter}');"><abbr title="点击查看订单详情">${o.orderNo}</abbr> </a>
					</c:if>
					<!-- 投诉 -->
					...
					<c:if test="${o.orderCondition.urgentLevel.id >0}">
						<a data-toggle="tooltip" data-tooltip="${o.orderCondition.urgentLevel.remarks}" class='label label-important' style="background-color: ${o.orderCondition.urgentLevel.markBgcolor};">加急</a>
					</c:if>
...
<script type="text/javascript">
    $(document).ready(function()
    {
		customerLimitDatePicker('beginDate','endDate',6,false);
