...
<body>
    <ul class="nav nav-tabs">
        ...
        <li class="active"><a href="javascript:void(0);">康宝</a></li>
        <li>
...
<form:form id="searchForm" modelAttribute="canboSearchModel" action="${ctx}/b2b/rpt/processlog/canboorder" method="post" class="breadcrumb form-search">
    ...
    <div>
        ...
        <select id="b2bInterfaceId" name="b2bInterfaceId" class="input-small" style="width:240px;">
            ...
            <c:forEach items="${b2bInterfaceId}" var="dict">
                <option value="${dict.value}" <c:out value="${(dict.value == canboSearchModel.b2bInterfaceId)?'selected=selected':''}" /> >${dict.label}</option>
            </c:forEach>
        </select>
        &nbsp;&nbsp;
        <label>状 态：</label>
        <select id="processFlag" name="processFlag" class="input-small" style="width:225px;">
            ...
            <option value="" <c:out value="${(empty canboSearchModel.processFlag)?'selected=selected':''}" />>所有</option>
            <c:forEach var="type" items="${processFlags}">
                <option value="${type.value}" <c:out value="${(type.value == canboSearchModel.processFlag)?'selected=selected':''}" /> >${type.label}</option>
            </c:forEach>
        </select>
        &nbsp;&nbsp;
