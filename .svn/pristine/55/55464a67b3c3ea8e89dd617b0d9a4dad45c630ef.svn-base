    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function () {
            ...
            $("#btnSubmit").click(function () {
                ...
                $("#searchForm").attr("action", "${ctx}/rpt/provider/complainCompleteRatio/cityComplainCompleteOrderRpt");
                $("#searchForm").submit();
            });
                        ...
                        if(data && data.success == true){
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/rpt/provider/complainCompleteRatio/cityComplainCompletedExport?"+ (new Date()).getTime(),
                                        data:$(searchForm).serialize(),
                                        success: function (data) {
                                            if(ajaxLogout(data)){
                                                return false;
...
<ul class="nav nav-tabs">
    ...
    <li class="active"><a href="javascript:void(0);">市投诉率</a></li>
    <li>
        <a href="${ctx}/rpt/provider/complainCompleteRatio/areaComplainCompleteOrderRpt">区/县投诉率</a>
    </li>
</ul>
<c:set var="currentuser" value="${fns:getUser()}"/>
<form:form id="searchForm" modelAttribute="rptSearchCondition"
           action="${ctx}/rpt/provider/complainCompleteRatio/cityComplainCompleteOrderRpt" method="post" class="breadcrumb form-search">
    <div style="width: 90%;">
...
<div id="divGrid" style="overflow-x:hidden;">
    <table id="contentTable" class="table  table-bordered table-condensed table-hover"
        ...
        <tr>
            ...
            <th colspan="${rptSearchCondition.days*5}" width="${rptSearchCondition.days*5*110}">每日完工和投诉(单)</th>
        ...
        <c:if test="${not empty list}">
            ...
            <c:forEach items="${list}" var="cItem">
                <c:set var="rowIndex" value="${rowIndex+1}"/>
                <c:choose>
                    <c:when test="${rowIndex < list.size()}">
                        <tr>
                            <td>
                                <c:if test="${provinceId != cItem.provinceId}">
                                    ${cItem.provinceName}
                                </c:if>
                            </td>
                            <td>
                                <a href="${ctx}/rpt/provider/complainCompleteRatio/areaComplainCompleteOrderRpt?cityId=${cItem.cityId}&selectedYear=${rptSearchCondition.selectedYear}&selectedMonth=${rptSearchCondition.selectedMonth}&areaName=${cItem.cityName}">
                                        ${cItem.cityName}</a>
                            </td>
                            <c:forEach var="i" begin="1" end="${rptSearchCondition.days}" step="1">
                                ...
                                <td>
                                    <fmt:formatNumber maxFractionDigits="0">${cItem[dlname]}</fmt:formatNumber>
                                </td>
                                <td>
                                    <fmt:formatNumber maxFractionDigits="0">${cItem[alname]}</fmt:formatNumber>
                                </td>
                                <td style="color: red;">
                                    <fmt:formatNumber maxFractionDigits="2"> ${cItem[dlname] eq 0 ? cItem[alname]*100 : cItem[alname]/cItem[dlname]*100}</fmt:formatNumber>%
                                </td>
                                <td>
                                    <fmt:formatNumber maxFractionDigits="0">${cItem[clname]}</fmt:formatNumber>
                                </td>
                                <td style="color: red;">
                                    <fmt:formatNumber maxFractionDigits="2"> ${cItem[dlname] eq 0 ? cItem[clname]*100 :cItem[clname]/cItem[dlname]*100}</fmt:formatNumber>%
                                </td>
                            </c:forEach>
                            <td>
                            ...
                            <td>
                                <fmt:formatNumber maxFractionDigits="0"> ${cItem.totalAmount}</fmt:formatNumber>
                            </td>
                            <td style="color: red;">
                                <fmt:formatNumber maxFractionDigits="2"> ${cItem.total eq 0 ? cItem.totalAmount*100 : cItem.totalAmount/cItem.total*100}</fmt:formatNumber>%
                            </td>
                            <td>
                                <fmt:formatNumber maxFractionDigits="0"> ${cItem.evaluateAmount}</fmt:formatNumber>
                            </td>
                            <td style="color: red;">
                                <fmt:formatNumber maxFractionDigits="2"> ${cItem.total eq 0 ? cItem.evaluateAmount*100 : cItem.evaluateAmount/cItem.total*100}</fmt:formatNumber>%
                            </td>
                            <c:set var="provinceId" value="${cItem.provinceId}"/>
                        </tr>
                    </c:when>

                    <c:otherwise>
                        <tr >
                            <td colspan="2">${cItem.provinceName }</td>
                            <c:forEach var="i" begin="1" end="${rptSearchCondition.days}" step="1">
                                <c:set var="dlname" value="d${i}"/>
                                <c:set var="alname" value="a${i}"/>
                                <c:set var="clname" value="c${i}"/>
