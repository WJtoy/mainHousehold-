...
import com.wolfking.jeesite.ms.b2bcenter.mq.sender.B2BCenterModifyKKLOrderMQSender;
...
@Slf4j
@Component
public class B2BCenterModifyKKLOrderRetryMQReceiver implements ChannelAwareMessageListener {

    @Autowired
    private B2BCenterOrderModifyService b2BCenterOrderModifyService;
    @Autowired
    private B2BCenterModifyKKLOrderMQSender b2BCenterModifyKKLOrderMQSender;
    @Autowired
    ...
    public void onMessage(Message message, Channel channel) throws Exception {
        ...
        try {
            ...
            if (modifyMessage != null) {
                MSResponse response = b2BCenterOrderModifyService.processModifyKKLOrderMessage(modifyMessage);
                if (!MSResponse.isSuccessCode(response)) {
                    ...
                    if (times < B2BPushOrderProcessLogToMSRetryMQConfig.RETRY_TIMES) {
                        times++;
                        b2BCenterModifyKKLOrderMQSender.sendRetry(modifyMessage, times);
                    } else {
                        String msgJson = new JsonFormat().printToString(modifyMessage);
                        B2BFailureLogUtils.saveFailureLog(B2BMQQueueType.B2BCENTER_MODIFY_KKL_ORDER_RETRY, msgJson,
                                0L,
                                B2BProcessFlag.PROCESS_FLAG_FAILURE, times + 1,
                                response != null ? response.getMsg() : "B2BCenterModifyKKLOrderRetryMQReceiver");
                    }
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            log.error("B2BCenterModifyKKLOrderRetryMQReceiver - {}", e.getLocalizedMessage());
        }
    }
}
