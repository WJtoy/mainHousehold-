...
@Slf4j
@Component
public class TMallWorkStatusUpdateMQSender implements RabbitTemplate.ConfirmCallback {
     ...
     * @param times   第几次发送
     */
    public void sendDelay(MQTmallPushWorkcardStatusUpdateMessage.TmallPushWorkcardStatusUpdateMessage message, int delay, int times) {
        /*
        try {
            retryTemplate.execute((RetryCallback<Object, Exception>) context -> {
                ...
                rabbitTemplate.convertAndSend(
                        B2BMQConstant.MQ_DELAY_TMALLPUSH_WORKCARDSTATUSUPDATE_EXCHANGE,
                        B2BMQConstant.MQ_DELAY_TMALLPUSH_WORKCARDSTATUSUPDATE_ROUTING,
                ...
                MQTmallPushWorkcardStatusUpdateMessage.TmallPushWorkcardStatusUpdateMessage msg = MQTmallPushWorkcardStatusUpdateMessage.TmallPushWorkcardStatusUpdateMessage.parseFrom((byte[]) msgObj);
                Throwable throwable = context.getLastThrowable();
                String msgJson = new JsonFormat().printToString(msg);
                mqB2bTmallLogService.insertMqB2bTmallLog(B2BMQQueueType.TMALLPUSH_DELAY_WORKCARD_STATUS_UPDATE, msgJson, 3L,
