...
<ul id="navtabs" class="nav nav-tabs">
    <li class="active"><a href="javascript:;" title="处理中列表">处理中</a></li>
    <li><a href="${ctx}/sd/order/crush/closelist" title="已完成列表">已完成</a></li>
</ul>
...
<form:form id="searchForm" modelAttribute="orderCrush" method="post" class="form-inline">
    ...
    <div class="control-group">
        ...
        <input type=text class="input-small" id="orderNo" name="orderNo" value="${orderCrush.orderNo}" maxlength="30"/>
        &nbsp;&nbsp;
        <label class="label-search">突击单号：</label>&nbsp;
        <input type=text class="input-small" id="crushNo" name="crushNo" value="${orderCrush.crushNo}" maxlength="30"/>
        &nbsp;&nbsp;
...
<table id="contentTable" class="table table-striped table-bordered table-condensed table-hover">
    ...
    <c:forEach items="${page.list}" var="entity">
        ...
        <tr>
            ...
            <td>
                <c:choose>
                    <c:when test="${crushedit eq 1}">
                        <a href="javascript:;" onclick="Order.crush_close('${entity.id}','${entity.quarter}')" class="${entity.itemNo>=2?'item_no':''}"> ${entity.crushNo}</a>
                    </c:when>
                    <c:when test="${crushview eq 1}">
                        <a href="javascript:;" onclick="Order.crush_view('${entity.id}','${entity.quarter}')" class="${entity.itemNo>=2?'item_no':''}"> ${entity.crushNo}</a>
                    </c:when>
                    <c:otherwise>${entity.crushNo}</c:otherwise>
                </c:choose>
            </td>
            ...
            <td><span class="label status_50">突击中</span></td>
            <td>${entity.createBy.name}</td>
            <td>
                <fmt:formatDate value="${entity.createDate}" pattern="yyyy-MM-dd HH:mm"></fmt:formatDate>
            </td>
            <td>
                <%--<c:out value="${fn:length(entity.createRemark)}" />--%>
                <c:choose>
                    <c:when test="${fn:length(entity.createRemark)>60}">
                        <a href="javascript:void(0);" data-toggle="tooltip" data-tooltip="${entity.createRemark}">${fns:abbr(entity.createRemark,120)}</a>
                    </c:when>
                    <c:otherwise>
                        ${entity.createRemark}
                    </c:otherwise>
            ...
            <td>
                <c:choose>
                    <c:when test="${crushedit eq 1}">
                        <a href="javascript:void(0);" class="btn btn-mini btn-warning" onclick="Order.crush_close('${entity.id}','${entity.quarter}');">处理</a>
                    </c:when>
                    <c:when test="${crushview eq 1}">
