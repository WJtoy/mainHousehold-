...
<script type="text/javascript">
    ...
    $(document).on("click", "#btnExport", function () {
        ...
        layer.confirm('确定导出配件列表吗?', {icon: 3, title: '系统确认'}, function (index, layero) {
            ...
            jQuery('<form id="exportForm" action="${ctx}/sd/kefuOrderMaterial/rejectlist/export" method="post" style="display:none;">'+ html + '</form>')
...
<form:form id="searchForm" modelAttribute="searchModel" method="post" action="${ctx}/sd/kefuOrderMaterial/rejectlist" class="breadcrumb form-search">
	...
	<div style="margin-top:5px;">
		...
		<label>驳回原因：</label>
		<form:select path="rejectReason" class="input-small" style="width:185px;">
			<form:option value="" label="所有"/>
			<form:options items="${fns:getDictListFromMS('material_reject_type')}" itemLabel="label" itemValue="value"
						  htmlEscape="false"/>
		</form:select>
		<label>跟踪状态：</label>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed table-hover">
	...
	<c:forEach items="${page.list}" var="master" varStatus="i" begin="0">
		...
		<tr id="tr_${master.id}" data-index="${i.index}">
			...
			<td rowspan="${rowspan}">${master.closeType}</td>
			<td rowspan="${rowspan}">${master.closeRemark}</td>
			<!-- 跟踪进度 -->
