    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#btnSubmit").click(function() {
                ...
                $("#searchForm").attr("action","${ctx}/finance/rpt/servicePointInvoice/servicePointInvoiceRpt");
                $("#searchForm").submit();
            });
            ...
            $("#btnExport").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "${ctx}/finance/rpt/servicePointInvoice/checkExportTask?"+ (new Date()).getTime(),
                        ...
                        if(data && data.success == true){
                            ...
                            top.$.jBox.confirm("确认要导出下游安维付款清单吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/finance/rpt/servicePointInvoice/export?"+ (new Date()).getTime(),
                                        data:$(searchForm).serialize(),
                                        success: function (data) {
                                            if(ajaxLogout(data)){
                                                return false;
...
<ul class="nav nav-tabs">
    <li>
    <a href="${ctx}/finance/rpt/servicePointInvoiceSummary/servicePointPaymentSummary">网点付款汇总</a>
    </li>
    <li class="active"><a href="javascript:void(0);">网点付款清单</a></li>
</ul>
<form:form id="searchForm" modelAttribute="rptSearchCondition" action="${ctx}/finance/rpt/servicePointInvoice/servicePointInvoiceRpt" method="post" class="breadcrumb form-search">
