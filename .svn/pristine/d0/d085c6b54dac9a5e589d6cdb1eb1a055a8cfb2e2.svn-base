        ...
        $(document).ready(function () {
            $("#inputForm").validate({
                ...
                submitHandler: function (form) {
                    ...
                    <c:if test="${limitRemoteDict != null}">
            ...
            $("[id='orderFee.planDistance']").on("blur",function(){
                ...
                if(totalCharge > limitTotalCharge){
                    $("#btnSubmit").attr('disabled', 'disabled');
                    layerAlert("远程费用和其他费用合计已超过" + limitTotalCharge + "元，不允许派单!");
                }else{
                    $("#btnSubmit").removeAttr('disabled');
                    $("[id='orderFee.customerPlanTravelCharge']").val("0");
                    $("[id='orderFee.customerPlanOtherCharge']").val("0");
...
<form:form id="inputForm" modelAttribute="order" action="${ctx}/sd/order/plan" method="post" class="form-horizontal">
    ...
    <c:if test="${order.dataSource.value eq '22'}">
        <div class="row-fluid">
            <div class="alert">
                <icon class="icon-warning-sign" style="margin-right:10px;"></icon>
    ...
    <div class="row-fluid">
        <div class="span6">
            <div class="control-group">
                ...
                <div class="controls">
                    <md:rushpointselectforplannew id="servicePoint" name="orderCondition.servicePoint.id" value="${order.orderCondition.servicePoint.id}" labelName="orderCondition.servicePoint.name" labelValue="${order.orderCondition.servicePoint.name}"
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input type="number" id="orderFee.planDistance" name="orderFee.planDistance" value="${fns:formatDouble(order.orderFee.planDistance,0)}" class="input-large required digits" />&nbsp;&nbsp;公里
                            <%--<form:input path="orderFee.planDistance" htmlEscape="false" min="0" max="999" cssClass="input-large required digits"/> &nbsp;&nbsp;公里--%>
                    </div>
                </div>
            </div>
        </c:if>
