...
<body>
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="md:urgentlevel:edit">
			<li><a href="${ctx}/md/urgentlevel/form">添加</a></li>
		</shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="urgentLevel" action="${ctx}/md/urgentlevel/list" method="post" class="breadcrumb form-search">
			...
			<c:forEach items="${page.list}" var="entity">
				<tr>
					...
					<td style="color: red;"><b><fmt:formatNumber value="${entity.chargeOut}"
																 pattern="0.00"></fmt:formatNumber></b></td>
					<td>${entity.sort}</td>
					<td><div style="width: 40px; height: 20px;background-color: ${entity.markBgcolor};"></div></td>
					<td>${entity.remarks}</td>
					<shiro:hasPermission name="md:urgentlevel:edit">
						<td><a href="${ctx}/md/urgentlevel/form?id=${entity.id}">修改</a>
							<a href="${ctx}/md/urgentlevel/delete?id=${entity.id}"
