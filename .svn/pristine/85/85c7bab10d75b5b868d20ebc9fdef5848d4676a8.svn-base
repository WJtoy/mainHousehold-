    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('change',"#productId",function (e) {
                ...
                setPage(0);
                var url = "${ctx}/provider/md/customerErrorType/findList?productId="+productId;
            ...
            $("#btnSubmit").on("click",function(){
                ...
                var url = "${ctx}/provider/md/customerErrorType/save";
                $("#searchForm").attr("action",url);
                $("#searchForm").submit();
                return false;
            });
            ...
            $("#searchForm").validate({
                rules: {
                    name: {remote: "${ctx}/provider/md/customerErrorType/checkName?productId="+$("#productId").val()+"&customerId="+$("#customerId").val()}
                },
                messages: {
        ...
        function del(id,productId,customerId) {
            $.ajax({
                url:"${ctx}/provider/md/errorCode/ajax/getIdByProductAndErrorType?productId=0&errorTypeId="+id,
                success:function (e) {
                    if(e.success){
                        if (e.data>0) {
                            layerError("该故障分类下有故障现象，不能删除","错误提示");
                        } else {
                            var url = "${ctx}/provider/md/customerErrorType/delete?id="+id+"&productId="+productId+"&customerId="+customerId;
                            $("#searchForm").attr("action",url);
                            $("#searchForm").submit();
                            return false;
                        }
            ...
            <select id="customerId" name="customerId" class="input-small" style="width:250px;">
                ...
                <c:forEach items="${fns:getMyCustomerList()}" var="dict">
                    <option value="${dict.id}"
                            <c:out value="${(errorType.customerId eq dict.id)?'selected=selected':''}" />>${dict.name}</option>
                </c:forEach>
            </select>
        </div>
    </div>
