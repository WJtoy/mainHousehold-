	...
	<%@ include file="/WEB-INF/views/include/pageSearch.jsp" %>
        ...
        <script type="text/javascript">
            ...
            function editCustomerBrand(type,id,customerName,brandName) {
                var text = "添加品牌";
                var url = "${ctx}/provider/md/customerBrand/form";
                var area = ['727px', '414px'];
                if(type == 20){
                    text = "修改品牌";
                    url = "${ctx}/provider/md/customerBrand/form?id= "+ id +"&customerName=${fns:urlEncode(customerName)}&brandName=${fns:urlEncode(brandName)}";
                }
                top.layer.open({
                    type: 2,
                    id:"customerBrand",
