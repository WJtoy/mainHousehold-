...
<ul id="navtabs" class="nav nav-tabs">
    <li><a href="${ctx}/sd/order/servicePointOrderList/reminderList" title="催单待回复">催单</a></li>
    <li><a href="${ctx}/sd/order/servicePointOrderList/noAppointmentList" title="未预约的订单">未预约</a></li>
    <li><a href="${ctx}/sd/order/servicePointOrderList/arriveAppointmentList" title="预约到期的订单">预约到期</a></li>
    <li><a href="${ctx}/sd/order/servicePointOrderList/passAppointmentList" title="预约超期的订单列表">预约超期</a></li>
    <li><a href="${ctx}/sd/order/servicePointOrderList/servicedList" title="待完成的订单列表">待完成</a></li>
    <li><a href="${ctx}/sd/order/servicePointOrderList/appCompletedList" title="待回访的订单列表">待回访</a></li>
    <li><a href="${ctx}/sd/order/servicePointOrderList/waitingAccessoryList" title="等配件的订单列表">等配件</a></li>
    <li><a href="${ctx}/sd/order/servicePointOrderList/pendingList" title="停滞的订单列表">停滞</a></li>
    <li><a href="${ctx}/sd/order/servicePointOrderList/uncompletedList" title="未完成的订单列表">未完成</a></li>
    <li><a href="${ctx}/sd/order/servicePointOrderList/completedList" title="已完成的订单列表">已完成</a></li>
    <li><a href="${ctx}/sd/order/servicePointOrderList/returnlist" title="退单列表">退单</a></li>
    <li class="active"><a href="javascript:void(0);">所有</a></li>
</ul>
<c:set var="currentuser" value="${fns:getUser() }"/>
<form:form id="searchForm" modelAttribute="order" action="${ctx}/sd/order/servicePointOrderList/allList" method="post" class="breadcrumb form-search">
    ...
    <div style="margin-top: 8px;">
        <label>接单时间：</label>
        <input id="acceptDateRange" name="acceptDateRange" type="text" style="width:185px;margin-left:4px" maxlength="25" class="input-xlarge Wdate"
...
<table id="contentTable" class="table table-bordered table-condensed table-striped" style="table-layout:fixed;" cellspacing="0" width="100%">
    <thead>
    <tr>
        <th width="30">序号</th>
        <th width="110">单号</th>
                ...
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu">
                        <c:if test="${ order.canPlanOrder()}">
                            <li>
                                <a href="javascript:void(0);" onclick="Order.servicePointPlan('${order.id}','${order.orderNo}','${order.quarter}');">
                                    <i class="icon-truck"></i>&nbsp;派单</a>
                            </li>
                        </c:if>
<%--                        <c:if test="${order.orderCondition.status.intValue >=20 and order.orderCondition.status.intValue <= 50 and order.orderCondition.subStatus >= 10 and order.orderCondition.subStatus <=40 and order.isAllowedForSetAppointment() == 1}">--%>
                        <c:if test="${order.orderCondition.status.intValue >=20 and order.orderCondition.status.intValue <= 55 and order.orderCondition.subStatus >= 10 and order.orderCondition.subStatus <=40 and order.isAllowedForSetAppointment() == 1}">
                        <li>
                            <a href="javascript:void(0);" onclick="Order.servicepointPending('${order.id}','${order.orderNo}','${order.quarter}',${order.orderCondition.reservationTimes});"><i
                                    class="icon-calendar"></i>&nbsp;预约时间</a>
                        </li>
                        </c:if>
                        <c:if test="${order.orderCondition.status.value eq '40'}">
                            <li>
                                <a href="javascript:void(0);" onclick="Order.orderConfirmDoor('${order.id}','${order.quarter}',undefined,1);"><i
                                        class="icon-home"></i>&nbsp;确认上门</a>
                            </li>
                        </c:if>
                        <c:if test="${order.orderCondition.status.value eq '50' and order.orderCondition.subStatus ne 70 }">
                            <li>
                                <a href="javascript:void(0);" onclick="Order.servicepointComplete('${order.id}','${order.orderNo}','${order.quarter}');"><i
            ...
            <td>
                ...
                <c:if test="${ order.canTracking()}">
                    <a href="javascript:void(0);" onclick="Order.trackingEnginner('${order.id}','${order.quarter}');"
            ...
            <td>
                <c:if test="${order.orderCondition.finishPhotoQty > 0 }">
                    <a href="javascript:void(0);"
                       onclick="Order.photolistNew('${order.id}','${order.quarter}',${fns:isNewOrder(order.orderNo)});"
                       class="btn btn-mini btn-primary">查看</a>
                </c:if>
                <c:if test="${order.orderCondition.finishPhotoQty eq 0}">
                    <a href="javascript:void(0);"
                       onclick="Order.photolistNew('${order.id}','${order.quarter}',${fns:isNewOrder(order.orderNo)});"
                       class="btn btn-mini btn-primary">上传</a>
                </c:if>
            </td>
            <td>
