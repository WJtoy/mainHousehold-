...
import com.wolfking.jeesite.ms.providerrpt.feign.MSCancelledOrderNewRptFeign;
...
@Service
public class MSCancelledOrderNewRptService {

    @Autowired
    private MSCancelledOrderNewRptFeign msCancelledOrderNewRptFeign;
    ...
    public Page<RPTCancelledOrderEntity> getCancelledOrderList(Page<RPTCancelledOrderEntity> page, Date cancelBeginDate, Date cancelEndDate,
                                                                 Long customerId, Integer paymentType, Integer cancelResponsible, Date createBeginDate, Date createEndDate,List<Long> productCategoryIds,Long salesId,Integer subFlag) {
        ...
        if (cancelBeginDate != null && cancelEndDate != null) {
            ...
            if (microServicesProperties.getReport().getEnabled()) {
                MSResponse<MSPage<RPTCancelledOrderEntity>> msResponse = msCancelledOrderNewRptFeign.getCancelledOrderList(search);
    ...
    public void checkRptExportTask(Date cancelBeginDate, Date cancelEndDate,
                                   Long customerId, Integer paymentType, Integer cancelResponsible, Date createBeginDate, Date createEndDate,List<Long> productCategoryIds,Long salesId,User user,Integer subFlag) {

        RPTCancelledOrderSearch searchCondition = setSearchCondition(cancelBeginDate,cancelEndDate,customerId,paymentType,cancelResponsible,createBeginDate,createEndDate,productCategoryIds,salesId,subFlag);
        ...
        reportExportTaskService.checkRptExportTask(RPTReportEnum.CANCELLED_ORDER_DAILY_RPT, user, searchConditionJson);
    }
    ...
    public void createRptExportTask(Date cancelBeginDate, Date cancelEndDate,
        ...
        searchCondition.setPageNo(1);
        searchCondition.setPageSize(200000);
        String reportTitle = customerName + DateUtils.formatDate(cancelBeginDate, "yyyy年MM月dd日") + "~" + DateUtils.formatDate(cancelEndDate, "yyyy年MM月dd日")+ "每日退单明细表";
        String searchConditionJson = RedisGsonUtils.toJson(searchCondition);
        reportExportTaskService.createRptExportTask(RPTReportEnum.CANCELLED_ORDER_DAILY_RPT, RPTReportTypeEnum.ORDER_REPORT, user, reportTitle, searchConditionJson);
    }
