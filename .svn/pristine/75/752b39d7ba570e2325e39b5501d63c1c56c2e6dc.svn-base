        ...
        $(document).ready(function() {
            $("#inputForm").validate({
                    submitHandler : function(form)
                    {
                        ...
                        //form.submit();
                        var options = {
                            url: "${ctx}/provider/md/customerProductModel/save",
                            type: 'post',
                            dataType: 'json',
                            data:$(form).serialize(),
	...
	<c:if test="${canAction == true}">
		<div id="main">
			...
			<div class="control-group">
				...
				<div class="controls">
					<c:choose>
						...
						<c:otherwise>
							<select id="productId" name ="productId" style="width:250px;" class="required">
			...
			<div class="control-group">
				...
				<div class="controls">
					<form:textarea path="remarks" htmlEscape="false" rows="3" maxlength="200" class="input-xlarge" cssStyle="min-width: 280px;max-width: 560px;min-height: 70px;max-height: 210px;width: 387px;"/>
				</div>
			</div>
		</div>
	</c:if>
