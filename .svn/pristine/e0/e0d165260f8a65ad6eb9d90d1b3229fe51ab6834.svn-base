...
<html>
<head>
	<title>投诉单-处理申诉</title>
	<meta name="description" content="处理申诉">
...
<body>
	...
	<fieldset>
		...
		<c:if test="${complain.status.value == '3'}">
			<legend>申诉</legend>
			<div class="row-fluid">
				<div class="span4">
					<div class="control-group">
						...
						<div class="controls">
							<form:input path="appealBy.name" disabled="true" htmlEscape="false" cssClass="input-block-level required" maxlength="10" />
						</div>
					</div>
				</div>
				<div class="span4">
					<div class="control-group">
						...
						<div class="controls">
							<input id="appealDate" name="appealDate"
								   type="text" disabled="true"
								   maxlength="21" class="input-block-level required Wdate"
								   value="${fns:formatDate(complain.appealDate,'yyyy-MM-dd HH:mm:ss')}" />
						</div>
					</div>
				</div>
			</div>
			<div class="row-fluid">
				<div class="span12">
					<div class="control-group">
						<label class="control-label">申诉内容:</label>
						<div class="controls">
							<form:textarea path="appealRemark" disabled="true" htmlEscape="false" rows="3" maxlength="500" class="input-block-level required" />
						</div>
					</div>
				</div>
			</div>
		...
		<div class="row-fluid">
			<div class="span12">
				<div class="control-group">
					...
					<div class="controls">
						<form:textarea path="completeRemark" htmlEscape="false" rows="3" maxlength="500" class="input-block-level required" />
					</div>
				</div>
			</div>
		</div>
			...
			<div class="row-fluid">
				<div class="span4">
					<div class="control-group">
						...
						<label class="control-label"><form:checkbox path="amerceResultIds" value="1" disabled="${canCheckedServicePoint == 'false'?'true':'false'}" label="网点" cssClass="amerceResultIds" /></label>
						<div class="controls">
		...
		<div class="form-actions">
			<c:if test="${canAction eq true }">
				<shiro:hasPermission name="sd:complain:complete">
					<c:if test="${complain.status.value eq '3' && complain.judgeDate ne null}">
...
<script type="text/javascript">
    ...
    //赔偿
    <c:choose>
    <c:when test="${complain.compensateResult == 1}">
    $("#compensateResultIds1").prop("checked",true);
    </c:when>
    <c:when test="${complain.compensateResult == 2}">
    $("#compensateResultIds2").prop("checked",true);
    </c:when>
    <c:when test="${complain.compensateResult == 3}">
    $("#compensateResultIds1").prop("checked",true);
    $("#compensateResultIds2").prop("checked",true);
    </c:when>
    </c:choose>
    // 罚款
    <c:choose>
    <c:when test="${complain.amerceResult == 1}">
    $("#amerceResultIds1").prop("checked",true);
    </c:when>
    <c:when test="${complain.amerceResult == 2}">
    $("#amerceResultIds2").prop("checked",true);
    </c:when>
    <c:when test="${complain.amerceResult == 3}">
    $("#amerceResultIds1").prop("checked",true);
    $("#amerceResultIds2").prop("checked",true);
    </c:when>
    </c:choose>
