...
<script type="text/javascript">
        ...
        $("#inputForm").validate({
            submitHandler: function (form) {
                ...
                if ($("#inputForm").data("changed")) {
                    // submit the form
                    var options = {
                        url: "${ctx}/provider/md/customerPrice/saveProductPricesNew?qCustomerId=${customerId}&qCustomerName=${fns:urlEncode(customerName)}&qProductCategoryId=${productCategoryId}&qProductCategoryName=${fns:urlEncode(productCategoryName)}&qProductId=${productId}&qProductName=${fns:urlEncode(productName)}&qFirstSearch=${qFirstSearch}",
                        type: 'post',
                        dataType: 'json',
                        data:$(form).serialize(),
    ...
    function disableOrEnable(obj, line){
        // var newPrice = $("#tr_"+line+" .p").attr("name", "newPrices["+line+"].price").val();
        // var newDiscountPrice = $("#tr_"+line+" .p").attr("name", "newPrices["+line+"].discountPrice").val();
        // var newBlockedPrice = $("#tr_"+line+" .p").attr("name", "newPrices["+line+"].blockedPrice").val();
