    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#btnSubmit").click(function() {
                ...
                $("#searchForm").attr("action","${ctx}/rpt/provider/depositRecharge/depositRechargeDetails");
                $("#searchForm").submit();
            });
            ...
            $("#btnExport").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "${ctx}/rpt/provider/depositRecharge/checkExportDetalis?"+ (new Date()).getTime(),
                        ...
                        if(data && data.success == true){
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/rpt/provider/depositRecharge/exportDetalis?"+ (new Date()).getTime(),
                                        data:$(searchForm).serialize(),
                                        success: function (data) {
                                            if(ajaxLogout(data)){
                                                return false;
...
<ul class="nav nav-tabs">
    <li>
        <a href="${ctx}/rpt/provider/depositRecharge/depositRechargeSummary">质保金汇总</a>
    </li>
    <li class="active"><a href="javascript:void(0);">质保金明细</a></li>
</ul>
<form:form id="searchForm" modelAttribute="rptSearchCondition" action="${ctx}/rpt/provider/depositRecharge/depositRechargeDetails" method="post" class="breadcrumb form-search">
    ...
    <div style="width: 90%">
        ...
        &nbsp;&nbsp;
        <shiro:hasPermission name="rpt:depositRecharge:view"><input id="btnSubmit" class="btn btn-primary" type="button" value="查询"/></shiro:hasPermission>
        &nbsp;&nbsp;
