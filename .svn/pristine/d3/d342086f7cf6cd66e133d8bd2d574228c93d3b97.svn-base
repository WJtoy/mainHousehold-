    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#btnSubmit").click(function(event){
                ...
                if(dismountData.uploadFlag === 0){
                    ...
                    validSN(oldSN,'正在请求第三方系统检验[旧产品SN码]，请耐心等待...','请输入[旧产品SN码]！','检验[旧产品SN码]错误，请重试!');
                    ...
                    var newSN = $("#dismountNewSN").val();
                    validSN(newSN,'正在请求第三方系统检验[新产品SN码]，请耐心等待...','请输入[新产品SN码]！','检验[新产品SN码]错误，请重试!');
                    if(snValidResult === false){
                        event.preventDefault();
                        $btnSubmit.removeAttr('disabled');
                        clickTag = 0;
                ...
                $.ajax({
                    ...
                    cache: false,
                    type: "POST",
                    url: "${ctx}/sd/order/return/saveCompleteFrom",
                    dataType: 'json',
        ...
        function validForm(){
            ...
            if(oldSN === "" || oldSN === undefined || oldSN === null){
                layerError("请输入[旧产品SN码]！","错误提示");
                return submitForm;
            }
            dismountData['oldSN'] = oldSN;
            var newSN = $("#dismountNewSN").val();
            newSN = newSN.replace(/(^\s*)|(\s*$)/g, '');  //去除空格;
            if(newSN === "" || newSN === undefined || newSN === null){
                layerError("请输入[新产品SN码]！","错误提示");
                return submitForm;
            }
            dismountData['newSN'] = newSN;
            ...
            // submitForm['dismountData'] = dismountData;
            //logistics
            ...
            // submitForm['logisticsData'] = logisticsData;
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data && data.success == true) {
                        ...
                        layerError(data.message + "<br>" + validErrorMsg, "错误提示");
                        return;
                    }
                    isExecute = false;
                    return;
    ...
    <div class="row-fluid">
        <div class="span6">
            <div class="control-group">
                <label class="control-label"><font color="red">*</font>旧产品SN码：</label>
                <div class="controls">
                    <input id="dismountOldSN" placeholder="请输入旧产品SN码"
        ...
        <div class="span6">
            <div class="control-group">
                <label class="control-label"><font color="red">*</font>新产品SN码：</label>
                <div class="controls">
                    <input id="dismountNewSN" placeholder="请输入新产品SN码"
                           class="fromInput input-block-level" ${dismountItem.uploadFlag == 1?'disabled':''}
                           value="${dismountItem.newSN}" maxlength="64"/>
                </div>
            </div>
        </div>
    </div>
