...
<head>
    ...
    <script type="text/javascript">
        ...
        function go2Edit(priceId,customerId){
            ...
            //window.location="${ctx}/fi/md/customerPrice/form?id="+priceId+"&qFirstSearch=" + qFirstSearch+"&qCustomerId=" + customerId + "&examine=examine";

            var text = "服务价格";
            //var url = "${ctx}/fi/md/customerPrice/productForm?customer.id="+customerId+"&product.id="+productId+"&"+params+"&qFirstSearch="+qFirstSearch+"";
            var url = "${ctx}/fi/md/customerPrice/form?id="+priceId+"&qFirstSearch=" + qFirstSearch+"&qCustomerId=" + customerId + "&examine=examine";
            top.layer.open({
                type: 2,
                id:"customerPrice",
                zIndex:19891015,
            ...
            var submit = function (v, h, f) {
                if (v == 'ok') {
                    ...
                    $.ajax({
                        cache: false,
                        type: "POST",
                        url: "${ctx}/fi/md/customerPrice/approve",
                        data: data,
            ...
            var submit = function (v, h, f) {
                if (v == 'ok') {
                    ...
                    $.ajax({
                        cache: false,
                        type: "POST",
                        url: "${ctx}/fi/md/customerPrice/stop",
                        data: data,
                        success: function (data) {
                            if(ajaxLogout(data)){
                                return false;
                            ...
                            if (data.success){
                                ...
                                location.href="${ctx}/fi/md/customerPrice/approvelist";
                            }
                            else{
                                top.$.jBox.closeTip();
                                top.$.jBox.error(data.message);
...
<form:form id="searchForm" modelAttribute="customerPrice" action="${ctx}/fi/md/customerPrice/approvelist" method="post" class="breadcrumb form-search">
    ...
    <shiro:hasPermission name="fi:md:customerprice:approve">
...
<table id="contentTable" class="table table-striped table-bordered table-condensed table-hover">
    ...
    <c:forEach items="${page.list}" var="customerprice" varStatus="i" begin="0">
        <tr>
            ...
            <td>
                <shiro:hasPermission name="fi:md:customerprice:approveedit">
                    <a href="javascript:void(0);" onclick="go2Edit('${customerprice.id}','${customerprice.customer.id}');">修改</a>
                </shiro:hasPermission>
            </td>
        </tr>
    </c:forEach>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("tbody tr td:first-child").css({"text-align":"center","vertical-align":"middle"});
    });
</script>
</body>
</html>
