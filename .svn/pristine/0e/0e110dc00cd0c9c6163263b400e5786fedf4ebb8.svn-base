    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $("#inputForm").validate({
                rules: {
                    mobile: {
                        remote: "${ctx}/sys/user/checkMobile?expectId=${user.id}&expectType=user&returnType=message"
                    }
                },
                messages: {
                    mobile: {
...
<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/info" method="post" class="form-horizontal"><%--
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <form:input path="mobile" htmlEscape="false" cssClass="required mobile" maxlength="11"/>
        </div>
    </div>
    <div class="control-group">
        ...
        <div class="controls">
            <c:choose>
                <c:when test="${user.userType == 2 || user.userType == 7}">
                    <form:input path="qq" htmlEscape="false" maxlength="11" cssClass="required digits" />
                    <span id="span_qq" class="help-inline"><font color="red">*</font> </span>
                </c:when>
                <c:otherwise>
                    <form:input path="qq" htmlEscape="false" maxlength="11" cssClass="digits" />
                </c:otherwise>
            </c:choose>
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <label class="lbl">${fns:getDictLabelFromMS(user.userType, 'sys_user_type', '无')}</label><%-- 切换为微服务 --%>
        </div>
    </div>
    <div class="control-group">
