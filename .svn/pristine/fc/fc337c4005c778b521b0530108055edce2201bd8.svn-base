...
<head>
    <title>工单投诉单处理中列表</title>
    <meta name="description" content="待跟进">
...
<ul id="navtabs" class="nav nav-tabs">
    <shiro:hasPermission name="sd:complain:judge">
        ...
        <li class="active"><a href="javascript:void(0);" title="待跟进投诉单列表">待跟进</a></li>
    </shiro:hasPermission>
    <shiro:hasPermission name="sd:complain:complete">
        ...
        <li><a href="${ctx}/sd/complain/appeallist" title="申诉投诉单列表">申诉</a></li>
    </shiro:hasPermission>
    <li><a href="${ctx}/sd/complain/alllist" title="所有投诉单列表">所有</a></li>
</ul>
<c:set var="currentuser" value="${fns:getUser() }"/>
<form:form id="searchForm" modelAttribute="complain" action="${ctx}/sd/complain/appointlist" method="post" class="form-inline">
    ...
    <div class="control-group">
        ...
        <label>区域</label>
        <%--
        <sys:treeselect id="area" name="area.id" value="${complain.area.id}" levelValue="${complain.areaLevel}" nodeLevel="true"
                        labelName="area.name" labelValue="${complain.area.name }" title="区域"
...
<div id="divGrid">
    <table id="contentTable" class="table table-bordered table-condensed " style="table-layout:fixed;">
        ...
        <tr>
            ...
            <th width="80">投诉日期</th>
            <th width="150">投诉对象</th>
            <th width="250">投诉项目</th>
        ...
        <c:forEach items="${page.list}" var="model">
            ...
            <tr>
                ...
                <td>
                    <c:choose>
                        <c:when test="${model.status.value eq '0' || model.status.value eq '1'}">
                            <shiro:hasPermission name="sd:complain:judge">
                                <a href="javascript:void(0);" onclick="Order.complain_judgeNew('${model.id}','${model.quarter}');">${model.complainNo}</a>
                            </shiro:hasPermission>
                        </c:when>
                ...
                <td>
                    <a href="javascript:void(0);" data-toggle="tooltip" data-tooltip="${model.complainRemark}">
                            ${fns:abbr(model.complainRemark,40)}
                    </a>
                </td>
                <td>${model.userName}</td>
                <td>${model.userPhone}</td>
                <td>${model.complainBy}</td>
                <td><fmt:formatDate value="${model.appointDate}" pattern="yyyy-MM-dd"></fmt:formatDate><br>
                    <fmt:formatDate value="${model.appointDate}" pattern="HH:mm"></fmt:formatDate><br>
                </td>
                <td><a href="javascript:void(0);" class="btn btn-mini btn-warning" onclick="openApppointForm('${model.id}','${model.quarter}');">待跟进</a></td>
                <td>
                    <c:choose>
                        <c:when test="${model.status.value eq '0' || model.status.value eq '1'}">
                            <shiro:hasPermission name="sd:complain:judge">
                                <a href="javascript:void(0);" class="btn btn-mini btn-warning" onclick="Order.complain_judge('${model.id}','${model.quarter}');">判定</a>
                            </shiro:hasPermission>
                        </c:when>
                        <c:when test="${model.status.value eq '1'}">
                            <shiro:hasPermission name="sd:complain:complete">
