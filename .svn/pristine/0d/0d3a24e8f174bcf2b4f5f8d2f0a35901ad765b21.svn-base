...
<ul id="navtabs" class="nav nav-tabs">
    <li><a href="${ctx}/sd/order/servicepoint/waitingAppointmentList" title="待预约的订单列表">待预约</a></li>
    <li><a href="${ctx}/sd/order/servicepoint/processingList" title="处理中的订单">处理中</a></li>
    <li><a href="${ctx}/sd/order/servicepoint/appointedList" title="预约中的订单列表">已预约</a></li>
    <li><a href="${ctx}/sd/order/servicepoint/waitingAccessoryList" title="等配件的订单列表">等配件</a></li>
    <li><a href="${ctx}/sd/order/servicepoint/pendingList" title="停滞的订单列表">停滞</a></li>
    <li><a href="${ctx}/sd/order/servicepoint/finishlist" title="完成的订单列表">完成单</a></li>
    <li><a href="${ctx}/sd/order/servicepoint/returnlist" title="退单列表">退单</a></li>
    <li class="active"><a href="javascript:void(0);">所有</a></li>
</ul>
<c:set var="currentuser" value="${fns:getUser() }"/>
<form:form id="searchForm" modelAttribute="order" action="${ctx}/sd/order/servicepoint/allList" method="post"
    ...
    <input id="pageSize" name="pageSize" type="hidden"
           value="${page.pageSize}"/>
    <div>
        <label>订单编号：</label>&nbsp;<input type=text class="input-small"
                                         id="orderNo" name="orderNo" value="${order.orderNo}" maxlength="20"/>
        <label>联系人：</label><input
            type=text class="input-mini" id="userName" name="userName"
            value="${order.userName}" maxlength="20"/>
        <label>电 话：</label><input type=text
                                  class="input-small" id="userPhone" name="userPhone" value="${order.userPhone}"
                                  maxlength="20"/>
        <label>安维姓名：</label>
        <input type=text class="input-mini" id="engineerName" name="engineerName" value="${order.engineerName}"
               maxlength="20"/>
    </div>
    <div style="margin-top: 8px;">
        <label>接单时间：</label><input id="beginAcceptDate" name="beginAcceptDate"
                                   type="text" readonly="readonly" style="width:120px;margin-left:4px"
                                   maxlength="20" class="input-small Wdate"
                                   value="${fns:formatDate(order.beginAcceptDate,'yyyy-MM-dd')}"
                                   onclick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'%y-{%M - 3}-%d %H:%m:%s',isShowClear:false});"/>
        <label style="width:45px;text-align: center;"> ~ </label>&nbsp;&nbsp;&nbsp;<input id="endAcceptDate"
                                                                                          name="endAcceptDate"
                                                                                          type="text"
                                                                                          readonly="readonly"
                                                                                          style="width:120px"
                                                                                          ...
                                                                                          value="${fns:formatDate(order.endAcceptDate,'yyyy-MM-dd')}"
