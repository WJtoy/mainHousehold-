...
<head>
	...
	<%@ include file="/WEB-INF/views/modules/servicePoint/sd/reminder/tpl/items.html" %>
	<!-- 禁用词 -->
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			var itemId = ${reminder.itemId};
			if(itemId==null || itemId==''){
                itemId = '0';
			}
			var data={itemId:itemId,items:items}
			ServicePointOrderService.showReminderItems(data);
		});
		...
		function saveReminderReply(){
			...
			layer.confirm('确定提交催单回复内容吗?', {
				...
				$.ajax({
					...
					cache: false,
					type: "POST",
					url: "${ctx}/servicePoint/sd/reminder/servicePointReplyReminder?"+ (new Date()).getTime(),
...
<body>
	<div class="centerForm">
		<textarea id="itemInfo" style="display: none">${fns:toJson(reminder.items)}</textarea>
