...
import com.wolfking.jeesite.modules.sd.service.CustomerOrderService;
 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/sd/order/customerNew")
@Slf4j
public class CustomerOrderController extends BaseController {

    @Autowired
    public CustomerOrderController(ProductService productService,
    ...
    private static final String VIEW_NAME_PROCESSING_ORDER_LIST = "modules/sd/customerNew/list";
    private static final String VIEW_NAME_COMPLETED_ORDER_LIST = "modules/sd/customerNew/finishList";
    private static final String VIEW_NAME_CANCELED_ORDER_LIST = "modules/sd/customerNew/cancelList";
    private static final String VIEW_NAME_RETURNED_ORDER_LIST = "modules/sd/customerNew/returnList";
    private static final String VIEW_NAME_ALL_ORDER_LIST = "modules/sd/customerNew/allList";
    private static final String VIEW_NAME_COMPLAINED_ORDER_LIST = "modules/sd/customerNew/complainList";
    private static final String VIEW_NAME_WAITING_PARTS_ORDER_LIST = "modules/sd/customerNew/orderListMaterial";
    private static final String VIEW_NAME_REMINDER_ORDER_LIST = "modules/sd/customerNew/reminderList";

    private static final String VIEW_NAME_NEW_ALL_ORDER_LIST = "modules/sd/customerNew/newallList";
    ...
    public String processingOrderLit(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            page = customerOrderService.getProcessingOrderLit(new Page<>(request, response, DEFAULT_PAGE_SIZE), order);
            //判断权限
    ...
    public String completedOrderLit(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            page = customerOrderService.getCompletedOrderLit(new Page<>(request, response, DEFAULT_PAGE_SIZE), order);
            model.addAttribute(MODEL_ATTR_PAGE, page);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
    ...
    public String canceledOrderLit(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if (request.getMethod().equalsIgnoreCase("post")) {
            try {
                page = customerOrderService.getCanceledOrderLit(new Page<>(request, response, DEFAULT_PAGE_SIZE), order);
                model.addAttribute(MODEL_ATTR_PAGE, page);
            } catch (Exception e) {
                addMessage(model, "错误：" + e.getMessage());
            }
    ...
    public String returnedOrderLit(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            page = customerOrderService.getReturnedOrderLit(new Page<>(request, response, DEFAULT_PAGE_SIZE), order);
            //判断权限
            Subject subject = SecurityUtils.getSubject();
            if(subject.isPermitted("sd:reminder:create")) {
                List<Order> orders = page.getList();
    ...
    public String allOrderList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if(request.getMethod().equalsIgnoreCase("post") ||
            ...
            try {
                page = customerOrderService.getAllOrderLit(new Page<>(request, response, DEFAULT_PAGE_SIZE), order);
                //判断权限
                Subject subject = SecurityUtils.getSubject();
                if (subject.isPermitted("sd:reminder:create")) {
                    List<Order> orders = page.getList();
    ...
    public String complainedOrderLit(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            page = customerOrderService.getComplainedOrderLit(new Page<>(request, response, DEFAULT_PAGE_SIZE), order);
            //判断权限
            Subject subject = SecurityUtils.getSubject();
            if(subject.isPermitted("sd:reminder:create")) {
                List<Order> orders = page.getList();
    ...
    public String waitingPartsOrderList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            page = customerOrderService.getWaitingPartsOrderList(new Page<>(request, response, DEFAULT_PAGE_SIZE), order);
            //判断权限
            Subject subject = SecurityUtils.getSubject();
            if(subject.isPermitted("sd:reminder:create")) {
                List<Order> orders = page.getList();
    ...
    public String customerReminderList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            page = customerOrderService.getReminderOrderLit(new Page<>(request, response, DEFAULT_PAGE_SIZE), order);
            model.addAttribute(MODEL_ATTR_PAGE, page);
        } catch (Exception e) {
            addMessage(model, "错误：" + e.getMessage());
        }
    ...
    public String newAllOrderList(OrderSearchModel order, HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            page = customerOrderService.getNewAllOrderLit(new Page<>(request, response, DEFAULT_PAGE_SIZE), order);
            //判断权限
            Subject subject = SecurityUtils.getSubject();
            if(subject.isPermitted("sd:reminder:create")) {
                List<Order> orders = page.getList();
