...
import com.wolfking.jeesite.ms.joyoung.rpt.fallback.MSJoyoungFailLogRptFeignFallbackFactory;
import org.springframework.cloud.netflix.feign.FeignClient;
import org.springframework.web.bind.annotation.*;

@FeignClient(name = "kklplus-b2b-joyoung", fallbackFactory = MSJoyoungFailLogRptFeignFallbackFactory.class)
public interface MSJoyoungFailLogRptFeign {

        //region 状态数据查询 报表
        @PostMapping("/failedLog/getList")
        MSResponse<MSPage<B2BOrderProcesslog>> getFailLogList(@RequestBody JoyoungSearchModel processlogSearchModel);
