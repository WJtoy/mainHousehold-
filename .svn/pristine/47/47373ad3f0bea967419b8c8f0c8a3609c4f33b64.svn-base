		...
		function returnaction(btn){
            ...
            top.layer.confirm(
                ...
                ,function(index){
                    ...
                    // do something
                    var loadingIndex;
                    var data = {ids:ids.join(","),action:action};
                    $.ajax({
                        async: false,
                        cache: false,
                        type: "POST",
...
//		$(document).on("click", "#btnReject", returnaction);
  ...
  <body>
	...
	<form:form id="searchForm" modelAttribute="order"  action="${ctx}/sd/order/customer/return/approvelist" method="post" class="breadcrumb form-search">
		...
		<div>
			...
			<label>订单号：</label><input type=text class="input-small" id="orderNo" name="orderNo" style="width: 90px;" value="${order.orderNo}" maxlength="20" />
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-hover">
		...
		<tbody>
			...
			<c:forEach items="${page.list}" var="order" varStatus="i" begin="0">
				<tr>
					<td><input type="checkbox" id="cbox${i.index}" value="${order.id}" name="checkedRecords"/></td>
					<td>${i.index+1}</td>
