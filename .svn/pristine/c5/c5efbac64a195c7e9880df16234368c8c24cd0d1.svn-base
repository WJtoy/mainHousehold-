	...
	<script type="text/javascript">
		$(document).ready(function() {
            ...
            $("#btnSubmit").on("click", function(){
                ...
                var url = "${ctx}/fi/customercurrency/blockamountlist";
...
<sys:message content="${message}"/>
	<ul class="nav nav-tabs">
		<li class="active"><a href="javascript:void(0);">冻结查询</a></li>
		<li><a href="${ctx}/fi/customercurrency/list">余额查询</a></li>
		<shiro:hasPermission name="fi:customercurrency:charge"><li><a href="${ctx}/fi/customercurrency/form">充值</a></li></shiro:hasPermission>
	</ul>
	<c:set var="currentuser" value="${fns:getUser() }" />
	<form:form id="searchForm" modelAttribute="searchEntity" action="${ctx}/fi/customercurrency/blockamountlist" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<form:hidden path="firstSearch" />
		<div>
			<%--
			<c:if test="${not currentuser.isCustomer()}">
				<label class="control-label">客&nbsp;&nbsp;户：</label>
				<select id="customer" name="customer.id" class="input-small" style="width:250px;">
					<option value="" <c:out value="${(empty searchEntity.customer.id)?'selected=selected':''}" />>所有</option>
					<c:forEach items="${fns:getMyCustomerListFromMS()}" var="dict">
						<option value="${dict.id}" <c:out value="${(searchEntity.customer.id eq dict.id)?'selected=selected':''}" />>${dict.name}</option>
					</c:forEach>
				</select>
			</c:if>
			--%>
		...
		<c:forEach items="${page.list}" var="item">
			...
			<c:if test="${!(item.customer.id eq customerId) }">
				...
				<tr id="${item.customer.id}" pId="">
					<%--<td colspan="2">${item.customer.name}(${item.customer.finance.paymentType.label})</td>--%>
					<td colspan="2">${item.customer.name}(${item.customer.paymentType.label})</td>
					<td colspan="4">${fns:formatNum(item.customerFinance.blockAmount)}</td>
					<td></td>
				</tr>
			</c:if>
			<tr id="${item.id}" pId="${customerId}">
				<td>${index+(page.pageNo-1)*page.pageSize}</td>
				<%--<td>${fns:getDictLabelFromMS(item.currencyType, 'BlockCurrencyType', '')}</td>--%>
