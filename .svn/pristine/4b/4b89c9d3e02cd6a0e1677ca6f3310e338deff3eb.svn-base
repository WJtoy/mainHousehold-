...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).on("click", "#btnClearSearch", function () {
            ...
            $("#completeBegin").val(startDate);
            $("#completeEnd").val(endDate);
            $("#endDate").val(endDate);
...
<ul id="navtabs" class="nav nav-tabs">
    <li><a href="${ctx}/sd/order/salesNew/materialList" data-toggle="tooltipnorth" data-tooltip="要发配件的订单">待发配件</a></li>
    <li><a href="${ctx}/sd/order/salesNew/list" data-toggle="tooltipnorth" data-tooltip="&nbsp;&nbsp;处理中订单列表">处理中</a></li>
    <li><a href="${ctx}/sd/order/salesNew/finishlist" data-toggle="tooltipnorth" data-tooltip="已完成订单列表">已完成</a></li>
<%--    <li><a href="${ctx}/sd/order/salesNew/cancellist" data-toggle="tooltipnorth" data-tooltip="取消单列表">取消单</a></li>--%>
<%--    <li><a href="${ctx}/sd/order/salesNew/returnlist" data-toggle="tooltipnorth" data-tooltip="退单列表">退单</a></li>--%>
    <li><a href="${ctx}/sd/order/salesNew/alllist" data-toggle="tooltipnorth" data-tooltip="所有订单列表">所有</a></li>
    <li><a href="${ctx}/sd/order/salesNew/complainlist" data-toggle="tooltipnorth" data-tooltip="投诉列表">投诉</a></li>
    <li class="active"><a href="javascript:;" data-toggle="tooltipnorth" data-tooltip="催单订单列表">催单</a></li>
</ul>
<c:set var="currentuser" value="${fns:getUser() }"/>
<form:form id="searchForm" modelAttribute="order" action="${ctx}/sd/order/salesNew/reminderlist" method="post"
    ...
    <div style="margin-top:5px">
        ...
        <label class="label-search">催单日期 ：</label>
    ...
    <table id="contentTable" class="table table-bordered table-condensed table-striped"
        ...
        <c:forEach items="${page.list}" var="o">
            ...
            <tr>
                ...
                <td>
                    <c:if test="${o.orderCondition.replyFlagKefu == 1 }">
                        <a href="javascript:void(0);" style="color: red;"
                           onclick="SalesOrderService.viewOrderDetail('${o.id}','${order.quarter}');"><abbr
                    ...
                    <c:if test="${o.orderStatus.reminderStatus >0}">
                        ...
                        <c:set var="reminderLabel" value="${fns:formatDateString(o.orderStatus.reminderCreateAt)}"/>
                        <c:choose>
                ...
                <td>${o.orderCondition.customerOwner}${not empty o.parentBizOrderId?'<br>'.concat(o.parentBizOrderId):''}
                </td>
...
<%--                               value="${o.orderCondition.partsFlag == 0 && (fn:containsIgnoreCase('10,20,30,40',o.orderCondition.status.value) || (o.orderCondition.status.value == '50' && o.orderCondition.serviceTimes == 0))}"/>--%>
                        ...
                        <c:if test="${canEdit || o.canSaleCanceled()}">
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu">
                                    <c:if test="${canEdit}">
                                        <shiro:hasPermission name="sd:order:edit">
                                            <li><a href="${ctx}/sd/order/edit?id=${o.id}&actiontype=sales_complain"><i class="icon-pencil"></i>修改</a>
                                            </li>
                                        </shiro:hasPermission>
                                    </c:if>
                                    <c:if test="${ o.canSaleCanceled() }">
                ...
                <td>
                    <a class="btn btn-mini btn-warning" href="javascript:void(0);"
                       onclick="Order.reminderView('','${o.id}','${o.quarter}');">查看</a>
                </td>
                <td>
                    <c:if test="${o.orderStatus.reminderStatus>0}">
                        <c:set var="cutoffClass" value="normal"/>
                        <c:choose>
                            <c:when test="${o.remiderCutOffTimeliness>1}">
                                <c:set var="cutoffClass" value="timeout"/>
                            </c:when>
                            <c:when test="${o.remiderCutOffTimeliness>0.67 and o.orderStatus.reminderStatus==1}">
                                <c:set var="cutoffClass" value="intime"/>
                            </c:when>
                        </c:choose>
                        <c:choose>
                            <c:when test="${o.orderStatus.reminderStatus==1}">
                                <p class="reminder_${cutoffClass}">${o.reminderCutOffLabel}</p>
                            </c:when>
                            <c:otherwise>
                                <p class="reminder_${cutoffClass}">用时：${o.reminderCutOffLabel}</p>
                            </c:otherwise>
                        </c:choose>
                    </c:if>
                </td>
...
<script type="text/javascript">
    $(document).ready(function () {
        customerLimitDatePicker('beginDate','endDate',6,true);
        customerLimitDatePicker('completeBegin','completeEnd',6);
