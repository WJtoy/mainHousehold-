...
public class MSProductCategoryNewFeignFallbackFactory implements FallbackFactory<MSProductCategoryNewFeign> {
    ...
    public MSProductCategoryNewFeign create(Throwable throwable) {
        return new MSProductCategoryNewFeign() {
            ...
            @Override
            public MSResponse<List<NameValuePair<Long, String>>> findIdAndNameListByCustomerId(Long customerId) {
                return new MSResponse<>(MSErrorCode.FALLBACK_FAILURE);
            }

            @Override
            public MSResponse<List<NameValuePair<Long, String>>> findIdAndNameListByCustomerIdForMD(Long customerId) {
                return new MSResponse<>(MSErrorCode.FALLBACK_FAILURE);
            }
        };
    }
