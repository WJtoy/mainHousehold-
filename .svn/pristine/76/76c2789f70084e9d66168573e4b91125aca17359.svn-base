...
import com.wolfking.jeesite.ms.b2bcenter.mq.config.B2BCenterNewB2BOrderReminderRetryMQConfig;
...
public class B2BCenterNewB2BOrderReminderMQSender implements RabbitTemplate.ConfirmCallback {
     ...
     * @param times   第几次发送
     */
    public void sendRetry(MQB2BOrderReminderMessage.B2BOrderReminderMessage message, int times) {
        try {
            retryTemplate.execute((RetryCallback<Object, Exception>) context -> {
                context.setAttribute(B2BMQConstant.RETRY_CONTEXT_ATTRIBUTE_KEY_MESSAGE, message);
                rabbitTemplate.convertAndSend(
                        B2BMQConstant.MQ_B2BCENTER_NEW_B2BORDER_REMINDER_RETRY,
                        B2BMQConstant.MQ_B2BCENTER_NEW_B2BORDER_REMINDER_RETRY,
                        message.toByteArray(), msg -> {
                            msg.getMessageProperties().setDelay(B2BCenterNewB2BOrderReminderRetryMQConfig.DELAY_MILLISECOND * times);
                ...
                Throwable throwable = context.getLastThrowable();
                String msgJson = new JsonFormat().printToString(msg);
                B2BFailureLogUtils.saveFailureLog(B2BMQQueueType.B2BCENTER_NEW_B2BORDER_REMINDER_RETRY, msgJson, 0L,
                        B2BProcessFlag.PROCESS_FLAG_FAILURE, 0,
