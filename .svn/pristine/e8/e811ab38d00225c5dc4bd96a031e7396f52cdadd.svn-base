...
//				pattern).parse(strDate);
     ...
     * @return
     */
    public static Float getDateDiffHourOfNow(Date startDate) {
        float nd = 1000f * 60 * 60;//一天的毫秒数
     ...
     * @return
     */
    public static double differTime(long beginDate,long endDate){
     ...
     * @return
     */
    public static double calculateTimeliness(Date date,long timeOut,long beginDt,long endDt){
        double timeDiffer = 0.0;
        if(timeOut > date.getTime()){//到期时间大于当前时间
            boolean isSameDate = DateUtils.isSameDay(date,new Date(timeOut));
            boolean isEffectiveDate = isEffectiveDate(date.getTime(),9,18); //判断前期时间是不否在当前9点到18点之间
            if(isSameDate){ //如果是同一天
                if(isEffectiveDate){ //在工作时间内当天9点～18点
                    timeDiffer = DateUtils.dateDiff(timeOut,date.getTime());
                }else { //如果非工作时间内,到期时间减去当天9点
                    timeDiffer = DateUtils.dateDiff(timeOut,beginDt);
                }
            }else { //如果到期时间在第二天
                if(isEffectiveDate){ //在工作时间内 当天18点-现在时间+到期时间-第二天9点
                   /* long longTimeDiffer = endDt-date.getTime()+timeOut-beginDt-(24*60*60*1000);
                    timeDiffer =(double)longTimeDiffer/(60*60*1000);
                    timeDiffer = BigDecimal.valueOf(timeDiffer).setScale(2, RoundingMode.HALF_UP).doubleValue();*/
                    timeDiffer = DateUtils.dateDiff(timeOut,date.getTime());
                }else{ //到期时间-第二天9点
                    timeDiffer = DateUtils.dateDiff(timeOut,beginDt+(24*60*60*1000));
                }
            }
        }else{
            timeDiffer = dateDiff(timeOut,date.getTime());
        }
        return timeDiffer;
    }

    /**
     * 判断当前时间是否在工作时间内(9点-18点)
     * @param dataMillis 当前时间
     * @return
     */
    public static boolean isEffectiveDate(long dataMillis,int firstHour,int secondHour) {
        TwoTuple<Long,Long> twoTuple = getTwoTupleDate(firstHour,secondHour);
        if(twoTuple.getAElement()<= dataMillis && dataMillis<= twoTuple.getBElement()){
            return true;
        }else{
            return false;
        }
    ...
    /**
     * 返回当天9点和18点的时间错个连个时间点
     * @param
     * @return
     */
    public static TwoTuple<Long,Long> getTwoTupleDate(int firstHour,int secondHour){
        Calendar c = Calendar.getInstance();
        ...
        TwoTuple<Long,Long> twoTuple = new TwoTuple<Long, Long>(startTime.getTime(),endTime.getTime());
        return twoTuple;
    }
     ...
     * 计算时效(小时)
     * 工作时间：9:00~18:00
     * @param completeAt 完成时间
     * @param createDt  创建时间
     */
    public static double dateDiff(long completeAt,long createDt){
        Calendar completeCalendar = Calendar.getInstance();
        completeCalendar.setTime(new Date(completeAt));
        Calendar createCalendar = Calendar.getInstance();
        createCalendar.setTime(new Date(createDt));
        completeCalendar.set(Calendar.MILLISECOND,0);
        int completeTime = completeCalendar.get(Calendar.HOUR_OF_DAY);
        if(completeTime >= 18 || completeTime < 9) {
            if (completeTime >= 18) {
                completeCalendar.set(Calendar.HOUR_OF_DAY, 18);
            } else if (completeTime < 9) {
                completeCalendar.set(Calendar.HOUR_OF_DAY, 9);
            }
            completeCalendar.set(Calendar.MINUTE,0);
            completeCalendar.set(Calendar.SECOND,0);
        }
        createCalendar.set(Calendar.MILLISECOND, 0);
        int createTime = createCalendar.get(Calendar.HOUR_OF_DAY);
        if(createTime >= 18 || createTime < 9) {
            if (createTime >= 18) {
                createCalendar.set(Calendar.HOUR_OF_DAY, 18);
            } else if (createTime < 9) {
                createCalendar.set(Calendar.HOUR_OF_DAY, 9);
            }
            createCalendar.set(Calendar.MINUTE, 0);
            createCalendar.set(Calendar.SECOND, 0);

        }
        int completeDay = completeCalendar.get(Calendar.DAY_OF_MONTH);
        int createDay = createCalendar.get(Calendar.DAY_OF_MONTH);
        double orderTimeLiness = (double)(completeCalendar.getTimeInMillis()-createCalendar.getTimeInMillis())/(60*60*1000);
        if(completeDay == createDay && orderTimeLiness < 24){
            return orderTimeLiness;
        }else{
            //完成时间与创建时间的时分秒时间对比，是否完成小于创建
            boolean isCompare = false;
            if(completeTime < createTime){
                isCompare = true;
            }else if(completeTime == createTime){
                int completeMinute = completeCalendar.get(Calendar.MINUTE);
                int createDateMinute = createCalendar.get(Calendar.MINUTE);
                if(completeMinute < createDateMinute){
                    isCompare = true;
                }else if(completeMinute == createDateMinute) {
                    int completeSecond = completeCalendar.get(Calendar.SECOND);
                    int createDateSecond = createCalendar.get(Calendar.SECOND);
                    if (completeSecond < createDateSecond) {
                        isCompare = true;
                    }
                }
            }
            if(completeTime < 9 || createTime >= 18 || isCompare){
                long day = (long)Math.ceil(orderTimeLiness/24);
                return orderTimeLiness - (day * 15);
            }else{
                long day = (long)Math.floor(orderTimeLiness/24);
                return orderTimeLiness - (day * 15);
            }
        }
    }
