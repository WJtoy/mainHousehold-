		...
		return this.each (function () {
			...
			if (cfg.fixedCols > 0) {
				...
				lc.ft_c			
					...
					.width(lc.ft_rc.outerWidth(true) - 1);//ryan old:+ 1
			}		
			...
			//remove td from span row ryan
			if(lc.ft_c != null){
				var frowidx=-1;
				var rowspan = 1;
				var ht =0;
				for (var i = 0; i < lc.ft_c[0].rows.length; i++) {
					var crow = lc.ft_c[0].rows[i];
					if($(crow).hasClass("item")){
						rowspan++;
						if(frowidx == -1){
							frowidx = i-1;
							ht=  $(lc.ft_c[0].rows[frowidx]).height();
						}
						//var h = $(lc.ft_c[0].rows[frowidx]).height();
						//$(lc.ft_c[0].rows[frowidx]).height(h*rowspan);
						ht = ht + $(lc.ft_c[0].rows[i]).height();
						$(lc.ft_c[0].rows[frowidx]).height(ht);
						$(crow).height(0);
						$.each ($(crow).find("td"), function (idx, td) {
							$(td).remove();
						});
						//$($($this)[0].rows[i]).height(h);//与订单第一条高度相同
