    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#btnSubmit").click(function() {
                ...
                $("#searchForm").attr("action","${ctx}/rpt/provider/customerReceivableSummary/customerReceivableSummaryRptNewer");
                $("#searchForm").submit();
            });
            ...
            $("#btnExport").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "${ctx}/rpt/provider/customerReceivableSummary/checkExportTask?"+ (new Date()).getTime(),
                        ...
                        if(data && data.success == true){
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/rpt/provider/customerReceivableSummary/export?"+ (new Date()).getTime(),
                                        data:$(searchForm).serialize(),
                                        success: function (data) {
                                            if(ajaxLogout(data)){
                                                return false;
...
<form:form id="searchForm" modelAttribute="rptSearchCondition" action="${ctx}/rpt/provider/customerReceivableSummary/customerReceivableSummaryRptNewer" method="post" class="breadcrumb form-search"  >
