...
<head>
	...
	<script type="text/javascript">
        ...
        function page(n,s){
            $("#pageNo").val(n);
            $("#pageSize").val(s);
            $("#searchForm").attr("action","${ctx}/fi/servicepointinvoice/confirm");
            $("#searchForm").submit();
            return false;
        }
        function repage(){
	...
	//                        $("#s2id_bank").find("span.select2-chosen").html('请选择');
		...
		$(document).on("click", "#btnExport", function () {
			...
			if ($("#bank").val()==null || $("#bank").val()==""){
				...
				if(v=="ok"){				
					$("#searchForm").attr("action","${ctx}/fi/servicepointinvoice/confirm/export");
					$("#searchForm").submit();
					$("#searchForm").attr("action","${ctx}/fi/servicepointinvoice/confirm");
				}
  ...
  <body>
	...
	<form:form id="searchForm" action="${ctx}/fi/servicepointinvoice/confirm" method="post" class="breadcrumb form-search">
