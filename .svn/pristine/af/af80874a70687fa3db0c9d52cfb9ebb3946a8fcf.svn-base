	...
	<script type="text/javascript">
		...
		$(document).on("click", "#btnSubmit", function () {
			if (clicktag == 0) {
				...
				beforePage();
				setPage();
				$("#searchForm").attr("action","${ctx}/sd/material/customer/applylist");
				$("#searchForm").submit();
			}
		});
		...
		$(document).on("click", "#btnExport", function () {
			...
			layer.confirm('确定导出配件列表吗?', {icon: 3, title: '系统确认'}, function (index, layero) {
                ...
                jQuery('<form id="exportForm" action="${ctx}/sd/material/customer/applylist/export" method="post" style="display:none;">'+ html + '</form>')
...
<form:form id="searchForm" modelAttribute="searchModel" method="post" action="${ctx}/sd/material/customer/applylist" class="breadcrumb form-search">
