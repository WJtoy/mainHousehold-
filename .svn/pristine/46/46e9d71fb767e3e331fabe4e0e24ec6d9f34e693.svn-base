...
<head>
	...
	<style type="text/css">
		...
		.fromInput {
			border:1px solid #ccc;padding:4px 6px;color:#555;border-radius:4px;margin-top: 5px;
		}
...
<ul class="nav nav-tabs">
	<li>
		<a href="${ctx}/provider/md/customerBrand/getList">列表</a>
	</li>
	<li class="active">
		<a href="javascript:void(0);">${not empty customerProductModel.id?'修改':'添加'}</a>
	</li>
</ul><br>
<form:form id="inputForm" modelAttribute="customerBrand" action="${ctx}/provider/md/customerBrand/save" method="post" class="form-horizontal">
	...
	<c:if test="${canAction == true}">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input class="fromInput required" name="brandName" value="${customerBrand.brandName}" htmlEscape="false" maxlength="45"/>
				<span class="add-on red">*</span>
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:textarea path="remarks" htmlEscape="false" rows="3" maxlength="100" class="input-xlarge" cssStyle="min-width: 280px;max-width: 560px;min-height: 70px;max-height: 210px;"/>
			</div>
		</div>
	<div class="control-group">
