    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#btnSubmit").click(function() {
                ...
                $("#searchForm").attr("action","${ctx}/finance/rpt/financialReview/financialReviewDetailsRptData");
                $("#searchForm").submit();
            });
            ...
            $("#btnExport").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "${ctx}/finance/rpt/financialReview/checkExportTask?"+ (new Date()).getTime(),
                        ...
                        if(data && data.success == true){
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/finance/rpt/financialReview/export?"+ (new Date()).getTime(),
                                        data:$(searchForm).serialize(),
                                        success: function (data) {
                                            if(ajaxLogout(data)){
                                                return false;
...
<ul class="nav nav-tabs">
    <li>
        <a href="${ctx}/finance/rpt/abnormalFinancialReview/abnormalFinancialReviewRptData">财务每日审单</a>
    </li>
    <li class="active"><a href="javascript:void(0);">财务审单明细</a></li>
</ul>
<form:form id="searchForm" modelAttribute="rptSearchCondition" action="${ctx}/finance/rpt/financialReview/financialReviewDetailsRptData" method="post" class="breadcrumb form-search">
    ...
    <div>
        ...
        <input id="endDate" name="endDate" type="text" readonly="readonly" style="width:98px" maxlength="20"
               ...
               onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
        &nbsp;&nbsp;
        <shiro:hasPermission name="rpt:finance:abnormalFinancialReview:view"><input id="btnSubmit" class="btn btn-primary" type="submit" value="查询" /></shiro:hasPermission>
        &nbsp;&nbsp;
