    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#btnSubmit").click(function() {
                ...
                $("#searchForm").attr("action","${ctx}/md/sysNotice/customerNoticeList");
                $("#searchForm").submit();
            });
        });

        function details(title,subtitle,content){
            top.layer.open({
                type: 2,
                id:'layer_unitCode',
                zIndex:19891015,
                title:'通知详情',
                content: "${ctx}/md/sysNotice/findDetails?title="+ (title || '') + "&subtitle=" + (subtitle || '') + "&content=" + (content || ''),
                area: ['830px', '380px'],
                shade: 0.3,
                maxmin: false,
...
<ul class="nav nav-tabs">
    <li class="active"><a href="javascript:void(0);">客户公告</a></li>
</ul>
<form:form id="searchForm" modelAttribute="systemNotice" action="${ctx}/md/sysNotice/customerNoticeList" method="post" class="breadcrumb form-search">
    <div>
        <input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
        <input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
        <label>标题：</label>
            <%-- <input id="title" class="input-small" name="title" value="${servicePointNotice.title}" maxlength="20">--%>
        <form:input path="title" htmlEscape="false" maxlength="30" class="input-small"/>
        &nbsp; &nbsp;
        <label>发布时间：</label>
            ...
            <tr>
                ...
                <td>
                    <a href="javascript:void(0);" class="btn btn-mini btn-warning"
                       onclick="javascript:details('${item.title}','${item.subtitle}','${item.content}')">查看</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
