package com.wolfking.jeesite.ms.jinran.sd.fallback;
...
@Slf4j
@Component
public class JinRanOrderFeignFallbackFactory implements FallbackFactory<JinRanOrderFeign> {

    @Override
    public JinRanOrderFeign create(Throwable throwable) {
        if (throwable != null) {
            log.error("JinRanOrderFeignFallbackFactory:{}", throwable.getMessage());
        }
        return new JinRanOrderFeign() {
            @Override
