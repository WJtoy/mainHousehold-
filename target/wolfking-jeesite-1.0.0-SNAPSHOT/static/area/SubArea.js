...
var SubAreaUtils = {
    ...
    open: function (obj, event,areaId) {
        var self = this;
        self.callback = null;
        //event.stopPropagation();
        if ($(".provinceCityAll", self.document).length == 0) {
            var areahtml = '<div class="tabs clearfix">' +
                '<ul class=""><li><a href="javascript:" tb="townAll">街道</a></li></ul>' +
                '</div>' +
                '<div class="con"><div class="townAll invis"><div class="list"><ul></ul></div></div></div>';
        ...
        if ($(".provinceCityAll", self.document).find(".tabs").find("a.current").length == 0) {
            ...
            $(".provinceCityAll", self.document).find(".tabs").find("a[tb=townAll]").addClass("current");
        ...
        if (self.provinces == null || self.provinces.length == 0) {
            self.loadProvinces(areaId);
        } else if ($(".townAll .list ul li", self.document).length == 0) {
            self.showProvince(areaId);
        }
    },
    opencallback: function (obj, event, callback,areaId) {
        var self = this;
    ...
    //装载省
    loadProvinces: function (countyId) {
    ...
    showProvince: function () {
      ...
      /*      if (self.provinces[i].name == '内蒙古自治区') {
            ...
            var p_name = self.provinces[i].name.substr(0, 4);
    ...
    initComponent: function (document,areaId) {
        ...
        if (this.provinces == null || this.provinces.length == 0) {
            this.loadProvinces(areaId);
        }
    }
};
window.SubAreaUtils = top.window.SubAreaUtils || parent.window.SubAreaUtils || SubAreaUtils;
window.SubAreaUtils.init();