...
<head>
	...
	<%@ include file="/WEB-INF/views/modules/sales/sd/feedback/tpl/replayContent.html" %>
						...
						<tr>
							...
							<td width="260px" align="left" style="vertical-align: middle;">
								<a href="javascript:;" id="btnSend" class="btn btn-success"
								   onclick="SalesOrderService.addFeedbackReply('${feedback.id}','${feedback.quarter}');"><b>回复</b>
								</a>
								<c:if test="${handleFeedback == true}">
									<a href="javascript:;" id="btnHandled" class="btn btn-success"
									   onclick="SalesOrderService.handled('${feedback.order.id}','${feedback.quarter}');" title="点击此按钮，确认异常处理完成"><b>已处理</b>
									</a>
								</c:if>
								<c:if test="${feedback.attachmentCount<5}">
										...
										<a id="btnAttach" href="javascript:void(0);"
										   class="btn btn-success" onclick="SalesOrderService.addReplyAttach('${feedback.id}','${feedback.quarter}',this_index)">
											<b>上传附件</b>
										</a>
									</shiro:hasPermission>
								</c:if>
...
<script type="text/javascript">
    ...
    {
        SalesOrderService.addReplyItem(data);
...
<script type="text/javascript">
    SalesOrderService.readReply('${feedback.id}','${feedback.quarter}');
</script>
</c:if>
</html>