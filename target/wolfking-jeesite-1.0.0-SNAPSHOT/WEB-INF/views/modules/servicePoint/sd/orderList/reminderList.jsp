...
<form:form id="searchForm" modelAttribute="order" action="${ctx}/servicePoint/sd/orderList/reminderList" method="post" class="breadcrumb form-search">
        ...
        <tr>
                ...
                <%--<c:if test="${order.orderStatus.reminderStatus == 1}">
                    <c:choose>
                        <c:when test="${order.orderCondition.engineer.id == currentuser.engineerId}">
                            <br/><a href="javascript:void(0);" class="btn btn-mini" onclick="ServicePointOrderService.servicePointReplyReminder('${order.reminderId}','${order.quarter}');">回复</a>
                        </c:when>
                        <c:otherwise>
                            <br/><a href="javascript:void(0);" class="btn btn-mini" onclick="ServicePointOrderService.servicePointViewReminder('${order.reminderId}','','${order.quarter}');">查看</a>
                        </c:otherwise>
                    </c:choose>
                </c:if>--%>
            </td>
                    ...
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu">
                            <c:if test="${ order.canPlanOrder()}">
                                <li>
                                    <a href="javascript:void(0);" onclick="ServicePointOrderService.servicePointPlan('${order.id}','${order.orderNo}','${order.quarter}');">
                                    <i class="icon-truck"></i>派单</a>
                                </li>
                            </c:if>
                            <li><a href="javascript:void(0);" onclick="ServicePointOrderService.servicePointPending('${order.id}','${order.orderNo}','${order.quarter}',${order.orderCondition.reservationTimes});"><i
            ...
            <td>
                <c:if test="${order.orderStatus.reminderStatus == 1}">
                       <c:choose>
                           <c:when test="${order.orderCondition.engineer.id == currentuser.engineerId}">
                               <a href="javascript:void(0);" class="btn btn-mini btn-warning" onclick="ServicePointOrderService.servicePointReplyReminder('${order.reminderId}','${order.quarter}');">回复</a>
                           </c:when>
                           <c:otherwise>
                               <a href="javascript:void(0);" class="btn btn-mini btn-warning" onclick="ServicePointOrderService.servicePointViewReminder('${order.reminderId}','','${order.quarter}');">查看</a>
                           </c:otherwise>
                       </c:choose>
                   </c:if>
            </td>
            <td>
                ...
                <c:if test="${ order.canTracking()}">
                    <a href="javascript:void(0);" onclick="ServicePointOrderService.servicePointTracking('${order.id}','${order.quarter}');"
