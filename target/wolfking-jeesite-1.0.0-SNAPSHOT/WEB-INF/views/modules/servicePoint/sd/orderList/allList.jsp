...
<form:form id="searchForm" modelAttribute="order" action="${ctx}/servicePoint/sd/orderList/allList" method="post" class="breadcrumb form-search">
...
<%--                        <c:if test="${order.orderCondition.status.intValue >=20 and order.orderCondition.status.intValue <= 50 and order.orderCondition.subStatus >= 10 and order.orderCondition.subStatus <=40 and order.isAllowedForSetAppointment() == 1}">--%>
                        ...
                        <li>
                            <a href="javascript:void(0);" onclick="ServicePointOrderService.servicePointPending('${order.id}','${order.orderNo}','${order.quarter}',${order.orderCondition.reservationTimes});"><i
                                    class="icon-calendar"></i>&nbsp;预约时间</a>
                        </li>
                        </c:if>
                        <c:if test="${order.orderCondition.status.value eq '40'}">
                            <li>
                                <a href="javascript:void(0);" onclick="ServicePointOrderService.servicePointConfirmDoorAuto('${order.id}','${order.quarter}',undefined);"><i
                                        class="icon-home"></i>&nbsp;确认上门</a>
                            </li>
                        </c:if>
                        <c:if test="${order.orderCondition.status.value eq '50' and order.orderCondition.subStatus ne 70 }">
                            <li>
                                <a href="javascript:void(0);" onclick="ServicePointOrderService.servicePointComplete('${order.id}','${order.orderNo}','${order.quarter}');"><i
            ...
            <td>
                <c:choose>
                    <c:when test="${order.orderCondition.status.intValue < 80 && order.isServicePointCompleted() == 0}">
