  ...
  <head>
	...
	<script type="text/javascript">
        ...
        function uploadfile(fileInputId,index) {
            ...
            $.ajaxFileUpload({
                ...
                success: function (data, status) {
                    ...
                    /*    var picServicePointFee = $("#pic_info_"+ index).data("servicepointfee");
                        var toldServicePointFee = parseFloat($("#applyServicepointPraiseFee").val()) + parseFloat(picServicePointFee);
                        $("#applyServicepointPraiseFee").val(toldServicePointFee);
                        var picCustomerFee = $("#pic_info_"+ index).data("customerfee");
                        var toldCustomerFee = parseFloat($("#applyCustomerPraiseFee").val()) + parseFloat(picCustomerFee);
                        $("#applyCustomerPraiseFee").val(toldCustomerFee);*/
                        sumPraiseFee()
        ...
        $(document).on("click", "#btnSubmit", function () {
            ...
            var loadingIndex;
            var options = {
                url: "${ctx}/servicePoint/receipt/praise/updatePraise",                 //默认是form的action， 如果申明，则会覆盖
        ...
        function closePraise() {
            top.layer.confirm('取消后不能重新申请好评费,确定取消吗', {icon: 3, title:'系统确认'}, function(index){
                ...
                $.ajax({
                    url:"${ctx}/servicePoint/receipt/praise/cancelled",
                    type:"POST",
                    data:{id:id,quarter:quarter,orderId:orderId,status:status},
                    dataType:"json",
                    success: function(data){
                        //提交后的回调函数
                        if(loadingIndex) {
        ...
        function sumPraiseFee() {
            ...
            servicePointFee = customerFee * (1- discount);
	...
	<c:if test="${canAction==true}">
		...
		<form:form id="inputForm" modelAttribute="praise" action="" method="post" class="form-horizontal">
				...
				<div class="span12">
					<div class="control-group">
						<div class="controls" style="margin-left: 20px;">
							<div class="input-block-level" style=" border:1px solid #FF9502;border-radius:3px;height: 30px; line-height:30px;vertical-align:middle;background-color: #FFF7EC">
								<i class="icon-question-sign" style="color: #FF9502;margin-left: 20px"></i>
								<span style="color: #FF9502">温馨提示：此处只用于上传好评照片，上传错误或虚假照片，考核200元/单；超过三次，取消接单权限。</span>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="row-fluid">
				<div class="span12">
					<div class="control-group">
						<label class="control-label">实例图片：</label>
						<div class="controls" id="examplePic">
							<c:forEach var="examplePic" items="${customerPraiseFee.examplePicItems}" varStatus="picStatus">
								<div class="upload_warp_img_div">
									...
									<div style="position:absolute;width:100px;z-indent:2;left:0;bottom:0;color: white;background-color: #333333;opacity: 0.6;">${examplePic.name}</div>
								</div>
							</c:forEach>
						</div>
					</div>
						...
						<div class="controls">
							...
							<div>
								...
								<span style="color: red;">支持jpg,jpeg,png格式,且大小不能超过2MB,建议尺寸600*800</span><br/>
									<%--<div id="exampleDiv">
										<c:forEach var="examplePic" items="${customerPraiseFee.examplePicItems}" varStatus="picStatus">
											<img id="first${picStatus.index}" src="${ctxStatic}${examplePic.url}" data-original="${ctxStatic}${examplePic.url}" style="display:none;">
										</c:forEach>
									</div>--%>
			...
			<div class="row-fluid">
				<div class="span6">
					<div class="control-group">
						...
						<div class="controls">
							<form:input  path="applyServicepointPraiseFee" readonly="true" cssClass="input-block-level"></form:input>
						</div>
					</div>
				</div>
			</div>
			<c:if test="${praise.status!=10 && praise.status !=20}">
				...
				<div class="row-fluid">
					<div class="span12">
						<div class="control-group">
							...
							<div class="controls">
								<form:textarea path="remarks" htmlEscape="false" rows="5" maxlength="255" cssClass="input-block-level" cssStyle="height:56px" disabled="true"/><br/>
							</div>
						</div>
					</div>
				</div>
		...
		<shiro:hasPermission name="sd:servicepointpraise:edit">
			<div style="position: fixed;bottom: 0; width: 100%;height: 60px;background-color: white">
				...
				<c:choose>
					<c:when test="${praise.status==30}">
						<input id="btnSubmit" class="btn btn-primary" type="submit" value="修 改" style="margin-left: 595px;margin-top: 10px;width: 82px"/>
				    ...
				    <c:otherwise>
						<input id="btnCancel" class="btn" type="button" value="关 闭" style="margin-top:10px;margin-left: 800px;"onclick="cancel()"/>
					</c:otherwise>
				</c:choose>
			</div>
		</shiro:hasPermission>
