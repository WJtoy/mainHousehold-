...
<head>
	...
	<script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#inputForm").validate({
                rules: {
                    loginName: {remote: "${ctx}/md/customer/account/checkLoginName?oldLoginName=" + encodeURIComponent('${!empty user.id?user.loginName:''}') +"&expectId=${empty user.id?'0':'' + user.id}"}
					,mobile: {remote:{
							type: "post",
...
//                  var  customerName=$("[name='customerAccountProfile.customer.id']").find("option:selected").text();
//                  $("#customerName").val(customerName);
//                  $("#name").val(customerName);
                }
            });

			/*$("[name='customerAccountProfile.customer.id']").change(function(){
			 if($("#userType").val()==4){
			 var  customerName=$("[name='customerAccountProfile.customer.id']").find("option:selected").text();
			 $("#customerName").val(customerName);
			 $("#name").val(customerName);
...
<ul class="nav nav-tabs">
	<li><a href="${ctx}/md/customer/account/list">客户账号列表</a></li>
...
<form:form id="inputForm" modelAttribute="user" action="${ctx}/md/customer/account/save" method="post" class="form-horizontal">
	<form:hidden path="id"/>
		...
		<div class="controls">
			<c:choose>
				...
				<c:otherwise>
					...
					<%--<form:select path="customerAccountProfile.customer.id" class="required input-xxlarge"  disabled="${currentuser.userType==3?'true':'false'}" >--%>
					<%--<form:option value="" label="请选择"/>--%>
					<%--<form:options items="${fns:getCustomerList()}" itemLabel="name" itemValue="id" htmlEscape="false" />--%>
					<%--</form:select>--%>
					<%--<input id="customerName" name="customerAccountProfile.customer.name" type="hidden" value="${user.customerAccountProfile.customer.name}" />--%>
					<span class="red">*</span>
				</c:otherwise>
			</c:choose>
		</div>
