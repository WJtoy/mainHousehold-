...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#btnApprove").click(function () {
                top.$.jBox.confirm("确认保存并通过？", "系统提示", function (v, h, f) {
                    if (v == "ok") {
                        $("#inputForm").attr("action", "${ctx}/md/customerNew/saveAndApprove");
                        $("#inputForm").submit();
                    }
                }, {buttonsFocus: 1});
            ...
            $("#btnUpdateSales").click(function () {
                top.layer.open({
                    ...
                    zIndex: 19891015,
                    title: '修改',
                    content: '${ctx}/md/customerNew/salesChangeForm?id=' + ($("#id").val() || '') + "&parentIndex=" + (orderdetail_index || ''),
                    shade: 0.3,
                    shadeClose: true,
                    area: ['550px', '350px'],
                    maxmin: false,
            ...
            $("#btnUpdateMerchandiser").click(function () {
                top.layer.open({
                    ...
                    zIndex: 19891016,
                    title: '修改',
                    content: '${ctx}/md/customerNew/merchandiserChangeForm?id=' + ($("#id").val() || '') + "&parentIndex=" + (orderdetail_index || ''),
                    shade: 0.3,
                    shadeClose: true,
                    area: ['550px', '350px'],
                    maxmin: false,
            ...
            $("#inputForm").validate({
                rules: {
                    code: {remote: "${ctx}/md/customerNew/checkLoginName?oldLoginName=" + encodeURIComponent('${customer.code}')},
                    'finance.credit': {min: 0, max: 10000000},
                    'finance.deposit': {min: 0, max: 1000000},
                    phone: {
                        remote: {
                            type: "post",
                            url: "${ctx}/md/customerNew/checkPhone",
                            data: {
                                id: function () {
                                    return $("#userId").val();
                                },
                    ...
                    name: {
                        remote: "${ctx}/md/customerNew/checkCustomerName?customerId=" + customerId
                    }
                },
                messages: {
                    code: {remote: "客户已存在"},
        ...
        function editCustomerAddresses(customerId, addressType,addressId,editType) {
            var text = "修改地址";
            var url = "${ctx}/md/customerNew/customerAddressForms?customerId=" + customerId + "&addressType=" + addressType +"&id="+ addressId +"&editType=" + editType + "&parentIndex=" + (orderdetail_index || '');
            var area = ['889px', '454px'];
            if (editType == 10) {
                text = "添加地址";
                url = "${ctx}/md/customerNew/customerAddressForms?customerId=" + customerId + "&addressType=" + addressType +"&editType=" + editType + "&parentIndex=" + (orderdetail_index || '');
                area = ['889px', '454px'];
            }
            top.layer.open({
                type: 2,
        ...
        function updateCustomerAddresses(id,userName, contactInfo,areaId,areaName,address,isDefault,addressType,editType) {
            ...
            var url = "${ctx}/md/customerNew/customerAddressForms?addressType=" + addressType +"&address=" + address +
        ...
        function editCustomerFinance(customerId, financeType) {
            var text = "修改账户";
            var url = "${ctx}/md/customerNew/customerFinanceForms?customerId=" + customerId + "&financeType=" + financeType + "&parentIndex=" + (orderdetail_index || '');
            var area = ['642px', '415px'];
            if (customerId == null) {
                text = "添加账户";
                url = "${ctx}/md/customerNew/customerFinanceForms?financeType=" + financeType + "&parentIndex=" + (orderdetail_index || '');
                area = ['642px', '415px'];
            }
            top.layer.open({
                type: 2,
        ...
        function deleteCustomerAddress(addressId) {
            top.layer.confirm("确定删除该地址吗?", {icon: 3, title:'系统确认'}, function(index){
                ...
                $.ajax({
                    type: "GET",
                    url: "${ctx}/md/customerNew/deleteCustomerAddress?addressId=" + addressId,
                    success: function (data) {
                        if (data.success){
                            layerMsg("删除成功",true)
                            delRow(addressId);
...
<form:form id="inputForm" modelAttribute="customer" action="${ctx}/md/customerNew/save" method="post"
...
<div class="line-row" style="width: 949px;float: left">
    ...
    <div class="control-group x" style="margin-left: -3px;">
        ...
        <div class="controls">
<%--            <form:input id="phone" path="phone" type="tel" htmlEscape="false" class="${empty customer.id?'input-y required mobile':''}" maxlength="11"--%>
<%--                        style="width:236px;"/>--%>
...
<div class="row-fluid">
    ...
    <div class="control-group x" >
        ...
        <div class="controls">
            ...
            <shiro:hasPermission name="md:customer:vip">
                <c:set var="canEdit" value="true" />
            </shiro:hasPermission>
            <shiro:lacksPermission name="md:customer:vip">
...
<div class="row-fluid">
    ...
    <div class="line-row">
        <div class="control-group x">
            <label class="control-label"><c:if test="${empty customer.id}"><span class="red">*</span></c:if>联系电话:</label>
            <div class="controls">
<%--                <form:input path="projectOwnerPhone" htmlEscape="false" maxlength="16" class="required phone" placeholder="填写项目负责人手机号码"/>--%>
...
<div class="row-fluid">
    ...
    <div class="line-row">
        <div class="control-group x">
            <label class="control-label"><c:if test="${empty customer.id}"><span class="red">*</span></c:if>联系电话:</label>
            <div class="controls">
<%--                <form:input path="serviceOwnerPhone" htmlEscape="false" maxlength="16" class="required phone" placeholder="填写售后负责人手机号码"/>--%>
...
<div class="row-fluid">
    ...
    <div class="line-row">
        <div class="control-group x">
            <label class="control-label">联系电话:</label>
            <div class="controls">
<%--                <form:input path="financeOwnerPhone" htmlEscape="false" maxlength="16" class="phone" placeholder="填写财务负责人手机号码"/>--%>
...
<div class="row-fluid">
    ...
    <div class="line-row">
        <div class="control-group x">
            <label class="control-label">联系电话:</label>
            <div class="controls">
<%--                <form:input path="technologyOwnerPhone" htmlEscape="false" maxlength="16" class="phone" placeholder="填写技术负责人手机号码"/>--%>
...
<%--        $("[name='customizePriceFlag'][value='1']:radio").prop("checked","true");--%>
<%--    </c:if>--%>
