...
<head>
	...
	<script type="text/javascript">
        ...
        function page(n,s){
            $("#pageNo").val(n);
            $("#pageSize").val(s);
            $("#searchForm").attr("action","${ctx}/fi/servicepointwithdraw/pay");
            $("#searchForm").submit();
            return false;
        }
        function repage(){
        ...
        function payServicePoint(servicePointId, servicePointNo, servicePointName, phone1, phone2, paymentType, bank, branch, bankNo, bankOwner, debtsAmount, totalAmount,
								 debtsDesc, bankIssue, invoiceFlag, discountFlag){
			$("#aSave").attr("href", "${ctx}/fi/servicepointwithdraw/paysave?servicePointId="+servicePointId+"&servicePointNo="+encodeURI(encodeURI(servicePointNo))+
				...
				"&bankOwner="+encodeURI(encodeURI(bankOwner))+"&debtsAmount="+debtsAmount+"&totalAmount="+totalAmount+
				"&debtsDesc="+encodeURI(encodeURI(debtsDesc))+"&bankIssue="+bankIssue+"&invoiceFlag="+invoiceFlag+"&discountFlag="+discountFlag+
				"&qYear="+$("#qYear").val()+"&qMonth="+$("#qMonth").val());
			$("#aSave").click();
	    }
  ...
  <body>
	...
	<form:form id="searchForm" action="${ctx}/fi/servicepointwithdraw/pay" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-hover">
		...
		<c:forEach items="${list}" var="servicePoint">
			<tr>
			<%i++;%>
				<td><input type="checkbox" id="cbox<%=i%>" value="${servicePoint.id};${servicePoint.finance.payableAmount};${servicePoint.finance.paymentType.value};${payableYear};${payableMonth};${servicePoint.finance.minusAmount};${servicePoint.finance.deductedAmount};${servicePoint.finance.platformFee}" name="checkedRecords"/></td>
				<td><%=i%></td>
				...
				<td>
					<input type="Button" id="${servicePoint.finance.id}" class="btn btn-mini btn-danger" value="付款"
						   onclick="payServicePoint('${servicePoint.finance.id}', '${servicePoint.servicePointNo}', '${servicePoint.name}',
							...
							'${servicePoint.finance.branch}', '${servicePoint.finance.bankNo}', '${servicePoint.finance.bankOwner}',
		...
		<tr>
			<td style="text-align:right;" colspan="12" ><B>合计</B></td>
