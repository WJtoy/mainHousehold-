...
<script type="text/javascript">
    ...
    $(document).on("click", "#btnSubmit", function () {
        if (clicktag == 0) {
            ...
            beforePage();
            setPage();
            $("#searchForm").attr("action","${ctx}/sd/material/kefu/alllist");
            $("#searchForm").submit();
        }
    });
    ...
    $(document).on("click", "#btnExport", function () {
        ...
        layer.confirm('确定导出配件列表吗?', {icon: 3, title: '系统确认'}, function (index, layero) {
            ...
            jQuery('<form id="exportForm" action="${ctx}/sd/material/kefu/alllist/export" method="post" style="display:none;">'+ html + '</form>')
...
<ul id="navtabs" class="nav nav-tabs">
	...
	<li><a href="${ctx}/sd/material/kefu/rejectlist" title="已驳回配件单">已驳回</a></li>
...
<form:form id="searchForm" modelAttribute="searchModel" method="post" action="${ctx}/sd/material/kefu/alllist" class="form-inline">
			...
			<tr id="tr_${master.id}" data-index="${i.index}">
				...
				<c:choose>
					<c:when test="${master.closeBy.id>0}">
						<td>${master.closeBy.name}</td>
						<td><fmt:formatDate value="${master.closeDate}" pattern="yyyy-MM-dd HH:mm"/> </td>
						<td><a href="javascript:" data-toggle="tooltip"  data-tooltip=" ${master.closeRemark}">${fns:abbr(master.closeRemark,40)}</a></td>
					</c:when>
					<c:otherwise>
						<td></td>
						<td></td>
