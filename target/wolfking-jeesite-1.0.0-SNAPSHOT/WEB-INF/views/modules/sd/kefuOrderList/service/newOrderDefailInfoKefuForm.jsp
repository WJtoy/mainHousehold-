...
<head>
    ...
    <%@ include file="/WEB-INF/views/modules/sd/kefuOrderList/service/tpl/newOrderDetailKefu.html" %>
    ...
    <style type="text/css">
        ...
        .label-fullPay{background-color: #E4F7FF;}
        .label-notFullPay{background-color: #FDF5E9;}
        .label-notPay{background-color: #FEEEEE;}
        .label-fullPayFont{color: #0096DA;}
        .label-notFullPayFont{color: #FF9502;}
        .label-notPayFont{color: #F54142;}
    </style>
            ...
            <div id="divheader" class="accordion-body">
                <table class="table table-bordered table-hover" style="margin-bottom: 0px;">
                    ...
                    <tr>
                        ...
                        <td><label id="lblArrivalDate"><fmt:formatDate value="${order.orderCondition.arrivalDate}" pattern="yyyy-MM-dd HH:mm"/></label>
                            <c:if test="${status>=20 && status<60 && order.orderCondition.arrivalDate == null}">
                                <button type="button" class="btn btn-primary" id="btnArrivalDate" onclick="Order.arrivalDate('${order.id}','${order.orderNo}','${order.quarter}',orderdetail_index);">
                                    <i class="icon-truck icon-white"></i> 编辑
                                </button>
                            </c:if>
                        </td>
                    </tr>
                ...
                <table id="productTable" class="table table-bordered table-condensed table-hover" style="margin-bottom: 0px;pmargin-top:3px;">
                    ...
                    <c:forEach items="${order.items}" var="item">
                        <tr>
                            ...
                            <td>
                                <c:if test="${status>=30 && status<60}">
                                    <a id="btn_MaterialApply" onclick="Order.materialApply('${order.id}','${order.quarter}','0','${item.product.id}',orderdetail_index);" href="javascript:;" title="新增配件申请">申请配件</a>
                                </c:if>
                            </td>
                        </tr>
                ...
                <div id="divengineer" class="accordion-body">
                    <table class="table table-bordered table-hover" style="margin-bottom: 0px;">
                        ...
                        <tr>
                            ...
                            <td class="tdbody">
                                <c:if test="${order.orderCondition.servicePoint.mdDepositLevel!=null and order.orderCondition.servicePoint.mdDepositLevel.id>0}">
                                    <c:choose>
                                        <c:when test="${order.orderCondition.servicePoint.finance.deposit<=0}">
                                            <c:set var="depositLevelLabel" value="label label-notPay"/>
                                            <c:set var="depositLevelPic" value="depositNotPay.png"/>
                                            <c:set var="depositLevelFont" value="label-notPayFont"/>
                                            <c:set var="depositInfo" value="质保等级：${order.orderCondition.servicePoint.mdDepositLevel.name}<br/>状态：未缴"/>
                                        </c:when>
                                        <c:when test="${order.orderCondition.servicePoint.finance.deposit>=order.orderCondition.servicePoint.deposit}">
                                            <c:set var="depositLevelLabel" value="label label-fullPay"/>
                                            <c:set var="depositLevelPic" value="depositFullPay.png"/>
                                            <c:set var="depositLevelFont" value="label-fullPayFont"/>
                                            <c:set var="depositInfo" value="质保等级：${order.orderCondition.servicePoint.mdDepositLevel.name}<br/>应缴质保金：${order.orderCondition.servicePoint.deposit}元<br/>状态：已缴满"/>
                                        </c:when>
                                        <c:otherwise>
                                            <c:set var="depositLevelLabel" value="label label-notFullPay"/>
                                            <c:set var="depositLevelPic" value="depositNotFullPay.png"/>
                                            <c:set var="depositLevelFont" value="label-notFullPayFont"/>
                                            <c:set var="depositInfo" value="质保等级：${order.orderCondition.servicePoint.mdDepositLevel.name}<br/>应缴质保金：${order.orderCondition.servicePoint.deposit}元<br/>状态：未缴满 ${order.orderCondition.servicePoint.finance.deposit-order.orderCondition.servicePoint.deposit}元"/>
                                        </c:otherwise>
                                    </c:choose>
                                    <span class="${depositLevelLabel}" style="padding: 4px 5px;">
                                        <a style="text-decoration:none" data-toggle="tooltipSouth" data-tooltip="${depositInfo}">
                                            <img src="${ctxStatic}/images/${depositLevelPic}" style="margin-top: -2px;width: 16px;height: 16px"><span style="margin-left: 2px;text-shadow: none;" class="${depositLevelFont}">${order.orderCondition.servicePoint.mdDepositLevel.name}</span>
                                        </a>
                                     </span>
                                </c:if>
                                ${order.orderCondition.servicePoint.servicePointNo}
                            ...
                            <td class="tdbody" style="width:180px;">${order.orderCondition.servicePoint.level.label}</td>
                            <td class="tdlable"><label class="control-label">网点电话:</label></td>
                            <td class="tdbody" style="width:180px;">${order.orderCondition.servicePoint.contactInfo1}</td>
                        ...
                        <tr>
                            ...
                            <td class="tdbody" style="width:180px;">
...
<c:if test="${errorFlag == false}">
    ...
    <div class="tabbable" style="margin:0px 20px">
        <ul class="nav nav-tabs">
            ...
            <shiro:hasPermission name="sd:order:insurance">
                <c:if test="${status>=50 && status<=100}">
                    <li id="litabInsurance">
                        <a href="#tabInsurance" data-toggle="tab" id="lnktabInsurance" onclick="Order.showInsurance('${order.id}','${order.quarter}')">互助基金</a></li>
                    </li>
                </c:if>
            </shiro:hasPermission>
        </ul>
                ...
                <div class="tab-pane ${tabActiveName=="tabService"?"active":""}" id="tabService" title="实际上门清单">
                    <c:if test="${status >= 40 && status<60}">
                        <div class="btn-toolbar" style="margin-top: 3px;margin-bottom: 4px;">
                            <button class="btn btn-primary " type="button" title="添加服务明细" onclick="Order.addService('${order.id}',orderdetail_index);"><i class="icon-plus"></i> 添加</button>
                            <c:if test="${order.detailList.size()==0}">
                                <c:choose>
                                    <c:when test="${order.orderCondition.orderServiceType == 1}">
                                        &nbsp;&nbsp;&nbsp;<button class="btn btn-warning " type="button" onclick="Order.orderConfirmDoor('${order.id}','${order.quarter}',orderdetail_index,0);"><i class="icon-home"></i> 确认上门</button>
                                    </c:when>
                                    <c:otherwise>
                                        &nbsp;&nbsp;&nbsp;<button class="btn btn-warning " type="button" onclick="Order.addService('${order.id}',orderdetail_index);"><i class="icon-home"></i> 确认上门</button>
                                    </c:otherwise>
                                </c:choose>
                            </c:if>
                        </div>
...
<%--                                    <td>${item.serviceCategory.label}</td>--%>
                                    ...
                                    <td>
                                        <c:if test="${status>=40 && status<60}">
                                            <a name="btn_MaterialApply" onclick="Order.materialApply('${order.id}','${order.quarter}','${item.id}','${item.product.id}',orderdetail_index);" href="javascript:;" title="新增配件申请">申请配件</a>
                                        </c:if>
                                    </td>
                                </tr>
                                <c:set var="ridx" value="${ridx+1}"/>
            ...
            <shiro:hasPermission name="sd:order:insurance">
              <c:if test="${status>=50 && status<=100}">
                 <div class="tab-pane ${tabActiveName=="tabInsurance"?"active":""}" id="tabInsurance"></div>
               </c:if>
            </shiro:hasPermission>
        </div>
    </div>
...
<c:if test="${errorFlag == false}">
    <script type="text/javascript">
        ...
        function testLongAjaxReturn() {
            $.ajax({
                cache: false,
                type: "POST",
                url: "${ctx}/sd/order/testLongAjaxReturn",
                success: function (data) {
                    console.log(data.data);
                },
                error: function (e) {
                    console.log("订单回滚错误:" + e, "错误提示");
                }
            });
        }
