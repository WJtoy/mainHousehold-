...
<ul id="navtabs" class="nav nav-tabs">
    <li class="active"><a href="javascript:void(0);" title="催单待回复">催单</a></li>
    ...
    <li><a href="${ctx}/sd/order/servicePointOrderList/allList" title="所有的订单列表">所有</a></li>
</ul>
<c:set var="currentuser" value="${fns:getUser() }"/>
<form:form id="searchForm" modelAttribute="order" action="${ctx}/sd/order/servicePointOrderList/reminderList" method="post" class="breadcrumb form-search">
...
<table id="contentTable" class="table table-bordered table-condensed table-striped"
    ...
    <tr>
        <th width="30">序号</th>
        <th width="110">单号</th>
        <th width="145">首次催单时间</th>
        <th width="145">催单时效</th>
        <th width="80">接单时间</th>
        <th width="100">联系人</th>
        <th width="180">地址</th>
    ...
    <c:forEach items="${page.list}" var="order">
        ...
        <tr>
            ...
            <td>
                ...
                <c:if test="${order.orderStatus.complainFlag == 2}">
            ...
            <!-- reminder-->
            <td>${order.reminderDate}</td>
            <td>
                <c:set var="cutoffClass" value="normal"/>
                <c:choose>
                    <c:when test="${order.cutOffTimeLiness>1}">
                        <c:set var="cutoffClass" value="timeout"/>
                    </c:when>
                    <c:when test="${order.cutOffTimeLiness>0.67}">
                        <c:set var="cutoffClass" value="intime"/>
                    </c:when>
                </c:choose>
                <p class="reminder_${cutoffClass}">${order.cutOffLabel}</p>
                <c:if test="${order.orderStatus.reminderStatus == 1}">
                    <c:choose>
                        <c:when test="${order.orderCondition.engineer.id == currentuser.engineerId}">
                            <br/><a href="javascript:void(0);" class="btn btn-mini" onclick="Order.reminderToReply('${order.reminderId}','${order.quarter}');">回复</a>
                        </c:when>
                        <c:otherwise>
                            <br/><a href="javascript:void(0);" class="btn btn-mini" onclick="Order.reminderView('${order.reminderId}','','${order.quarter}');">查看</a>
                        </c:otherwise>
                    </c:choose>
                </c:if>
            </td>
            <td><fmt:formatDate value="${order.orderStatus.planDate}" pattern="yyyy-MM-dd"/></td>
                ...
                <shiro:hasPermission name="sd:order:engineeraccept">
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <li><a href="javascript:void(0);" onclick="Order.servicepointPending('${order.id}','${order.orderNo}','${order.quarter}',${order.orderCondition.reservationTimes});"><i
