	...
	<script type="text/javascript">
     ...
     /*       var picServicePointFee = $("#pic_info_"+ index).data("servicepointfee");
        ...
        $(document).on("click", "#btnSubmit", function () {
            ...
            var loadingIndex;
            var options = {
                url: "${ctx}/sd/abnormalForm/updatePraiseForKefu",                 //默认是form的action， 如果申明，则会覆盖
        ...
        function closePraise() {
            top.layer.confirm('取消后不能重新申请好评费,确定取消吗', {icon: 3, title:'系统确认'}, function(index){
                ...
                $.ajax({
                    url:"${ctx}/sd/abnormalForm/cancelled",
                    type:"POST",
                    data:{id:id,quarter:quarter,orderId:orderId,status:status,remarks:$("#remarks").val(),
			...
			<div class="row-fluid" style="margin-top: 7px">
				<div class="span12">
					<div class="control-group">
						<div class="controls">
							<%--<span style="margin-left: 570px">${praise.praiseNo}</span>&nbsp;&nbsp;<span style="background-color: #FF3A30;color: white;padding: 2px 4px;border-radius: 3px">${praise.strStatus}</span>--%>
								...
								<%--<c:if test="${praise.pics!=null && fn:length(praise.pics) >0}">
									<c:forEach items="${praise.pics}" var="pic" varStatus="picIndex">
										<div class="upload_warp_img_div" id="divImg_${picIndex.index + 1}">
											<a href="javascript:" title="点击删除图片" onclick="deletePic('${picIndex.index + 1}')" class="upload_warp_img_div_del"></a>
						...
						<%--</div>--%>
							<div>
								...
								<span style="color: red">支持jpg,jpeg,png格式,且大小不能超过2MB,建议尺寸600*800</span><br/>
									<%--	<div id="exampleDiv">
											<c:forEach var="examplePic" items="${customerPraiseFee.examplePicItems}" varStatus="picStatus">
												<img id="first${picStatus.index}" src="${examplePic.url}" data-original="${examplePic.url}" style="display:none;">
											</c:forEach>
										</div>--%>
