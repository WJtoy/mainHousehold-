...
<head>
	...
	<%@ include file="/WEB-INF/views/modules/sd/service/tpl/orderDetailKefu.html" %>
...
<body onload="load()">
	...
	<form:form id="inputForm" action="#" method="post" class="form-horizontal">
		...
		<c:if test="${errorFlag == false}">
			...
			<%--<c:set var="cancopy" value="${!order.isClosed()}" />--%>
			...
			<c:if test="${order.dataSource.value != '0'}">
				...
				<c:if test="${not empty order.parentBizOrderId}">
					<c:set var="ordermsg" value="${ordermsg}(${order.parentBizOrderId})"/>
				</c:if>
			</c:if>
			...
			<c:set var="msg" value="${order.orderCondition.userName}{{=it.servicePhone}}${' '} {{=it.serviceAddress}} ${msg}请2小时内联系用户确认环境并预约，48小时内上门，严禁对产品作任何评价，有问题请联系客服" />
		...
		<div class="row-fluid" data-spy="affix" data-offset-top="10" style="z-index: 999999999;background-color: #fff;">
			...
			<%--<c:if test="${cancopy eq true }">--%>
				...
				<div class="span3" style="height: 40px;line-height: 40px;">
					...
					<a class="btn btn-success" href="javascript:;" id="btn_engineer_copy" ><i class="icon-envelope-alt"></i> 安维短信</a>

					<c:if test="${status>=40}">
					<a class="btn btn-success" href="javascript:;" id="btn_user_copy" ><i class="icon-envelope-alt"></i> 用户短信</a>
					</c:if>
				...
				<table class="table table-bordered table-hover" style="margin-bottom: 0px;">
					<tbody>
						<tr>
							...
							<td class="tdbody">
								<c:if test="${order.dataSource.value == '2'}">
									<span><img style="width: 38px; height: 19px;" src="${ctxStatic}/images/tmall.png"/></span>
								</c:if>
								<span id="spOrderNo">
									<c:choose>
								...
								<%--<a class="btn btn-success btn-mini" href="javascript:;" id="btnReload" onclick="reload();" >调用刷新</a>--%>
								<%--<a class="btn btn-success btn-mini" href="javascript:;" id="btnReload" onclick="refreshMaterialButton();" >显示配件按钮</a>--%>
								<%--<a class="btn btn-success btn-mini" href="javascript:;" id="btnReload" onclick="testLongAjaxReturn();" >测试返回long</a>--%>
								<%--<a class="btn btn-success btn-mini" href="javascript:;" id="btn_engineer_edit" >安维短信</a>--%>
								<%--<a class="btn btn-success btn-mini" href="javascript:;" id="btn_user_edit" >用户短信</a>--%>
							</td>
							<td class="tdlable">
								<label class="control-label">客户名称:</label>
							</td>
							<td class="tdbody">
								<a href="javascript:" data-toggle="tooltipeast" data-tooltip="${order.orderCondition.customer.remarks}">${order.orderCondition.customer.name}</a>
								<%--<c:if test="${order.dataSource.value != '1' && !order.dataSource.value != '0'}">--%>
								<c:if test="${order.dataSource.value != '0'}">
						...
						<tr>
							...
							<td class="tdlable"><label class="control-label">到货日期:</label></td>
							<td><label id="lblArrivalDate"><fmt:formatDate value="${order.orderCondition.arrivalDate}" pattern="yyyy-MM-dd HH:mm"/></label>
								...
								<button type="button" class="btn btn-primary" id="btnArrivalDate" onclick="Order.arrivalDate('${order.id}','${order.orderNo}','${order.quarter}',orderdetail_index);"><i class="icon-truck icon-white"></i>  编辑</button>
								</c:if>
							</td>
						</tr>
					</tbody>
				...
				<table id="productTable"
					...
					<!-- 客户备注 -->
						<tr>
							...
							<td colspan="7">${order.orderCondition.customer.remarks}</td>
						</tr>
					</tbody>
				</table>
			</div>
	...
	<script class="removedscript" type="text/javascript">
        $(document).ready(function(){
            ...
            Order.detail_order_toolbar(orderInfo); //button
            // orderInfo.orderCondition.layerIndex = orderdetail_index;//layer index of this dialog
            // Order.detail_order_toolbar(orderInfo.orderCondition); //button
            //load active tab
...
//            if($('#divserviceTable').length>0) {
//			    $('#divserviceTable').slimscroll({height: '300px'});
//            }
        });
	</script>
	</c:if>
...
<script type="text/javascript">
    $(document).ready(function() {
		...
		//短信控制
        $('#btn_engineer_edit').click(function() {
            ...
            top.layer.open({
                ...
                ,success: function(layero){
                    var btn = layero.find('#btn_engineer_copy');
                    btn.click();
                }
            });
            return false;
            ...
            success: function (data) {
                ...
                if (data.success ) {
                    if(data.data && data.data.length>0) {
                        ...
                        var doTtmpl = doT.template(tmpl);
                        var item = {parentLayerIndex:parentLayerIndex,data:data.data,servicemonitor:servicemonitor};
