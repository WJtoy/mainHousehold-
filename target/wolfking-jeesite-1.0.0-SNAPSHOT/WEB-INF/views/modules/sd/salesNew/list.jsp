...
<form:form id="searchForm" modelAttribute="order" action="${ctx}/sd/order/salesNew/list" method="post"
...
<%--        <input type="text" class="input-small" id="address" name="address" value="${order.address}" maxlength="100"/>--%>
    ...
    <div style="margin-top:5px">
        ...
        <%--<c:set var="pendingList" value="${fns:getDictListFromMS('PendingType')}"/>&lt;%&ndash;切换为微服务&ndash;%&gt;--%>
        <%--<select id="pendingType" name="pendingType" class="required input-small" style="width: 98px;">--%>
            <%--<option value="">全部</option>--%>
            <%--<c:forEach items="${pendingList}" var="pending">--%>
                <%--<option value="${pendingType.value}">${pendingType.label}</option>--%>
            <%--</c:forEach>--%>
        <%--</select>--%>
