...
<head>
    <title>工单投诉单待处理列表</title>
    <meta name="description" content="待处理">
...
<ul id="navtabs" class="nav nav-tabs">
    <shiro:hasPermission name="sd:complain:judge">
        <li class="active"><a href="javascript:void(0);" title="待处理投诉单列表">待处理</a></li>
...
<form:form id="searchForm" modelAttribute="complain" action="${ctx}/sd/complain/judgelist" method="post" class="form-inline">
                ...
                <td>
                    <c:choose>
                        <c:when test="${model.status.value eq '0' || model.status.value eq '1'}">
                            <shiro:hasPermission name="sd:complain:judge">
                                <a href="javascript:void(0);" onclick="Order.complain_judge('${model.id}','${model.quarter}');">${model.complainNo}</a>
                            </shiro:hasPermission>
                        </c:when>
