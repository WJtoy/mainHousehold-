...
<script type="text/javascript">
    ...
    $(document).on("click", "#btnSubmit", function () {
        if (clicktag == 0) {
            ...
            beforePage();
            setPage();
            $("#searchForm").attr("action","${ctx}/sd/kefuOrderMaterial/alllist");
            $("#searchForm").submit();
        }
    });
    ...
    $(document).on("click", "#btnExport", function () {
        ...
        layer.confirm('确定导出配件列表吗?', {icon: 3, title: '系统确认'}, function (index, layero) {
            ...
            jQuery('<form id="exportForm" action="${ctx}/sd/kefuOrderMaterial/alllist/export" method="post" style="display:none;">'+ html + '</form>')
...
<ul id="navtabs" class="nav nav-tabs">
	...
	<li><a href="${ctx}/sd/kefuOrderMaterial/tosendlist" title="待发货配件单">待发货</a></li>
	...
	<li class="active"><a href="javascript:void(0);" title="所有配件单">所有<span id="spn_count" class="badge badge-info">${page !=null ?page.count:''}</span></a></li>
	<li><a href="${ctx}/sd/kefuOrderMaterial/waitSignMaterialReturnList" title="待签收(旧件)">待签收(旧件)</a></li>
</ul>
<form:form id="searchForm" modelAttribute="searchModel" method="post" action="${ctx}/sd/kefuOrderMaterial/alllist" class="breadcrumb form-search">
	...
	<tbody>
		<c:forEach items="${page.list}" var="master" varStatus="i" begin="0">
			...
			<tr id="tr_${master.id}" data-index="${i.index}">
				...
				<c:choose>
					<c:when test="${master.closeBy.id>0}">
						<td rowspan="${rowspan}">${master.closeBy.name}</td>
						<td rowspan="${rowspan}"><fmt:formatDate value="${master.closeDate}" pattern="yyyy-MM-dd HH:mm"/> </td>
						<td rowspan="${rowspan}"><a href="javascript:" data-toggle="tooltip"  data-tooltip=" ${master.closeRemark}">${fns:abbr(master.closeRemark,40)}</a></td>
					</c:when>
					<c:otherwise>
