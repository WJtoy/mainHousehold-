...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="javascript:void(0);">网点师傅管理</a></li>
		<li><a href="${ctx}/tmall/md/servicepoint/servicePointBatchProcessList">网点批处理</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="mdB2bTmall" action="${ctx}/tmall/md/servicepoint/workerList" method="POST" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li>
				<label>师傅名称：</label>
				<form:input path="engineer.name" htmlEscape="false" maxlength="30"	class="input-small" />
			</li>
			<li>
				<label>师傅电话：</label>
				<form:input path="engineer.contactInfo" htmlEscape="false" maxlength="20" class="input-small" />
			</li>
        ...
        <c:set var="ACTION_TYPE_DELETE" value="<%=B2BActionType.ACTION_TYPE_DELETE.value%>"/>
			<c:forEach items="${page.list}" var="item">
				...
				<tr id="${item.servicePoint.id}">
					...
					<td>${item.engineer.contactInfo}</td>
					<td>${item.engineer.address}</td>
					<td>
						<a class="btn btn-primary" href="${ctx}/tmall/md/servicepoint/processWorker?id=${item.id}">同步 -
                            <c:choose>
