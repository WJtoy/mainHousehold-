...
<html>
  ...
  <body>
	...
	<form:form id="searchForm" modelAttribute="auxiliaryMaterialCategory" action="${ctx}/provider/md/auxiliaryMaterialCategory/getList" method="post" class="breadcrumb form-search">
	    ...
	    <select name="type" style="width:225px;">
            <c:choose>
                <c:when test="${auxiliaryMaterialCategory.type==1}">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-hover">
		<thead>
			<tr>
				...
				<th width="200">类型</th>
				<th>备注</th>
				<shiro:hasPermission name="md:auxiliarymaterialcategory:edit"><th width="100">操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		...
		<c:forEach items="${page.list}" var="entity">
			<tr>
				...
				<td>
                    <a href="${ctx}/provider/md/auxiliaryMaterialCategory/form?id=${entity.id}">${entity.name}</a>
                </td>
				<c:choose>
				...
				<shiro:hasPermission name="md:auxiliarymaterialcategory:edit"><td>
    				<a href="${ctx}/provider/md/auxiliaryMaterialCategory/form?id=${entity.id}">修改</a>
					<a href="${ctx}/provider/md/auxiliaryMaterialCategory/delete?id=${entity.id}" onclick="return confirmx('确认要删除吗？', this.href)">删除</a>
