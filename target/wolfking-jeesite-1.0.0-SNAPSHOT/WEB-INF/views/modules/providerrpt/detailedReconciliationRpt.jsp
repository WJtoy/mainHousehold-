    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function () {
            ...
            $("#btnSubmit").click(function () {
                $("#pageNo").val(1);
                $("#searchForm").attr("action", "${ctx}/rpt/provider/detailedReconciliation/getNetworkReconciliation");
                $("#searchForm").submit();
            });
            ...
            $("#btnExport").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "${ctx}/rpt/provider/detailedReconciliation/checkExportTask?" + (new Date()).getTime(),
                        ...
                        if (data && data.success == true) {
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/rpt/provider/detailedReconciliation/export?" + (new Date()).getTime(),
                                        data: $(searchForm).serialize(),
                                        success: function (data) {
                                            if (ajaxLogout(data)) {
                                                return false;
...
<form:form id="searchForm" modelAttribute="rptSearchCondition" action="${ctx}/rpt/provider/detailedReconciliation/getNetworkReconciliation" method="post" class="breadcrumb form-search" onsubmit="return validate(this)">
