	...
	<script type="text/javascript" language="javascript">
		$(document).ready(function() {
			...
			$("#btnSubmit").click(function(){
				...
				$("#searchForm").attr("action","${ctx}/rpt/provider/gradedOrder/kefuOrderCompletedDailyRpt");
				$("#searchForm").submit();
			});
			...
			$("#btnExport").click(function () {
				...
				$.ajax({
					type: "POST",
					url: "${ctx}/rpt/provider/gradedOrder/kefuCompletedCheckExportTask?"+ (new Date()).getTime(),
						...
						if(data && data.success == true){
							...
							top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
								if (v == "ok") {
									...
									$.ajax({
										type: "POST",
										url: "${ctx}/rpt/provider/gradedOrder/kefuCompletedExport?"+ (new Date()).getTime(),
										data:$(searchForm).serialize(),
										success: function (data) {
											if(ajaxLogout(data)){
												return false;
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="javascript:void(0);">每日完工单情况</a></li>
		<li>
			<a href="${ctx}/rpt/provider/gradedOrder/keFuOrderCompleteChart">客服完工图表</a>
		</li>
	</ul>
	<form:form id="searchForm" modelAttribute="rptSearchCondition" action="${ctx}/rpt/provider/gradedOrder/kefuOrderCompletedDailyRpt" method="post" class="breadcrumb form-search">
		<div style="width: 90%;">
			...
			<div style="position: relative" class="parent">
				...
				<div style="text-align:right;position: absolute;width: 300px;height: 100px;right:1px;top:31px;" class="target">
					<div style="text-align:left;background-color: #1B1E24;border-radius: 5px;opacity: 0.8;padding: 10px;font-size:14px;color:white;min-width:100px;max-width:400px;">
						...
						统计方式：客评时间 <br/>
						栏位说明：<br/>
						【每日完工环比】当日完工单量和上月平均每天完工单量比较 <br/>
						环比计算公式 = (当月每日完工单量-上月平均每日完工单量)/上月平均每日完工单量 <br/>
						【每日完工同比】当日完工单量和去年同月平均每天完工单量比较 <br/>
						同比计算公式 = (当月每日完工单量-去年同月平均每日完工单量)/去年同月平均每日完工单量
					</div>
				</div>
