    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#btnSubmit").click(function() {
                ...
                $("#searchForm").attr("action","${ctx}/rpt/provider/keFuAverageOrderFee/CWKeFuAverageOrderFeeReport");
                $("#searchForm").submit();
            });
            ...
            $("#btnExport").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "${ctx}/rpt/provider/keFuAverageOrderFee/checkCWExportTask?"+ (new Date()).getTime(),
                        ...
                        if(data && data.success == true){
                            ...
                            top.$.jBox.confirm("确认要导出数据吗？", "系统提示", function (v, h, f) {
                                if (v == "ok") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "${ctx}/rpt/provider/keFuAverageOrderFee/cwExport?"+ (new Date()).getTime(),
                                        data:$(searchForm).serialize(),
                                        success: function (data) {
                                            if(ajaxLogout(data)){
                                                return false;
...
<ul class="nav nav-tabs">
    <li>
        <a href="${ctx}/rpt/provider/keFuAverageOrderFee/vipKeFuAverageOrderFeeReport">KA均单费用</a>
    </li>
    <li>
        <a href="${ctx}/rpt/provider/keFuAverageOrderFee/keFuAverageOrderFeeReport">客服均单费用</a>
    </li>
    <li class="active"><a href="javascript:void(0);">厨电客服均单费用</a></li>
</ul>
<form:form id="searchForm" modelAttribute="rptSearchCondition" action="${ctx}/rpt/provider/keFuAverageOrderFee/CWKeFuAverageOrderFeeReport" method="post" class="breadcrumb form-search">
    <div style="width: 90%">
        ...
        <c:choose>
            ...
            <c:otherwise>
                ...
                <select id="customerId" name="customerId" class="input-small" style="width:225px;">
                    ...
                    <c:forEach items="${fns:getMyNotVipCustomerList()}" var="dict">
        ...
        <div style="position: relative" class="parent">
            ...
            <div style="text-align:right;position: absolute;width: 250px;height: 100px;right:1px;top:31px;" class="target">
                <div style="text-align:left;background-color: #1B1E24;border-radius: 5px;opacity: 0.8;padding: 10px;font-size:14px;color:white;min-width:100px;max-width:400px;">
                    ...
                    统计方式：客评时间<br/>
                    栏位说明：<br/>
                    【完成单数】省市区的完成单数(非VIP客户)<br/>
                    【合计费用】其他费用+远程费用(非VIP客户其他远程费用)<br/>
                    【均单费用】合计费用/完成单数
                </div>
            </div>
