...
<form:form id="searchForm" modelAttribute="order" action="${ctx}/sd/finance/customerOrder/list" method="post"
    ...
    <div>
        ...
        <sys:treeselect id="area" name="area.id" value="${order.area.id}"
                        ...
                        nameLevel="3"/>  --%>
        <%--<sys:treeselectareanew id="area" name="area.id" value="${order.area.id}"
                        labelName="area.name" labelValue="${order.area.name }" title="区域"
                        url="/sys/area/treeDataNew" allowClear="true" nodesLevel="-1" nodeLevel="true"
                        levelValue="${order.areaLevel}" loadFourLevel="false"
                        nameLevel="3"/>--%>
