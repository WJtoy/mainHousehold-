...
<head>
    ...
    <script type="text/javascript">
        ...
        function editServicePoint(id){
            var text = "添加服务网点";
            var url = "${ctx}/fi/md/servicePoint/form";
            if(id != null){
                text = "修改";
                url = "${ctx}/fi/md/servicePoint/newForm?id=" + id;
            }
        ...
        function removeServicePoint(id,name){
            layer.confirm(
                '确认要删除网点' +'<label style="color:#63B9E6">'+ name +'</label>吗？',
                {
                    ...
                    $.ajax({
                        url: "${ctx}/fi/md/servicePoint/ajax/delete?id="+id+"&type=listDelete",
        ...
        function synchronizationServicePoint(id){
            layer.confirm(
                '确认要同步网点数据到服务器吗？',
                {
                    ...
                    $.ajax({
                        url: "${ctx}/fi/md/servicePoint/ajax/syncDataToEs?id=" + id,
                        success:function (data) {
                            // 提交后的回调函数
                            if(loadingIndex) {
                                setTimeout(function () {
...
<ul class="nav nav-tabs">
    ...
    <li><a href="${ctx}/fi/md/servicePoint/disableList">停用网点</a></li>
</ul>
<sys:message content="${message}" type="loading"/>
<shiro:hasPermission name="fi:md:servicepoint:view">
    <form:form id="searchForm" modelAttribute="servicePoint" action="${ctx}/fi/md/servicePoint" method="POST"
...
<%--                <td>${point.name}</td>--%>
                ...
                <shiro:hasAnyPermissions name="fi:md:servicepoint:edit,fi:md:servicepoint:delete">
                    <td id="td_left">
                        ...
                        <shiro:hasAnyPermissions name="fi:md:engineer:edit">
