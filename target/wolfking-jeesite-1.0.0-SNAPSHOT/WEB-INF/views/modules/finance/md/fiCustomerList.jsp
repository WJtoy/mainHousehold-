...
<head>
    ...
    <script type="text/javascript">
        ...
        function editCustomer(type,id) {
            var text = "添加客户";
            var url = "${ctx}/fi/md/customer/form?sort=" + type;
            var area = ['1000px', '888px'];
            if(type == 20){
                text = "修改";
                url = "${ctx}/fi/md/customer/form?id=" + id;
            }
            top.layer.open({
                type: 2,
                id:"customer",
        ...
        function enableCustomer(customerId,name){
            var loadingIndex;
            $.ajax({
                url: "${ctx}/fi/md/customer/selectCustomerOrder?id="+ customerId,
                    ...
                    if (data.success) {
                        layer.confirm(
                            '确认要删除客户<label style="color:#63B9E6">'+name +'</label>吗？',
                            {
                                ...
                                $.ajax({
                                    url: "${ctx}/fi/md/customer/ajax/delete?id="+ customerId,
                                    success:function (data) {
                                        // 提交后的回调函数
                                        if(loadingIndex) {
                                            setTimeout(function () {
...
<shiro:hasPermission name="fi:md:customer:view">
<form:form id="searchForm" modelAttribute="mdCustomer" action="${ctx}/fi/md/customer/list" method="post" class="breadcrumb form-search">
        ...
        <tr>
            ...
            <shiro:hasPermission name="fi:md:customer:edit">
                <td>
                    ...
                    <shiro:hasPermission name="fi:md:customer:detelete">
