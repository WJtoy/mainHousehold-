...
<head>
    ...
    function resend(){
        ...
        var params = {dataSource: $("#dataSource").val(), b2bInterfaceId: $("#b2bInterfaceId").val(),processLogId:$("#processLogId").val(),completesdate:"${completedRetryBean.completesdate!=null?completedRetryBean.completesdate:0}",orderNo:"${completedRetryBean.orderNo}"};
        var pic1 = [];
        var pic2 = [];
        var pic3 = [];
        var pic4 = [];
        ...
        top.layer.confirm('确定重发吗？', {icon: 3, title:'系统确认'}, function(index){
            ...
            $.ajax({
                cache: false,
                type: "POST",
                url: "${ctx}/b2b/rpt/processlog/joyoungCompletedRetry",
                dataType: 'json',
    ...
    function closeme(){
        ...
        top.layer.confirm('确定忽略本条信息吗？', {icon: 3, title:'系统确认'}, function(index){
            ...
            $.ajax({
                cache: false,
                type: "POST",
                url: "${ctx}/b2b/rpt/processlog/joyoungCompletedCloseLog",
...
<form:form id="inputForm" modelAttribute="completedRetryBean" action="${ctx}/b2b/rpt/processlog/canboResend" method="post" class="form-horizontal">
    ...
    <input type="hidden" id="completesdate" name="completesdate" value="${completedRetryBean.completesdate}">
    <div class="row-fluid">
    ...
    <c:forEach items="${completedRetryBean.items}" var="item">
        ...
        <div class="row-fluid" style="margin-top: 10px;min-height: 30px;">
                    <label style="color:red;margin-left: 80px">失败原因:</label>
                    <label style="margin-left: 2px;text-align: left;color:red">
