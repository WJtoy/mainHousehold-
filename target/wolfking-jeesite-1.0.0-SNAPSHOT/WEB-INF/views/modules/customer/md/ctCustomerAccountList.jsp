    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("#btnSubmit").on("click", function(){
                ...
                var url = "${ctx}/customer/md/customerAccount/list";
        ...
        function disableOrEnable(line,obj,flag,userId,loginName,customerId){
            ...
            if (delFlag == 1) {
                ...
                url = "${ctx}/customer/md/customerAccount/enable?id="+userId+"&loginName="+loginName+"&customerAccountProfile.customer.id=" + customerId;

                msg = '正在启用账号，请稍等...'
            } else {
                title = "停用";
                content = '确认要停用【'+loginName+'】账号吗？';
                url = "${ctx}/customer/md/customerAccount/delete?id="+userId+"&loginName="+loginName+"&customerAccountProfile.customer.id=" + customerId;
                msg = '正在停用账号，请稍等...'
            }
            layer.confirm(
                content,
        ...
        function editCustomerAccount(type,id) {
            var text = "添加子账号";
            var url = "${ctx}/customer/md/customerAccount/form";
            var area = ['1000px', '580px'];
            if(type == 20){
                text = "修改子账号";
                area = ['1000px', '676px'];
                url = "${ctx}/customer/md/customerAccount/form?id=" + id;
            }
            top.layer.open({
                type: 2,
                id:"user",
        ...
        function resetPassword(id){
            ...
            top.layer.confirm("重置密码后，需要重新登录，<br/>确定要<font color='blue'>重置</font>密码吗?", {icon: 3, title:'系统确认'}, function(index){
                ...
                $.ajax({
                    cache: false,
                    type: "POST",
                    url: "${ctx}/customer/md/customerAccount/resetPassword",
                    data: data,
                    success: function (data) {
                        if (data.success){
...
<form:form id="searchForm" modelAttribute="user" action="${ctx}/customer/md/customerAccount/list" method="post" class="breadcrumb form-search">
    ...
    <ul class="ul-form">
        ...
        <li>
            <label style="margin-left: 28px">帐户类型：</label>
