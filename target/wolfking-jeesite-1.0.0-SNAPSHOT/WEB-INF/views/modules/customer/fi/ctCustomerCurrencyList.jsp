...
<ul class="nav nav-tabs">
    ...
    <shiro:hasPermission name="customer:fi:customerCurrency:charge"><li><a href="${ctx}/customer/fi/customerCurrency/form">充值</a></li></shiro:hasPermission>
</ul>
...
<form:form id="searchForm" modelAttribute="customerCurrency" action="${ctx}/customer/fi/customerCurrency/list" method="post" class="breadcrumb form-search">
    ...
    <div>
        ...
        <c:if test="${currentuser.isCustomer()}">
            <shiro:hasPermission name="customer:fi:customercurrency:chargeonline"><a class="btn btn-primary" target="_blank" href="${ctx}/fi/customercurrency/chargeonline">在线充值</a>
            </shiro:hasPermission>
        </c:if>
...
<table id="treeTable" class="table table-striped table-bordered table-condensed table-hover">
    ...
    <tr>
        ...
        <th>创建时间</th>
        <th>修改时间</th>
        <shiro:hasPermission name="customer:fi:customercurrency:view">
    ...
    <c:forEach items="${page.list}" var="customerCurrency">
        ...
        <c:if test="${!(customerCurrency.customer.id eq customerId) }">
            ...
            <tr id="${customerCurrency.customer.id}" pId="">
                ...
                <td>
                    <shiro:hasPermission name="customer:fi:customercurrency:charge">
                        <a href="${ctx}/customer/fi/customerCurrency/form?customer.id=${customerCurrency.customer.id}">充值</a>
                    </shiro:hasPermission>
                </td>
                <td></td>
            </tr>
        ...
        <tr id="${customerCurrency.id}" pId="${customerId}">
            ...
            <shiro:hasPermission name="customer:fi:customercurrency:view"><td>
                <a href="${ctx}/customer/fi/customerCurrency/form?id=${customerCurrency.id}">查看</a>
            </td>
            </shiro:hasPermission>
        </tr>
    </c:forEach>
