...
<div class="input-append">
    ...
    <input id="${id}Name" name="${labelName}" readonly="readonly" type="text" value="${labelValue}" maxlength="80" placeholder="请选择区域,可选择街道"
...
<script type="text/javascript">
    $("#${id}Button").click(function(){
        ...
        var replyIdex = top.layer.open({
            ...
            zIndex: 29891015,
            title:'选择${title}',
            content: "${ctx}/tag/treeselectarea?url="+encodeURIComponent("${url}${fn:indexOf(url,'?')==-1?'?':'&'}filter="+filtervalue)+"&module=&checked=${checked}&extId=&nodesLevel=${nodesLevel}&selectParentId=" + $("#${id}ParentId").val() + "&selectIds="+$("#${id}Id").val() + "&areaLevel=" + $("#${id}Level").val(),
            shade: 0.3,
            area: ['320px', '420px'],
            maxmin: false,
            btn: btnlist,
