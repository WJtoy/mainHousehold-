...
<script type="text/javascript">
    $("#${id}Button").click(function(){
        ...
        // 正常打开
        top.$.jBox.open("iframe:${ctx}/tag/treeselectarea?url="+encodeURIComponent("${url}${fn:indexOf(url,'?')==-1?'?':'&'}filter="+filtervalue)+"&module=${module}&checked=${checked}&extId=${extId}&nodesLevel=${nodesLevel}&selectIds="+$("#${id}Id").val(), "选择${title}", 300, 420, {
            persistent:false,
